...
The 2026-07-10 live catalog audit has 17,694 `product_data` rows, 13,123 dog/cat rows, 17,067 rows with product images, and 12,188 rows that meet the broad complete-food/ingredient-count gate. The strict source-backed contract has 8,297 `verified_ready` rows: 4,947 dog and 3,350 cat. The remaining quality states are 2,774 `needs_ingredients`, 1,117 `identity_only`, and 5,506 `excluded`. `catalog_acquisition_queue` still has 4,787 open/in-progress rows affecting 7,865 products: 3,817 product rows and 970 brand rows; all 4,787 need verified ingredients, 4,770 need a verified image, and 849 need pet type. A transaction-rolled-back strict reconciliation probe across the ten highest-demand brands resolved zero rows, confirming that the queue is not padded by stale exact matches. Do not claim complete US dog/cat food coverage yet.
