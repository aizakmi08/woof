...
38. The native StoreKit review request runs only after repeat successful good-scoring pet-food scans for the current guest/account, never more than once per app version, does not overlap the first-scan toast or post-scan paywall prompt, and never displays a custom review card before the system prompt.
