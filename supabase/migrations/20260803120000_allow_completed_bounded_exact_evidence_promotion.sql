-- A deliberately bounded official-source batch is not a complete catalog crawl,
-- but it can still provide complete evidence for every formula in that batch.
-- Permit promotion only when the bounded batch is explicitly exact, internally
-- complete, and has zero rejected observations. All existing ingredient,
-- identity, image, and serving-row postconditions remain mandatory.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE observation.formula_id = p_formula_id
      ...
      AND (
        source_run.pagination_complete
        OR (
          source_run.metadata->>'bounded_exact_evidence' = 'true'
