WITH inventory AS (
  ...
  WHERE run.run_key =
      'nutrish:official-formula-inventory:47272a75c19a34b64e7db4c0'
    AND serving.source_url LIKE 'https://www.nutrish.com/%'
...
BEGIN
      ...
      WHERE public.normalize_verified_product_search_query(
        ...
        AND EXISTS (
          ...
          WHERE alias.active
            ...
            AND alias.normalized_alias =
                public.normalize_verified_product_search_query(
                  concat('Nutrish ', inventory.product_name)
                )
        )
    )
  INTO
  ...
  IF v_formula_count <> 34
     OR v_exact_alias_count <> v_formula_count
     OR v_brand_prefix_needed <> 3
     OR v_brand_prefix_count <> v_brand_prefix_needed THEN
    RAISE EXCEPTION
      'Nutrish aliases incomplete: % exact for % formulas, % brand aliases for % needed',
      v_exact_alias_count,
      v_formula_count,
      v_brand_prefix_count,
      v_brand_prefix_needed;
