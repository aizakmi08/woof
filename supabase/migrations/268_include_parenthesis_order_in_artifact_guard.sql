-- Keep the general ingredient artifact guard aligned with the stronger
-- ordered-parentheses validator. Equal open/close counts are not enough:
-- strings like "Chicken), Minerals (Zinc Sulfate" are still malformed.
...
BEGIN
  IF public.catalog_has_ingredient_ocr_artifacts(
    'Chicken, Chicken Fat (Preserved With Mixed Tocopherols), Salt'
  ) THEN
    RAISE EXCEPTION 'valid ordered ingredient parentheses must not be flagged';
  END IF;

  IF NOT public.catalog_has_ingredient_ocr_artifacts(
    'Chicken), Minerals (Zinc Sulfate'
  ) THEN
    RAISE EXCEPTION 'negative-depth ingredient parentheses must be flagged';
  END IF;

  IF NOT public.catalog_has_ingredient_ocr_artifacts(
    'Minerals (Zinc Sulfate, Sodium Selenite), Choline Chloride, Sodium Selenite), Mixed Tocopherols (Preservative'
  ) THEN
    RAISE EXCEPTION 'ordered parenthesis artifacts with equal counts must be flagged';
  END IF;
  ...
  IF public.catalog_has_ingredient_ocr_artifacts(
    ...
    RAISE EXCEPTION 'balanced official vitamin/mineral curly groups must not be flagged';
  END IF;
END;
$$;
