-- Extend the OCR artifact detector with additional Solid Gold label OCR
-- misspellings found during manual ingredient review.
...
DO $$
BEGIN
  IF public.catalog_has_ingredient_ocr_artifacts('Magnesium Sulfate, Potassium Chloride, Niacin Supplement') THEN
    RAISE EXCEPTION 'valid sulfate/chloride/niacin ingredient text must not be flagged';
  END IF;

  IF NOT public.catalog_has_ingredient_ocr_artifacts('Magnesium Sufate, Potassium Chioride, Nacin Supplement') THEN
    RAISE EXCEPTION 'Solid Gold OCR misspellings must be flagged';
  END IF;
END;
$$;
