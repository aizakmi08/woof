-- Avoid creating a second serving row when a staged formula is already present
-- under a source-specific cache key. Reuse is permitted only when exact
-- identity, structured variant fields, ingredients, and front image all agree.

ALTER FUNCTION public.promote_catalog_formula_without_completed_run_gate(BIGINT)
  RENAME TO promote_catalog_formula_without_exact_evidence_reuse;
...
BEGIN
  ...
  IF v_formula.promoted_cache_key IS NULL THEN
    ...
    WHERE lower(btrim(COALESCE(existing.brand, '')))
      ...
      AND lower(btrim(COALESCE(existing.product_line, '')))
            = lower(btrim(COALESCE(v_formula.product_line, '')))
      AND lower(btrim(COALESCE(existing.flavor, '')))
            = lower(btrim(COALESCE(v_formula.flavor, '')))
      AND lower(btrim(COALESCE(existing.life_stage, '')))
            = lower(btrim(COALESCE(v_formula.life_stage, '')))
...
REVOKE ALL ON FUNCTION public.promote_catalog_formula_without_exact_evidence_reuse(BIGINT)
  FROM PUBLIC;
REVOKE ALL ON FUNCTION public.promote_catalog_formula_without_exact_evidence_reuse(BIGINT)
  FROM anon;
REVOKE ALL ON FUNCTION public.promote_catalog_formula_without_exact_evidence_reuse(BIGINT)
