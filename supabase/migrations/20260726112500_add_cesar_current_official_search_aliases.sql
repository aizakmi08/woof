WITH inventory AS (
  ...
  WHERE run.run_key =
      'cesar-mars-petcare:official-formula-inventory:0c4b401c59b32d12206ac887'
    AND serving.source_url LIKE 'https://www.cesar.com/%'
      ...
      WHERE public.normalize_verified_product_search_query(
        ...
        AND EXISTS (
          ...
          WHERE alias.active
            ...
            AND alias.normalized_alias =
                public.normalize_verified_product_search_query(
                  concat('Cesar ', inventory.product_name)
                )
        )
    )
  INTO
