-- Tighten the identity reconciler after live sample review: a queued recipe
-- must name at least one protein, and the source-backed catalog row must have
-- the same protein set. Rows resolved by the looser 094 guard are reopened
-- when they no longer pass this stricter check.
...
SET
  status = 'open',
  resolved_at = NULL,
  resolution_reason = 'reopened after stricter source-backed identity guard',
  updated_at = now(),
  sample_metadata = q.sample_metadata || jsonb_build_object(
    'reopened_at', now(),
    'reopened_by', '095_stricter_catalog_acquisition_identity_guard',
