-- If a newly promoted version-safe GTIN does not satisfy the runtime resolver
-- postcondition, preserve the exact formula for typed/photo search but remove
-- deterministic barcode ownership and record an explicit abstention.
...
DECLARE
  ...
  v_postcondition_abstain_count INTEGER := 0;
...
BEGIN
  ...
  CREATE TEMP TABLE IF NOT EXISTS
    pg_temp.reviewed_package_postcondition_abstain (
      observation_id BIGINT PRIMARY KEY,
      gtin TEXT NOT NULL
    )
  ON COMMIT DROP;

  DELETE FROM pg_temp.reviewed_package_postcondition_abstain;

  INSERT INTO pg_temp.reviewed_package_postcondition_abstain (
    observation_id,
    gtin
  )
  SELECT
  ...
  INTO v_postcondition_abstain_count
  FROM pg_temp.reviewed_package_postcondition_abstain;

  UPDATE public.catalog_skus sku
  SET active = false, updated_at = now()
  FROM pg_temp.reviewed_package_postcondition_abstain abstention
  WHERE sku.gtin = abstention.gtin
    AND sku.active;
  ...
  SET
    ...
    formula_version_provenance =
      COALESCE(serving.formula_version_provenance, '{}'::JSONB) ||
      jsonb_build_object(
        'package_gtin', abstention.gtin,
        'barcode_resolution_policy',
          'abstain_failed_safe_gtin_postcondition',
        'barcode_postcondition_checked_at', now()
      ),
    updated_at = now()
  FROM pg_temp.reviewed_package_postcondition_abstain abstention
  WHERE regexp_replace(COALESCE(serving.gtin, ''), '\D', '', 'g') =
  ...
  FROM public.catalog_source_runs source_run,
    pg_temp.reviewed_package_postcondition_abstain abstention
  WHERE observation.run_id = source_run.id
    AND source_run.run_key = p_accepted_run_key
    AND observation.raw_payload->>'target_upc' = abstention.gtin;
  ...
  IF EXISTS (
    ...
    WHERE EXISTS (
      SELECT 1
      FROM public.resolve_verified_product_by_gtin(abstention.gtin, 8)
    )
  ) THEN
    RAISE EXCEPTION
      'A failed version-safe retailer GTIN still resolved after abstention';
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'source_version_safe_package_count', v_safe_count,
    'safe_gtin_postcondition_abstention_count',
      v_postcondition_abstain_count,
