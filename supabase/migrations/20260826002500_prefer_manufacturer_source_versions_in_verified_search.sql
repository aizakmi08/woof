-- Prefer the highest-authority source version while collapsing identical
-- brand/product/species search candidates. This ensures an official
-- manufacturer row carrying source-backed typical analysis is not discarded
-- in favor of an identically named retailer record before outer ranking runs.

CREATE OR REPLACE FUNCTION public.search_verified_products_base_v2(q text, max_results integer DEFAULT 10)
 RETURNS TABLE(cache_key text, product_name text, brand text, gtin text, product_line text, flavor text, life_stage text, food_form text, package_size text, pet_type text, ingredient_count integer, source text, source_quality text, ingredient_verification_status text, image_verification_status text, verified_at timestamp with time zone, image_url text, ingredients text[], ingredient_text text, nutritional_info jsonb, nutrient_panel jsonb, has_published_nutrients boolean, source_url text, rank real)
 LANGUAGE plpgsql
  ...
  final_rows AS MATERIALIZED (
    SELECT
      ...
      row_number() OVER (
        ...
        ORDER BY
          CASE
            WHEN scored.source_quality = 'manufacturer'
              OR scored.ingredient_verification_status = 'manufacturer'
              THEN 0
            WHEN scored.source_quality IN ('gdsn', 'official')
              OR scored.ingredient_verification_status IN ('gdsn', 'official')
              THEN 1
            ELSE 2
          END,
