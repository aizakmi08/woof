-- Keep the default acquisition reconciliation batch below the Supabase MCP
-- timeout observed on dense brand slices. Larger batches remain available to
-- explicit callers with longer-lived SQL sessions.

DROP FUNCTION IF EXISTS public.reconcile_catalog_acquisition_queue_batch(INTEGER);
