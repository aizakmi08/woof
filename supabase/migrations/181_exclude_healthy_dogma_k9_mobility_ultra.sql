-- Exclude Healthy Dogma K9 Mobility Ultra supplement without excluding
-- the complete-food Mobility PetMix formula.
...
WITH rejected AS (
  ...
  SET
    ...
    updated_at = now()
  WHERE source = 'healthy-dogma'
    AND public.normalize_product_catalog_name(product_name) ~ '(^| )k9 mobility ultra( |$)'
  RETURNING cache_key
)
...
DO $$
BEGIN
  IF public.is_likely_non_product_catalog_row('Mobility PetMix', 'Healthy Dogma') THEN
    RAISE EXCEPTION 'Healthy Dogma Mobility PetMix should remain allowed';
  END IF;

  IF NOT public.is_likely_non_product_catalog_row('K9 Mobility Ultra', 'Healthy Dogma') THEN
    RAISE EXCEPTION 'Healthy Dogma K9 Mobility Ultra should be rejected';
  END IF;
