-- Persistent acquisition backlog for closing catalog coverage gaps.
-- The app logs lookup and verification demand; service-role jobs refresh this
-- queue and use it to acquire official/manufacturer/GDSN records.

CREATE TABLE IF NOT EXISTS public.catalog_acquisition_queue (
  ...
  source_quality TEXT,
  source_url TEXT,
  needs_product_record BOOLEAN NOT NULL DEFAULT FALSE,
  needs_verified_ingredients BOOLEAN NOT NULL DEFAULT FALSE,
  needs_verified_image BOOLEAN NOT NULL DEFAULT FALSE,
  needs_pet_type BOOLEAN NOT NULL DEFAULT FALSE,
  ready_rows INTEGER NOT NULL DEFAULT 0,
  affected_product_count INTEGER NOT NULL DEFAULT 0,
  demand_events INTEGER NOT NULL DEFAULT 0,
...
ALTER TABLE public.catalog_acquisition_queue
  ...
  ADD COLUMN IF NOT EXISTS needs_product_record BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS needs_verified_ingredients BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS needs_verified_image BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS needs_pet_type BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS ready_rows INTEGER NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS affected_product_count INTEGER NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS demand_events INTEGER NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS sample_metadata JSONB NOT NULL DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS acquisition_notes TEXT;

ALTER TABLE public.catalog_acquisition_queue
  ALTER COLUMN gap_key SET NOT NULL,
  ALTER COLUMN gap_type SET NOT NULL;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'catalog_acquisition_queue_gap_key_key'
      AND conrelid = 'public.catalog_acquisition_queue'::regclass
  ) THEN
    ALTER TABLE public.catalog_acquisition_queue
      ADD CONSTRAINT catalog_acquisition_queue_gap_key_key UNIQUE (gap_key);
  END IF;
END;
$$;

ALTER TABLE public.catalog_acquisition_queue
  DROP CONSTRAINT IF EXISTS catalog_acquisition_queue_gap_type_check,
  DROP CONSTRAINT IF EXISTS catalog_acquisition_queue_status_check,
  DROP CONSTRAINT IF EXISTS catalog_acquisition_queue_nonnegative_check;

ALTER TABLE public.catalog_acquisition_queue
  ADD CONSTRAINT catalog_acquisition_queue_gap_type_check
    CHECK (gap_type IN ('brand', 'product', 'lookup')),
  ADD CONSTRAINT catalog_acquisition_queue_status_check
    CHECK (status IN ('open', 'in_progress', 'imported', 'resolved', 'deferred', 'blocked')),
  ADD CONSTRAINT catalog_acquisition_queue_nonnegative_check
    CHECK (
      priority_score >= 0
      AND ready_rows >= 0
      AND affected_product_count >= 0
      AND demand_events >= 0
    );

CREATE INDEX IF NOT EXISTS idx_catalog_acquisition_queue_status_priority
  ON public.catalog_acquisition_queue (status, priority_score DESC, updated_at DESC);

CREATE INDEX IF NOT EXISTS idx_catalog_acquisition_queue_type_priority
  ON public.catalog_acquisition_queue (gap_type, priority_score DESC, updated_at DESC);

CREATE INDEX IF NOT EXISTS idx_catalog_acquisition_queue_brand
  ON public.catalog_acquisition_queue (lower(brand))
  WHERE brand IS NOT NULL;

ALTER TABLE public.catalog_acquisition_queue ENABLE ROW LEVEL SECURITY;

REVOKE ALL ON TABLE public.catalog_acquisition_queue FROM PUBLIC;
REVOKE ALL ON TABLE public.catalog_acquisition_queue FROM anon;
REVOKE ALL ON TABLE public.catalog_acquisition_queue FROM authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.catalog_acquisition_queue TO service_role;
