-- Store pet type explicitly so official/manufacturer catalog feeds do not
-- depend on product names containing "dog" or "cat".

ALTER TABLE public.product_data
  ADD COLUMN IF NOT EXISTS pet_type TEXT;

ALTER TABLE public.product_data
  DROP CONSTRAINT IF EXISTS product_data_pet_type_check;

ALTER TABLE public.product_data
  ADD CONSTRAINT product_data_pet_type_check
  CHECK (pet_type IS NULL OR pet_type IN ('dog', 'cat', 'unknown'));

CREATE INDEX IF NOT EXISTS idx_product_data_pet_type_ready
  ON public.product_data (pet_type, ingredient_count DESC, expires_at DESC)
