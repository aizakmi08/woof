-- Recover exact retailer package versions when their complete ingredient text
-- is byte-for-byte identical to one already verified serving formula. The
-- client artifact also requires same brand/species and protected identity
-- agreement; this RPC independently rechecks the two ingredient hashes and
-- the referenced serving image before updating staging evidence.

CREATE OR REPLACE FUNCTION public.apply_exact_ingredient_formula_images(
...
BEGIN
  ...
  IF jsonb_typeof(COALESCE(p_payload, '[]'::JSONB)) <> 'array' THEN
    RAISE EXCEPTION 'Exact ingredient image payload must be a JSON array';
  END IF;
  ...
  WITH parsed AS (
    ...
    FROM jsonb_to_recordset(COALESCE(p_payload, '[]'::JSONB)) AS row(
      ...
      retailer_ingredient_raw_hash TEXT,
      product_name TEXT,
      pet_type TEXT,
      retailer_brand TEXT,
      retailer_gtin TEXT,
      ...
      matched_source_ingredient_raw_hash TEXT,
    ...
    JOIN public.product_data source_formula
     ...
     AND encode(digest(source_formula.ingredient_text, 'sha256'), 'hex')
       = parsed.matched_source_ingredient_raw_hash
    ...
    WHERE parsed.source_slug = 'walmart'
      ...
      AND parsed.retailer_ingredient_raw_hash ~ '^[0-9a-f]{64}$'
      AND parsed.matched_source_ingredient_raw_hash
        = parsed.retailer_ingredient_raw_hash
      ...
      AND parsed.evidence_method = 'exact_cross_source_ingredient_identity_v1'
      ...
      AND parsed.identity_resolution->>'deterministic_evidence'
        = 'exact_full_ingredient_text_brand_species_and_protected_identity'
  ), changed AS (
    ...
    SET
      ...
      raw_payload = evidence.raw_payload || jsonb_build_object(
        ...
        'matched_source_ingredient_raw_hash',
          valid.matched_source_ingredient_raw_hash,
        ...
        'image_identity_scope',
          'exact ingredient formula; package size remains an SKU child',
    ...
    WHERE evidence.import_run_id = p_import_run_id
      ...
      AND encode(digest(evidence.ingredient_text, 'sha256'), 'hex')
        = valid.retailer_ingredient_raw_hash
...
BEGIN
  ...
  IF v_patched = v_definition
    OR position('exact_cross_source_ingredient_identity_v1' IN v_patched) = 0
  THEN
    RAISE EXCEPTION 'cross-source ingredient materialization marker not found';
  END IF;
  EXECUTE v_patched;
END;
$migration$;

REVOKE ALL ON FUNCTION public.apply_exact_ingredient_formula_images(UUID, JSONB)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.apply_exact_ingredient_formula_images(UUID, JSONB)
  TO service_role;
