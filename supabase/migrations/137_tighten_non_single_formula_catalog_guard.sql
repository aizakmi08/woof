-- Keep non-single-formula rows out of the ready catalog. A variety pack or an
-- ambiguous count pack cannot be scored against one exact ingredient panel.
...
flags AS (
  SELECT
    ...
    product_name ~ '(^| )(variety|varieties|variety packs?|bundles?|samplers?|sample packs?|starter kits?)( |$)' AS non_single_formula,
    (
