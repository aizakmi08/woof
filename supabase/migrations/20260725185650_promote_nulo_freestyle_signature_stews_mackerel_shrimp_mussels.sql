...
DECLARE
  ...
  v_run BIGINT;
  v_old_key TEXT;
  v_key TEXT := 'nulo|nulo|freestyle signature stews|cat|all life stages|wet|mackerel shrimp and mussels recipe|';
  v_cache TEXT := 'nulo:nulo freestyle cat kitten mackerel shrimp mussel stew';
  v_duplicate_cache TEXT := 'census:84f1d34bb63485d62489b010c241b396';
  v_source TEXT := 'https://nulo.com/products/freestyle-signature-stews-mackerel-shrimp-mussels-recipe-for-cats';
  v_front TEXT := 'https://cdn.shopify.com/s/files/1/0084/9664/4192/files/zas0pq1cxrsnon7ewil1.png?v=1776774896';
  v_back TEXT := 'https://cdn.shopify.com/s/files/1/0084/9664/4192/files/uyncndangrz1ys8we6he.png?v=1776774896';
  v_pdf TEXT := 'https://images.salsify.com/image/upload/s--Ufd1cWrq--/icixrdkf4b4swrlzf4nz.pdf';
  v_top TEXT;
BEGIN
  SELECT id,formula_key INTO STRICT v_formula,v_old_key
  FROM public.catalog_formulas
  WHERE formula_key='nulo|nulo|freestyle and mackerel|cat|kitten|wet||';
  SELECT id INTO STRICT v_duplicate
  FROM public.catalog_formulas
  WHERE formula_key='nulo|nulo|freestyle signature stews mackerel|cat|kitten|wet|shrimp and mussels recipe|';
  SELECT id INTO STRICT v_discovery
  FROM public.catalog_formulas
  WHERE formula_key='nulo|nulo|nulo freestyle mackerel shrimp and mussels stew wet cat food|cat|unknown|wet||';

  IF EXISTS (SELECT 1 FROM public.catalog_formulas WHERE formula_key=v_key AND id<>v_formula) THEN
    RAISE EXCEPTION 'Nulo Mackerel Shrimp Mussels exact key belongs to sibling';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Nulo Mackerel Shrimp Mussels manufacturer serving row missing';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Nulo Mackerel Shrimp Mussels duplicate rows do not carry identical exact ingredients';
  END IF;

  UPDATE public.product_data SET
    product_name='Nulo FreeStyle Signature Stews Mackerel, Shrimp & Mussels Recipe Wet Cat Food',
    brand='Nulo', product_line='FreeStyle Signature Stews',
    pet_type='cat', life_stage='all life stages', food_form='wet',
    flavor='Mackerel, Shrimp & Mussels Recipe', package_size='2.8 oz cup',
  ...
  UPDATE public.catalog_formulas SET
    ...
    flavor='mackerel shrimp and mussels recipe', diet_condition='',
    ...
    ingredient_verification_status='manufacturer',
    image_verification_status='manufacturer',
    protected_terms=ARRAY['nulo','freestyle','signature stews','mackerel','shrimp','mussels','cat','kitten','wet','all life stages']::TEXT[],
  ...
  WHERE formula_id IN (v_duplicate,v_discovery)
     OR (source_slug='chewy-public-sitemap' AND source_external_id='355558');
  ...
  SELECT alias_key,v_formula,formula.identity_hash,'manual_review',v_source,
    jsonb_build_object('exact_formula_identity',true,'official_sku','63WM02','current_formula_key',v_key,'reconciled_at',now()),now()
  FROM unnest(ARRAY[
    v_old_key,
    'nulo|nulo|freestyle signature stews mackerel|cat|kitten|wet|shrimp and mussels recipe|',
    'nulo|nulo|nulo freestyle mackerel shrimp and mussels stew wet cat food|cat|unknown|wet||'
  ]::TEXT[]) alias_key
  ...
  UPDATE public.catalog_formulas SET
    ...
    complete_food_evidence='Superseded duplicate current Nulo manufacturer identity; exact formula retained under canonical FreeStyle Signature Stews Mackerel, Shrimp & Mussels identity.',
    updated_at=now()
  ...
  UPDATE public.catalog_formulas SET
    verification_status='quarantined',active=false,
    absent_since=COALESCE(absent_since,now()),promoted_cache_key=NULL,promoted_at=NULL,
    complete_food_evidence='Superseded exact Chewy discovery identity linked to the current official Nulo FreeStyle Signature Stews Mackerel, Shrimp & Mussels formula.',
    updated_at=now()
  ...
  INSERT INTO public.catalog_skus(
    ...
    v_formula,NULL,'2.8 oz cup',1,'nulo-manufacturer-manual','63WM02:2.8oz',
  ...
  INSERT INTO public.catalog_verified_product_search_aliases(
    ...
    v_cache,
    'Nulo FreeStyle Mackerel Shrimp Mussels Stew Wet Cat Food',
    public.normalize_verified_product_search_query('Nulo FreeStyle Mackerel Shrimp Mussels Stew Wet Cat Food'),
    v_source,'manufacturer',now(),
    jsonb_build_object(
      'source','current_official_product_page','official_sku','63WM02',
      'food_form_boundary','wet','species_boundary','cat',
      'life_stage_boundary','all life stages',
      'recipe_boundary','mackerel shrimp mussels'
    )
  )
  ...
  INSERT INTO public.catalog_source_runs(
    ...
    'manual-exact-evidence:nulo:freestyle-signature-stews-mackerel-shrimp-mussels:20260725',
    'nulo-manufacturer-manual','manufacturer','verification','completed',
    now(),now(),1,1,1,0,true,
    encode(digest(v_source||'|63WM02|'||v_front||'|'||v_pdf,'sha256'),'hex'),
    '{}'::JSONB,NULL,
    jsonb_build_object(
      'manual_exact_evidence',true,'search_results_are_discovery_only',true,
      'official_sku','63WM02','official_package_size','2.8 oz cup',
  ...
  SELECT v_run,v_formula,'nulo-manufacturer-manual','63WM02',v_source,'manufacturer',
    ...
    'accepted',ARRAY[]::TEXT[],
    jsonb_build_object(
      'official_sku','63WM02','official_front_image',v_front,
      'official_back_image',v_back,'official_ingredient_pdf',v_pdf,
  ...
  FROM (
    VALUES
      ...
      ('ingredient_pdf_url',v_pdf),('official_sku','63WM02'),
  ...
  SELECT
    'manual-search:nulo:freestyle-signature-stews-mackerel-shrimp-mussels:20260725',
    formula_key,formula_key,'Nulo',product_name,
    'Nulo FreeStyle Mackerel Shrimp Mussels Stew Wet Cat Food ingredients',
    ...
    jsonb_build_object(
      ...
      'life_stage','all life stages','food_form','wet',
      'flavor','Mackerel, Shrimp & Mussels Recipe'
    ),
    jsonb_build_object(
      ...
      'flavor',flavor,'official_sku','63WM02','package_size','2.8 oz cup'
    ),
    'promoted',NULL,
  ...
  UPDATE public.catalog_acquisition_queue SET
    ...
    resolution_reason='Exact current Nulo FreeStyle Signature Stews Mackerel, Shrimp & Mussels formula promoted from official manufacturer evidence.',
    acquisition_notes='Official SKU 63WM02 verifies 35 exact ingredients, 2.8 oz cup, all-life-stages cat boundary, wet stew form, matching front/back images, and AAFCO adequacy. No GTIN was inferred.',
  ...
  WHERE gap_key='census:c0a66a98bd7c741b00a26006fc5587c0';

  SELECT cache_key INTO v_top FROM public.search_verified_products(
    'Nulo FreeStyle Mackerel Shrimp Mussels Stew Wet Cat Food',8
  ) ORDER BY rank DESC LIMIT 1;
  IF v_top IS DISTINCT FROM v_cache THEN
    RAISE EXCEPTION 'Nulo Mackerel Shrimp Mussels exact search regression: %',v_top;
  END IF;

  IF EXISTS(
    SELECT 1 FROM public.catalog_formulas
    WHERE id IN (v_duplicate,v_discovery) AND (active OR verification_status<>'quarantined')
  ) THEN RAISE EXCEPTION 'Nulo Mackerel Shrimp Mussels duplicate formulas remain active'; END IF;

  IF (SELECT count(*) FROM public.catalog_formulas
      WHERE source_url=v_source AND active AND verification_status='verified') <> 1
  THEN RAISE EXCEPTION 'Nulo Mackerel Shrimp Mussels canonical active formula is not unique'; END IF;

  IF NOT EXISTS(
    SELECT 1 FROM public.catalog_skus
    WHERE formula_id=v_formula AND active AND source_external_id='63WM02:2.8oz'
      AND package_size='2.8 oz cup' AND gtin IS NULL
  ) THEN RAISE EXCEPTION 'Nulo Mackerel Shrimp Mussels exact package child missing'; END IF;

  IF NOT EXISTS(
    SELECT 1 FROM public.product_data
    WHERE cache_key=v_duplicate_cache
      AND catalog_exclusion_reason='duplicate_alias_of_verified_formula'
  ) THEN RAISE EXCEPTION 'Nulo Mackerel Shrimp Mussels duplicate serving row not excluded'; END IF;

  IF NOT EXISTS(
    SELECT 1 FROM public.catalog_acquisition_queue
    WHERE gap_key='census:c0a66a98bd7c741b00a26006fc5587c0' AND status='resolved'
  ) THEN RAISE EXCEPTION 'Nulo Mackerel Shrimp Mussels queue gap did not close'; END IF;
END
$$;
