-- Fast duplicate cleanup for stale legacy catalog rows. This avoids calling the
-- app search RPC for every queued row by joining directly against same-brand
-- verified-ready catalog rows, then applying the existing strict identity,
-- life-stage, line-term, package-count, species, and ambiguity guards.
...
BEGIN
  ...
  raw_candidates AS (
    SELECT
      ...
      public.catalog_acquisition_identity_normalize(concat_ws(' ', qs.brand, qs.product_name)) AS legacy_identity_norm,
      vc.catalog_identity_norm,
      (
        CASE
          WHEN public.catalog_acquisition_identity_normalize(concat_ws(' ', qs.brand, qs.product_name)) = vc.catalog_title_norm THEN 120
          WHEN vc.catalog_identity_norm LIKE '%' || public.catalog_acquisition_identity_normalize(concat_ws(' ', qs.brand, qs.product_name)) || '%' THEN 105
          WHEN public.catalog_acquisition_identity_normalize(concat_ws(' ', qs.brand, qs.product_name)) LIKE '%' || vc.catalog_title_norm || '%' THEN 100
          ELSE 80
        END
        + LEAST(COALESCE(vc.ingredient_count, 0), 40) * 0.1
      )::REAL AS direct_identity_score
    FROM queue_scope qs
    JOIN verified_catalog vc
      ON vc.cache_key <> qs.cache_key
    WHERE public.catalog_acquisition_life_stage_terms_match(qs.product_name, vc.catalog_identity)
      AND public.catalog_acquisition_protected_line_terms_match(qs.product_name, vc.catalog_identity)
      AND public.catalog_acquisition_package_count_match(qs.product_name, vc.catalog_identity)
      AND (
...
BEGIN
  ...
  IF function_sql LIKE '%search_verified_products%' THEN
    RAISE EXCEPTION 'direct verified identity duplicate closer must not call search_verified_products';
  END IF;

  IF function_sql NOT LIKE '%catalog_quality_state%' THEN
    RAISE EXCEPTION 'direct verified identity duplicate closer must require verified-ready catalog rows';
  END IF;

  IF function_sql NOT LIKE '%catalog_acquisition_life_stage_terms_match%' THEN
    RAISE EXCEPTION 'direct verified identity duplicate closer must keep life-stage guard';
  END IF;

  IF function_sql NOT LIKE '%catalog_acquisition_protected_line_terms_match%' THEN
    RAISE EXCEPTION 'direct verified identity duplicate closer must keep protected line guard';
  END IF;

  IF function_sql NOT LIKE '%catalog_acquisition_package_count_match%' THEN
    RAISE EXCEPTION 'direct verified identity duplicate closer must keep package-count guard';
  END IF;

  IF function_sql NOT LIKE '%ambiguous_direct_verified_identity_match%' THEN
    RAISE EXCEPTION 'direct verified identity duplicate closer must keep ambiguity marker';
  END IF;
  ...
  IF NOT public.catalog_acquisition_package_count_match(
    'Fancy Feast Grilled Chicken Feast in Gravy 24 cans',
    'Fancy Feast Grilled Chicken Feast in Gravy Gourmet Cat Food 24 ct Pack'
  ) THEN
    RAISE EXCEPTION 'direct verified identity package-count guard should accept matching counts';
  END IF;

  IF public.catalog_acquisition_package_count_match(
    'Fancy Feast Grilled Chicken Feast in Gravy 24 cans',
    'Fancy Feast Grilled Chicken Feast in Gravy Gourmet Cat Food 12 ct Pack'
  ) THEN
    RAISE EXCEPTION 'direct verified identity package-count guard must reject mismatched counts';
  END IF;
END $$;
