...
DECLARE
  ...
  v_run BIGINT;
  v_old_key TEXT;
  v_key TEXT := 'nulo|nulo|freestyle small breed pate|dog|all life stages|wet|duck and chickpeas recipe|';
  v_cache TEXT := 'nulo:nulo nulo small breed duck chickpeas recipe';
  v_source TEXT := 'https://nulo.com/products/pate-small-breed-duck-chickpeas-recipe-for-dogs';
  v_front TEXT := 'https://cdn.shopify.com/s/files/1/0084/9664/4192/files/txpoobxuvynepdonlxas.png?v=1776774832';
  v_back TEXT := 'https://cdn.shopify.com/s/files/1/0084/9664/4192/files/nncfa4bij90ojlhfehyf.png?v=1776774832';
  v_pdf TEXT := 'https://images.salsify.com/image/upload/s--55hOsyTn--/hwumhepaqjaihwbz1r1q.pdf';
  v_top TEXT;
BEGIN
  ...
  WHERE formula_key='nulo|nulo|nulo small breed adult grain free duck recipe wet pate canned dog food|dog|adult|wet||';

  IF EXISTS (SELECT 1 FROM public.catalog_formulas WHERE formula_key=v_key AND id<>v_formula) THEN
    RAISE EXCEPTION 'Nulo Duck Chickpeas exact key belongs to sibling';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Nulo Duck Chickpeas manufacturer serving row missing';
  END IF;

  UPDATE public.product_data SET
    product_name='Nulo FreeStyle Small Breed Duck & Chickpeas Recipe Paté for Dogs',
    brand='Nulo', product_line='FreeStyle Small Breed Paté',
    pet_type='dog', life_stage='all life stages', food_form='wet',
    flavor='Duck & Chickpeas Recipe', package_size='5.5 oz can',
  ...
  UPDATE public.catalog_formulas SET
    formula_key=v_key,
    product_name='Nulo FreeStyle Small Breed Duck & Chickpeas Recipe Paté for Dogs',
    product_line='freestyle small breed pate', pet_type='dog',
    life_stage='all life stages', food_form='wet',
    flavor='duck and chickpeas recipe', diet_condition='',
    ...
    ingredient_verification_status='manufacturer',
    image_verification_status='manufacturer',
    protected_terms=ARRAY['nulo','freestyle','small breed','pate','duck','chickpeas','dog','wet','all life stages']::TEXT[],
  ...
  WHERE formula_id=v_discovery OR (
    source_slug='chewy-public-sitemap' AND source_external_id='259050'
  );
  ...
  SELECT alias_key,v_formula,formula.identity_hash,'manual_review',v_source,
    jsonb_build_object(
      'exact_formula_identity',true,'official_sku','52SD05',
      'current_formula_key',v_key,'reconciled_at',now()
    ),now()
  FROM unnest(ARRAY[
    v_old_key,
    'nulo|nulo|nulo small breed adult grain free duck recipe wet pate canned dog food|dog|adult|wet||'
  ]::TEXT[]) alias_key
  ...
  UPDATE public.catalog_formulas SET
    ...
    promoted_at=NULL,
    complete_food_evidence='Superseded exact Chewy discovery identity linked to the current official Nulo FreeStyle Small Breed Duck & Chickpeas Paté formula.',
    updated_at=now()
  ...
  INSERT INTO public.catalog_skus(
    ...
    v_formula,NULL,'5.5 oz can',1,'nulo-manufacturer-manual',
    '52SD05:5.5oz',v_source,true,now(),now(),now()
  )
  ...
  INSERT INTO public.catalog_verified_product_search_aliases(
    ...
    v_cache,
    'Nulo FreeStyle Small Breed Duck Chickpeas Pate Wet Dog Food',
    public.normalize_verified_product_search_query('Nulo FreeStyle Small Breed Duck Chickpeas Pate Wet Dog Food'),
    v_source,'manufacturer',now(),
    jsonb_build_object('source','current_official_product_page','official_sku','52SD05','food_form_boundary','wet','species_boundary','dog')
  )
  ...
  INSERT INTO public.catalog_source_runs(
    ...
    'manual-exact-evidence:nulo:freestyle-small-breed-duck-chickpeas-pate:20260725',
    'nulo-manufacturer-manual','manufacturer','verification','completed',
    now(),now(),1,1,1,0,true,
    encode(digest(v_source||'|52SD05|'||v_front||'|'||v_pdf,'sha256'),'hex'),
    '{}'::JSONB,NULL,
    jsonb_build_object(
      'manual_exact_evidence',true,'search_results_are_discovery_only',true,
      'official_sku','52SD05','official_package_size','5.5 oz',
  ...
  SELECT v_run,v_formula,'nulo-manufacturer-manual','52SD05',v_source,'manufacturer',
    NULL,manufacturer,brand,product_name,product_line,pet_type,life_stage,
    food_form,flavor,diet_condition,'5.5 oz can',ingredient_text,v_front,true,true,
    now(),encode(digest(formula_key||'|'||ingredient_text||'|'||v_front,'sha256'),'hex'),
    'accepted',ARRAY[]::TEXT[],
    jsonb_build_object(
      'official_sku','52SD05','official_front_image',v_front,
      'official_back_image',v_back,'official_ingredient_pdf',v_pdf,
      'official_package_size','5.5 oz',
  ...
  FROM (
    VALUES
      ...
      ('ingredient_pdf_url',v_pdf),('official_sku','52SD05'),
  ...
  SELECT
    'manual-search:nulo:freestyle-small-breed-duck-chickpeas-pate:20260725',
    formula_key,formula_key,'Nulo',product_name,
    'Nulo FreeStyle Small Breed Duck Chickpeas Pate ingredients',
    jsonb_build_array(v_source,v_front,v_back,v_pdf),v_source,
    'manufacturer_page',
    jsonb_build_object('brand','Nulo','line','FreeStyle Small Breed Pate','pet_type','dog','life_stage','all life stages','food_form','wet','flavor','Duck & Chickpeas Recipe'),
    jsonb_build_object('manufacturer',manufacturer,'brand',brand,'product_line',product_line,'pet_type',pet_type,'life_stage',life_stage,'food_form',food_form,'flavor',flavor,'official_sku','52SD05','package_size','5.5 oz can'),
    'promoted',NULL,
    ...
    'Official current Nulo page and package assets verify exact FreeStyle Small Breed Duck & Chickpeas Paté identity, 37 ingredients, 5.5 oz can, all-life-stages adequacy boundary, and matching front/back images. No GTIN was published, so none was inferred.',
  ...
  UPDATE public.catalog_acquisition_queue SET
    ...
    resolution_reason='Exact current Nulo FreeStyle Small Breed Duck & Chickpeas Paté formula promoted from official manufacturer evidence.',
    acquisition_notes='Official SKU 52SD05 verifies 37 exact ingredients, 5.5 oz can, adult/puppy all-life-stages boundary, wet paté form, matching front/back images, and AAFCO adequacy. No GTIN was inferred.',
  ...
  WHERE gap_key='census:aa62e40f4f817dc95d4b572e4e96518a';

  SELECT cache_key INTO v_top FROM public.search_verified_products(
    'Nulo FreeStyle Small Breed Duck Chickpeas Pate Wet Dog Food',8
  ) ORDER BY rank DESC LIMIT 1;
  IF v_top IS DISTINCT FROM v_cache THEN
    RAISE EXCEPTION 'Nulo Duck Chickpeas exact search regression: %',v_top;
  END IF;
  ...
  THEN RAISE EXCEPTION 'Nulo Duck Chickpeas discovery duplicate remains active'; END IF;

  IF NOT EXISTS(
    SELECT 1 FROM public.catalog_acquisition_queue
    WHERE gap_key='census:aa62e40f4f817dc95d4b572e4e96518a' AND status='resolved'
  ) THEN RAISE EXCEPTION 'Nulo Duck Chickpeas queue gap did not close'; END IF;
END
$$;
