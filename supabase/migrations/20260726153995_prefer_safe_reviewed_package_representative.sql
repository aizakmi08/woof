-- Improve reviewed retailer promotion diagnostics while retaining alias deduplication behavior.
...
-- Repair the already-promoted v158 Healthy Weight representative using the
-- same deterministic safety rule. Keep the conflicting package in provenance.
DO $$
DECLARE
  v_formula_id BIGINT;
  v_cache_key TEXT;
  v_safe_source_url TEXT;
  v_safe_image_url TEXT;
BEGIN
  SELECT
    safe.formula_id,
    formula.promoted_cache_key,
    safe.source_url,
    safe.front_image_url
  INTO STRICT
    v_formula_id,
    v_cache_key,
    v_safe_source_url,
    v_safe_image_url
  FROM public.catalog_observations safe
  JOIN public.catalog_source_runs source_run
    ON source_run.id = safe.run_id
   AND source_run.run_key = 'target-blue-buffalo-reviewed-v158:20260727'
  JOIN public.catalog_formulas formula
    ON formula.id = safe.formula_id
  JOIN public.catalog_observations conflicting
    ON conflicting.formula_id = safe.formula_id
   AND conflicting.run_id = safe.run_id
   AND conflicting.raw_payload->>'barcode_resolution_policy' =
      'abstain_reused_gtin_formula_conflict'
  WHERE safe.source_external_id = '53261928'
    AND safe.raw_payload->>'barcode_resolution_policy' =
      'exact_source_version'
    AND conflicting.source_external_id = '76400742'
    AND public.catalog_normalize_ingredient_evidence(
      safe.ingredient_text
    ) = public.catalog_normalize_ingredient_evidence(
      conflicting.ingredient_text
    )
    AND formula.formula_evidence_tier = 'retailer_web_version'
    AND formula.promoted_cache_key IS NOT NULL;
  ...
  SET
    source_url = v_safe_source_url,
    front_image_url = v_safe_image_url,
    updated_at = now()
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION
      'Healthy Weight safe package representative repair failed';
  END IF;
END
$$;
