WITH inventory AS (
  ...
  WHERE run.run_key =
      'nestle-purina-fancy-feast:official-formula-inventory:6492b54255afb86457ff5d4a'
...
aliases AS (
  ...
  SELECT
    inventory.*,
    concat('Fancy Feast ', product_name),
...
BEGIN
      ...
      WHERE public.normalize_verified_product_search_query(
        ...
        AND EXISTS (
          ...
          WHERE alias.active
            ...
            AND alias.normalized_alias =
                public.normalize_verified_product_search_query(
                  concat('Fancy Feast ', inventory.product_name)
                )
        )
    )
  INTO
  ...
  IF v_formula_count <> 96
     OR v_exact_alias_count <> v_formula_count
     OR v_brand_prefix_needed <> 1
     OR v_brand_prefix_count <> v_brand_prefix_needed THEN
    RAISE EXCEPTION
      'Fancy Feast aliases incomplete: % exact for % formulas, % brand aliases for % needed',
      v_exact_alias_count,
      v_formula_count,
      v_brand_prefix_count,
      v_brand_prefix_needed;
