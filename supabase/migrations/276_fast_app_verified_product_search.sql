-- Keep the app-facing verified product search on an indexed hot path.
-- The previous wrapper delegated every request to the historical ranked
-- function, which is accurate but too broad for keystroke/search UX.
-- This version collects a small indexed candidate set first, then applies
-- the same verified-ready gates, required-term guard, duplicate collapse,
-- and sibling-variant penalties before returning app results.
...
AS $$
  ...
  candidates AS MATERIALIZED (
    ...
    FROM (
      ...
      FROM ready_rows r
      CROSS JOIN query
      WHERE r.indexed_identity_lc LIKE '%' || query.normalized || '%'
        OR r.product_name_raw_lc LIKE query.normalized || '%'
        OR (
          r.brand_raw_lc <> ''
          AND query.normalized LIKE '%' || r.brand_raw_lc || '%'
        )
    ...
    FROM (
      ...
      FROM ready_rows r
      CROSS JOIN query
      WHERE query.normalized !~ '\m[a-z0-9]{16,}\M'
        AND (
