-- Wet complete-food product names often use package texture language such as
-- "in Fish Broth Gravy" or "in Sauce" without saying "food" or "recipe" in
-- the title. Keep rejecting toppers, bundles, and training sausages, but do
-- not reject manufacturer-backed named wet formulas.
...
DO $$
BEGIN
  IF public.is_likely_non_product_catalog_row('Minced Salmon & Cod in Fish Broth Gravy', 'The Honest Kitchen') THEN
    RAISE EXCEPTION 'valid named fish-broth gravy formula is still rejected';
  END IF;

  IF public.is_likely_non_product_catalog_row('Home Delights Sausage with Egg & Cheese in Sauce', 'Cesar') THEN
    RAISE EXCEPTION 'valid named wet sauce formula is still rejected';
  END IF;

  IF NOT public.is_likely_non_product_catalog_row('Bone Broth Topping', 'Example Brand') THEN
    RAISE EXCEPTION 'broth topper guard was weakened';
  END IF;

  IF NOT public.is_likely_non_product_catalog_row('Training Sausage Bites', 'Example Brand') THEN
    RAISE EXCEPTION 'training sausage guard was weakened';
  END IF;

  IF NOT public.is_likely_non_product_catalog_row('Seafood Variety Pack 24 ct.', 'Example Brand') THEN
    RAISE EXCEPTION 'variety-pack guard was weakened';
  END IF;
END;
$$;
