...
--
-- Equivalent packages link to the current canonical formula. Distinct versions
-- never overwrite current ingredients. Reused GTINs remain in provenance but
-- have no active deterministic barcode owner.
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'A current-equivalent retailer GTIN did not prefer manufacturer current';
  END IF;
