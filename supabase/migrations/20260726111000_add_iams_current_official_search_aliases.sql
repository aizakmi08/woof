WITH inventory AS (
  ...
  WHERE run.run_key =
      'iams:official-formula-inventory:8b660b75a9c36d4724811739'
    AND serving.source_url LIKE 'https://www.iams.com/%'
...
BEGIN
      ...
      WHERE public.normalize_verified_product_search_query(
        ...
        AND EXISTS (
          ...
          WHERE alias.active
            ...
            AND alias.normalized_alias =
                public.normalize_verified_product_search_query(
                  concat('IAMS ', inventory.product_name)
                )
        )
    )
  INTO
  ...
  IF v_formula_count <> 60
     OR v_exact_alias_count <> v_formula_count
     OR v_brand_prefix_needed <> 4
     OR v_brand_prefix_count <> v_brand_prefix_needed THEN
    RAISE EXCEPTION
      'IAMS aliases incomplete: % exact for % formulas, % brand aliases for % needed',
      v_exact_alias_count,
      v_formula_count,
      v_brand_prefix_count,
      v_brand_prefix_needed;
