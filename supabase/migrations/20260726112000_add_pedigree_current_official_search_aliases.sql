WITH inventory AS (
  ...
  WHERE run.run_key =
      'pedigree-mars-petcare:official-formula-inventory:0cb4820e398541c040d6cea2'
    AND serving.source_url LIKE 'https://www.pedigree.com/%'
...
BEGIN
      ...
      WHERE public.normalize_verified_product_search_query(
        ...
        AND EXISTS (
          ...
          WHERE alias.active
            ...
            AND alias.normalized_alias =
                public.normalize_verified_product_search_query(
                  concat('Pedigree ', inventory.product_name)
                )
        )
    )
  INTO
  ...
  IF v_formula_count <> 45
     OR v_exact_alias_count <> v_formula_count
     OR v_brand_prefix_needed <> 43
     OR v_brand_prefix_count <> v_brand_prefix_needed THEN
    RAISE EXCEPTION
      'Pedigree aliases incomplete: % exact for % formulas, % brand aliases for % needed',
      v_exact_alias_count,
      v_formula_count,
      v_brand_prefix_count,
      v_brand_prefix_needed;
