-- Prefer the current manufacturer formula for a front-label scan while still
-- returning source-version rows for the client-side exact-identity guard.
-- Formula evidence remains embedded in nutritional_info to preserve the
-- existing RPC return type used by released clients.
...
AS $function$
      ...
      SELECT
        ...
        (
          ...
          + CASE
              ...
              ELSE 0.0
            END
          + CASE COALESCE(product.formula_evidence_tier, '')
              WHEN 'manufacturer_current_exact' THEN 3.0
              WHEN 'retailer_web_version' THEN 1.0
              WHEN 'web_label_version' THEN 0.5
              ELSE 0.0
            END
          + CASE
              WHEN product.source_quality IN ('manufacturer', 'official') THEN 2.0
              ELSE 0.0
            END
  ...
  SELECT
    ...
    COALESCE(product.nutritional_info, '{}'::JSONB) || jsonb_build_object(
      'formula_evidence_tier', product.formula_evidence_tier,
      'formula_version_provenance', COALESCE(product.formula_version_provenance, '{}'::JSONB)
    ) AS nutritional_info,
  ...
  ORDER BY
    ...
    CASE COALESCE(product.formula_evidence_tier, '')
      WHEN 'manufacturer_current_exact' THEN 4
      WHEN 'retailer_web_version' THEN 3
