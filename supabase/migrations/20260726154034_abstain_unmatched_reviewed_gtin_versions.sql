-- A reviewed exact-package GTIN can already belong to an older or differently
-- normalized formula identity even when the ingredient text happens to match.
-- Preserve that package as a searchable source version, but abstain on barcode
-- ownership unless an existing verified serving formula is an exact compatible
-- match. This extends the existing reused-GTIN version-conflict policy.
