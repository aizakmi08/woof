...
DECLARE
  v_formula BIGINT;
  v_duplicate BIGINT;
  v_duplicate_two BIGINT;
  v_discovery BIGINT;
  v_run BIGINT;
  v_old_key TEXT;
  v_key TEXT := 'nulo|nulo|freestyle signature stews|cat|all life stages|wet|yellowfin tuna and crab recipe|';
  v_cache TEXT := 'nulo:nulo freestyle cat kitten yellowfin tuna crab stew';
  v_duplicate_cache TEXT := 'census:a17d575bcb0c69bb01e85534dced0d9f';
  v_source TEXT := 'https://nulo.com/products/freestyle-signature-stews-yellowfin-tuna-crab-recipe-for-cats';
  v_front TEXT := 'https://cdn.shopify.com/s/files/1/0084/9664/4192/files/qi29jehrnxuuw6chohtz.png?v=1776774878';
  v_back TEXT := 'https://cdn.shopify.com/s/files/1/0084/9664/4192/files/b59hwedyqyt7gi4bx3ww.png?v=1776774878';
  v_pdf TEXT := 'https://images.salsify.com/image/upload/s--3m0VRX9I--/n2acxqvxbbct9bvxyccw.pdf';
  v_top TEXT;
BEGIN
  SELECT id,formula_key INTO STRICT v_formula,v_old_key
  FROM public.catalog_formulas
  WHERE formula_key='nulo|nulo|freestyle and yellowfin|cat|kitten|wet|tuna and crab stew|';
  SELECT id INTO STRICT v_duplicate
  FROM public.catalog_formulas
  WHERE formula_key='nulo|nulo|freestyle signature stews|cat|all life stages|wet|yellowfin tuna and crab|';
  SELECT id INTO STRICT v_duplicate_two
  FROM public.catalog_formulas
  WHERE formula_key='nulo|nulo|freestyle signature stews yellowfin|cat|kitten|wet|tuna and crab recipe|';
  SELECT id INTO STRICT v_discovery
  FROM public.catalog_formulas
  WHERE formula_key='nulo|nulo|nulo freestyle yellowfintuna and crab stew wet cat food|cat|unknown|wet||';

  IF EXISTS (SELECT 1 FROM public.catalog_formulas WHERE formula_key=v_key AND id<>v_formula) THEN
    RAISE EXCEPTION 'Nulo Yellowfin Tuna Crab exact key belongs to sibling';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE cache_key=v_cache AND source_quality='manufacturer'
      ...
      AND cardinality(ingredients)=35
      AND catalog_exclusion_reason IS NULL
  ) THEN
    RAISE EXCEPTION 'Nulo Yellowfin Tuna Crab manufacturer serving row missing';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Nulo Yellowfin Tuna Crab duplicate rows do not carry identical exact ingredients';
  END IF;

  UPDATE public.product_data SET
    product_name='Nulo FreeStyle Signature Stews Yellowfin Tuna & Crab Recipe Wet Cat Food',
    brand='Nulo', product_line='FreeStyle Signature Stews',
    pet_type='cat', life_stage='all life stages', food_form='wet',
    flavor='Yellowfin Tuna & Crab Recipe', package_size='2.8 oz cup',
  ...
  UPDATE public.catalog_formulas SET
    ...
    flavor='yellowfin tuna and crab recipe', diet_condition='',
    ...
    ingredient_verification_status='manufacturer',
    image_verification_status='manufacturer',
    protected_terms=ARRAY['nulo','freestyle','signature stews','yellowfin tuna','tuna','crab','cat','kitten','wet','all life stages']::TEXT[],
  ...
  WHERE formula_id IN (v_duplicate,v_duplicate_two,v_discovery)
     OR (source_slug='chewy-public-sitemap' AND source_external_id='355560');
  ...
  SELECT alias_key,v_formula,formula.identity_hash,'manual_review',v_source,
    jsonb_build_object('exact_formula_identity',true,'official_sku','63WT02','current_formula_key',v_key,'reconciled_at',now()),now()
  FROM unnest(ARRAY[
    v_old_key,
    'nulo|nulo|freestyle signature stews|cat|all life stages|wet|yellowfin tuna and crab|',
    'nulo|nulo|freestyle signature stews yellowfin|cat|kitten|wet|tuna and crab recipe|',
    'nulo|nulo|nulo freestyle yellowfintuna and crab stew wet cat food|cat|unknown|wet||'
  ]::TEXT[]) alias_key
  ...
  UPDATE public.catalog_formulas SET
    ...
    complete_food_evidence='Superseded duplicate current Nulo manufacturer identity; exact formula retained under canonical FreeStyle Signature Stews Yellowfin Tuna & Crab identity.',
    updated_at=now()
  WHERE id IN (v_duplicate,v_duplicate_two);

  UPDATE public.catalog_formulas SET
    verification_status='quarantined',active=false,
    absent_since=COALESCE(absent_since,now()),promoted_cache_key=NULL,promoted_at=NULL,
    complete_food_evidence='Superseded exact Chewy discovery identity linked to the current official Nulo FreeStyle Signature Stews Yellowfin Tuna & Crab formula.',
    updated_at=now()
  ...
  INSERT INTO public.catalog_skus(
    ...
    v_formula,NULL,'2.8 oz cup',1,'nulo-manufacturer-manual','63WT02:2.8oz',
  ...
  INSERT INTO public.catalog_verified_product_search_aliases(
    ...
    v_cache,
    'Nulo FreeStyle Yellowfin Tuna Crab Stew Wet Cat Food',
    public.normalize_verified_product_search_query('Nulo FreeStyle Yellowfin Tuna Crab Stew Wet Cat Food'),
    v_source,'manufacturer',now(),
    jsonb_build_object(
      'source','current_official_product_page','official_sku','63WT02',
      'food_form_boundary','wet','species_boundary','cat',
      'life_stage_boundary','all life stages',
      'recipe_boundary','yellowfin tuna crab'
    )
  )
  ...
  INSERT INTO public.catalog_source_runs(
    ...
    'manual-exact-evidence:nulo:freestyle-signature-stews-yellowfin-tuna-crab:20260725',
    'nulo-manufacturer-manual','manufacturer','verification','completed',
    now(),now(),1,1,1,0,true,
    encode(digest(v_source||'|63WT02|'||v_front||'|'||v_pdf,'sha256'),'hex'),
    '{}'::JSONB,NULL,
    jsonb_build_object(
      'manual_exact_evidence',true,'search_results_are_discovery_only',true,
      'official_sku','63WT02','official_package_size','2.8 oz cup',
  ...
  SELECT v_run,v_formula,'nulo-manufacturer-manual','63WT02',v_source,'manufacturer',
    ...
    'accepted',ARRAY[]::TEXT[],
    jsonb_build_object(
      'official_sku','63WT02','official_front_image',v_front,
      'official_back_image',v_back,'official_ingredient_pdf',v_pdf,
  ...
  FROM (
    VALUES
      ...
      ('ingredient_pdf_url',v_pdf),('official_sku','63WT02'),
  ...
  SELECT
    'manual-search:nulo:freestyle-signature-stews-yellowfin-tuna-crab:20260725',
    formula_key,formula_key,'Nulo',product_name,
    'Nulo FreeStyle Yellowfin Tuna Crab Stew Wet Cat Food ingredients',
    ...
    jsonb_build_object(
      ...
      'life_stage','all life stages','food_form','wet',
      'flavor','Yellowfin Tuna & Crab Recipe'
    ),
    jsonb_build_object(
      ...
      'flavor',flavor,'official_sku','63WT02','package_size','2.8 oz cup'
    ),
    'promoted',NULL,
  ...
  UPDATE public.catalog_acquisition_queue SET
    ...
    resolution_reason='Exact current Nulo FreeStyle Signature Stews Yellowfin Tuna & Crab formula promoted from official manufacturer evidence.',
    acquisition_notes='Official SKU 63WT02 verifies 35 exact ingredients, 2.8 oz cup, all-life-stages cat boundary, wet stew form, matching front/back images, and AAFCO adequacy. No GTIN was inferred.',
  ...
  WHERE gap_key='census:25d170cccaefc338ad7a7e7ddf92cde1';

  SELECT cache_key INTO v_top FROM public.search_verified_products(
    'Nulo FreeStyle Yellowfin Tuna Crab Stew Wet Cat Food',8
  ) ORDER BY rank DESC LIMIT 1;
  IF v_top IS DISTINCT FROM v_cache THEN
    RAISE EXCEPTION 'Nulo Yellowfin Tuna Crab exact search regression: %',v_top;
  END IF;

  IF EXISTS(
    SELECT 1 FROM public.catalog_formulas
    WHERE id IN (v_duplicate,v_duplicate_two,v_discovery) AND (active OR verification_status<>'quarantined')
  ) THEN RAISE EXCEPTION 'Nulo Yellowfin Tuna Crab duplicate formulas remain active'; END IF;

  IF (SELECT count(*) FROM public.catalog_formulas
      WHERE source_url=v_source AND active AND verification_status='verified') <> 1
  THEN RAISE EXCEPTION 'Nulo Yellowfin Tuna Crab canonical active formula is not unique'; END IF;

  IF NOT EXISTS(
    SELECT 1 FROM public.catalog_skus
    WHERE formula_id=v_formula AND active AND source_external_id='63WT02:2.8oz'
      AND package_size='2.8 oz cup' AND gtin IS NULL
  ) THEN RAISE EXCEPTION 'Nulo Yellowfin Tuna Crab exact package child missing'; END IF;

  IF NOT EXISTS(
    SELECT 1 FROM public.product_data
    WHERE cache_key=v_duplicate_cache
      AND catalog_exclusion_reason='duplicate_alias_of_verified_formula'
  ) THEN RAISE EXCEPTION 'Nulo Yellowfin Tuna Crab duplicate serving row not excluded'; END IF;

  IF NOT EXISTS(
    SELECT 1 FROM public.catalog_acquisition_queue
    WHERE gap_key='census:25d170cccaefc338ad7a7e7ddf92cde1' AND status='resolved'
  ) THEN RAISE EXCEPTION 'Nulo Yellowfin Tuna Crab queue gap did not close'; END IF;
END
$$;
