WITH inventory AS (
  ...
  WHERE run.run_key =
    ...
    AND serving.source_url LIKE 'https://www.nutro.com/%'
...
BEGIN
  ...
  IF v_formula_count <> 90 OR v_exact_alias_count <> v_formula_count THEN
    RAISE EXCEPTION
      'Nutro exact aliases incomplete: % aliases for % formulas',
      v_exact_alias_count,
      v_formula_count;
  END IF;
END
