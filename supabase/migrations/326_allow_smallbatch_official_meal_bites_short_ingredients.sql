-- Smallbatch's current cat Meal Bites are complete whole-food formulas with
-- 13-14 ingredients. Accept them only when the exact official PDP supplies
-- the matching protein, full guaranteed analysis, and AAFCO all-life-stages
-- statement. This keeps the general short-ingredient demotion gate closed.
...
BEGIN
  ...
  has_complete_food_ingredient_evidence :=
    ...
    OR (
      clean_ingredient_count BETWEEN 12 AND 19
      AND lower(COALESCE(NEW.source, '')) = 'smallbatch-pets'
      AND lower(COALESCE(NEW.brand, '')) = 'smallbatch'
      AND NEW.source_quality = 'manufacturer'
      AND NEW.ingredient_verification_status = 'manufacturer'
      AND NEW.pet_type = 'cat'
      AND COALESCE(NEW.food_form, '') ~* '^freeze[- ]dried$'
      AND (
        (
          lower(COALESCE(NEW.flavor, '')) = 'beef'
          AND COALESCE(NEW.source_url, '') ~* '^https://(www\.)?smallbatchpets\.com/products/freeze-dried-beef-meal-bites-cat/?$'
          AND COALESCE(exact_ingredient_text, clean_ingredient_text) ~* '^\s*beef\M'
        )
        OR (
          lower(COALESCE(NEW.flavor, '')) = 'chicken'
          AND COALESCE(NEW.source_url, '') ~* '^https://(www\.)?smallbatchpets\.com/products/freeze-dried-chicken-meal-bites/?$'
          AND COALESCE(exact_ingredient_text, clean_ingredient_text) ~* '^\s*chicken\M'
        )
        OR (
          lower(COALESCE(NEW.flavor, '')) = 'turkey'
          AND COALESCE(NEW.source_url, '') ~* '^https://(www\.)?smallbatchpets\.com/products/freeze-dried-turkey-meal-bites/?$'
          AND COALESCE(exact_ingredient_text, clean_ingredient_text) ~* '^\s*turkey\M'
        )
      )
      AND nutrient_evidence ~* '\mcrude\s+protein\M.*[0-9]'
      AND nutrient_evidence ~* '\mcrude\s+fat\M.*[0-9]'
      AND nutrient_evidence ~* '\mcrude\s+fiber\M.*[0-9]'
      AND nutrient_evidence ~* '\mmoisture\M.*[0-9]'
      AND nutrient_evidence ~* 'formulated\s+to\s+meet.*AAFCO\s+cat\s+food.*all\s+life\s+stages'
    )
    OR (
      clean_ingredient_count BETWEEN 10 AND 19
...
SET
  ...
  updated_at = NOW()
WHERE cache_key IN (
    'smallbatch-pets:793888799448',
    'smallbatch-pets:793888799349',
    'smallbatch-pets:793888799547'
  )
  AND source = 'smallbatch-pets'
  AND source_quality = 'manufacturer'
  AND image_verification_status = 'manufacturer'
  AND pet_type = 'cat'
  AND food_form = 'freeze-dried'
  AND is_complete_food IS TRUE
  AND ingredient_count BETWEEN 12 AND 19
  AND nutritional_info::TEXT ~* 'formulated\s+to\s+meet.*AAFCO\s+cat\s+food.*all\s+life\s+stages';
...
BEGIN
  ...
  IF verified_count <> 3 THEN
    RAISE EXCEPTION 'Smallbatch Meal Bites verification repair expected 3 rows, found %', verified_count;
  END IF;
END;
$$;
