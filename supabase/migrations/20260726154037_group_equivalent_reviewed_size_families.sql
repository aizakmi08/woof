-- Group exact-equivalent reviewed package siblings before promotion. If one
-- package in the exact identity/ingredient family matches manufacturer current,
-- its sibling packages attach to that same canonical formula. Reused sibling
-- GTINs remain provenance-only and barcode lookup abstains.
