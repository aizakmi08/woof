-- Valid wet-food formulas are often named "Protein in Broth" without the
-- words "recipe" or "food" in the title. Keep excluding broth toppers and
-- generic broth pages, but do not reject complete manufacturer-backed formulas
-- that contain named animal proteins and "in broth".
...
signals AS (
  SELECT
    ...
    (
      product_name ~ '(^| )(chicken|beef|steak|turkey|salmon|lamb|duck|tuna|whitefish|venison|pork|rabbit|cod|trout|bison|crab|prawns?|shrimp|mackerel|sardines?|seabass|tilapia|quail|liver)( |$)'
      AND product_name ~ '(^| )in broths?( |$)'
...
DO $$
BEGIN
  IF public.is_likely_non_product_catalog_row('Ahi Tuna & Crab in Broth', 'TIKI PETS') THEN
    RAISE EXCEPTION 'valid named in-broth formula is still rejected';
  END IF;
  ...
  IF NOT public.is_likely_non_product_catalog_row('Seafood Selects - 24 ct.', 'TIKI PETS') THEN
    RAISE EXCEPTION 'count-pack guard was weakened';
  END IF;
END $$;
