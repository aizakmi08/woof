...
| — | CI StoreKit execution on pinned iOS 26.1 | FAIL | 25m50s job; 12.67s tests | Run `33813710244`: 3 passed, 1 skipped by explicit expiry opt-in, and the strengthened Ask to Buy path proved both pending purchases plus approval. Its only failure was expecting a declined transaction to disappear from test-session history; Apple documents that it remains as Ask to Buy Declined. The replacement asserts that the retained transaction is no longer pending and has failed, and awaits a new CI run. |
