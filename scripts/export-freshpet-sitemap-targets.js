...
const DEFAULT_OUTPUT = ".tmp/freshpet-sitemap-catalog-targets.json";
...
const DOG_TERMS = [
  "dog", "dogs", "puppy", "puppies", "canine", "small dog", "large dog",
];
...
const CORE_FOOD_TERMS = [
  "freshpet", "grain free", "complete meal", "complete nutrition",
  "roasted meals", "fresh from the kitchen", "vital", "select",
  "small dog", "large dog", "puppy", "recipe", "chicken", "beef",
  "salmon", "egg", "green beans", "carrots", "cranberries", "spinach",
  "sweet potatoes",
];

const TREAT_TERMS = [
  "treat", "treats", "snack", "snacks", "bacon treats", "dog joy", "dognation",
  "biscuit", "biscuits", "chew", "chews", "jerky",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:freshpet-sitemap-catalog -- --output=.tmp/freshpet-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit Freshpet sitemap URL (${DEFAULT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.freshpet\.com\/sitemap\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://www.freshpet.com/sitemap.xml.");
  }
}
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .join("")
    .replace(/\bFreshpet\b/g, "Freshpet")
    .replace(/\s+([,])/g, "$1")
    .trim();
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Freshpet sitemap fetch failed ${response.status}: ${url}`);
...
function inferFreshpetPetTypes(row) {
  ...
  return inferPetTypes({
    ...
    source: "freshpet_sitemap",
  }, { includeAmbiguous: false });
}

function isCandidateFoodRow(row) {
  const text = normalizeText(`${row.name} ${row.sourceUrl} ${row.retailerProductId}`);
  const petTypes = inferFreshpetPetTypes(row);
  if (!/^https:\/\/www\.freshpet\.com\/products\//i.test(row.sourceUrl)) return false;
...
function normalizeTarget(row) {
  ...
  return {
    ...
    cacheKey,
    source: "freshpet_sitemap_export",
...
async function main() {
  ...
  const manifest = {
    ...
    source: "freshpet_product_sitemap",
    sitemapUrl,
    includeExisting,
    includeTreats,
    skipExistingScan,
  ...
  console.log(`Freshpet products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Freshpet sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Freshpet sitemap catalog target export failed:", err.message);
  process.exit(1);
});
