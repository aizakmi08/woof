...
const source = fs.readFileSync(path.join(root, "scripts/export-purinaone-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Purina ONE sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://www.purina.com/sitemap.xml") &&
    source.includes("const BRAND = \"Purina ONE\"") &&
    ...
    source.includes("www\\.purina\\.com\\/(cats|dogs)\\/shop\\/purina-one-") &&
    !source.includes("product-lookup") &&
    !source.includes("cheerio") &&
    !source.includes("puppeteer"),
  "exporter must read the official Purina sitemap index, derive titles from Purina ONE shop slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"true instinct\"") &&
    source.includes("\"tender cuts\"") &&
    source.includes("\"healthy puppy\"") &&
    source.includes("\"healthy kitten\"") &&
    source.includes("\"live clear\"") &&
    source.includes("\"whole body support\"") &&
    ...
    source.includes("function inferPurinaOnePetTypes(row)") &&
    ...
    source.includes("if (!/^https:\\/\\/www\\.purina\\.com\\/(cats|dogs)\\/shop\\/purina-one-/i.test(row.sourceUrl)) return false") &&
    source.includes("if (!new RegExp(`-(wet|dry)-${row.petType}-food(?:-|$)`, \"i\").test(row.retailerProductId)) return false") &&
  ...
  "exporter must strictly filter Purina ONE sitemap rows to core dry/wet cat and dog foods while rejecting treats, toppers, supplements, accessories, and non-food rows"
);
...
assert(
  source.includes("source: \"purinaone_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"purinaone_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:purinaone-sitemap-catalog": "node scripts/export-purinaone-sitemap-targets.js"') &&
    packageJson.includes('"test:purinaone-sitemap-catalog-export": "node scripts/test-purinaone-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:purinaone-sitemap-catalog-export"),
  "package scripts must expose Purina ONE sitemap exporter and include its guard in test:guards"
);

console.log("Purina ONE sitemap catalog export guard passed");
