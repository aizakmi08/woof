...
const DEFAULT_SITEMAP_URL = "https://www.nutrish.com/product-sitemap.xml";
const DEFAULT_OUTPUT = ".tmp/nutrish-sitemap-catalog-targets.json";
const REST_PAGE_SIZE = 1000;
const USER_AGENT = "Mozilla/5.0 Woof catalog coverage export";
const BRAND = "Rachael Ray Nutrish";
...
const CORE_FOOD_TERMS = [
  ...
  "puppy", "large breed", "small breed", "senior", "healthy aging",
  "healthy weight", "weight management", "indoor", "limited ingredient",
  "real chicken", "real beef", "real salmon", "grain free", "premium pate",
  "chunks in gravy", "variety pack", "recipe", "stew", "paw pie",
];
const TREAT_TERMS = [
  "treat", "treats", "chew", "chews", "soup bone", "soup bones",
  "burger bites", "savory roasters", "triple delights", "mini",
  "long lasting", "peanut butter",
];
const SUPPLEMENTAL_FOOD_TERMS = [
  "broth", "broths", "lickable", "complement", "complements", "topper",
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:nutrish-sitemap-catalog -- --output=.tmp/nutrish-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit Nutrish product sitemap URL (${DEFAULT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.nutrish\.com\/product-sitemap\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://www.nutrish.com/product-sitemap.xml.");
  }
}
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .replace(/\bPurrfection\b/g, "Purrfection")
    .replace(/\bPurrcata\b/g, "Purrcata")
    .replace(/\bFish A Licious\b/g, "Fish-a-Licious")
    .replace(/\bCatch Iatore\b/g, "Catch-Iatore")
    .replace(/\bStroganwoof\b/g, "Stroganwoof")
    .replace(/\bMuttballs\b/g, "Muttballs")
    .replace(/\s+([,])/g, "$1")
    .trim();
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/www\.nutrish\.com\/product\/[a-z0-9-]+$/i.test(url)) return "";
  return url;
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Nutrish sitemap fetch failed ${response.status}: ${url}`);
...
function inferNutrishPetTypes(row) {
  const slug = normalizeText(row.retailerProductId);
  if (/\b(puppy|dog|dogs)\b/.test(slug)) return ["dog"];
  if (/\b(cat|cats|kitten|kittens)\b/.test(slug)) return ["cat"];
  return inferPetTypes({
    brand: BRAND,
    product_name: `${row.name} ${row.sourceUrl}`,
    cache_key: normalizeCacheKey(`${row.name} ${row.sourceUrl}`),
    source: "nutrish_sitemap",
  }, { includeAmbiguous: false });
}

function isCandidateFoodRow(row) {
  const text = normalizeText(`${row.name} ${row.sourceUrl} ${row.retailerProductId}`);
  const petTypes = inferNutrishPetTypes(row);
  if (!/^https:\/\/www\.nutrish\.com\/product\//i.test(row.sourceUrl)) return false;
  if (petTypes.length !== 1) return false;
  if (!/\b(dry|wet)\b/.test(text)) return false;
  if (!/\b(dog|cat|puppy|kitten)\b/.test(text)) return false;
...
async function main() {
  ...
  const manifest = {
    ...
    source: "nutrish_product_sitemap",
    sitemapUrl,
    includeExisting,
    includeTreats,
    skipExistingScan,
  ...
  console.log(`Nutrish products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Nutrish sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Nutrish sitemap catalog target export failed:", err.message);
  process.exit(1);
});
