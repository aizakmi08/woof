...
const DEFAULT_SITEMAP_INDEX_URL = "https://www.farmandfleet.com/sitemap.xml.gz";
const DEFAULT_OUTPUT = ".tmp/farmandfleet-sitemap-catalog-targets.json";
...
const NON_FOOD_TERMS = [
  ...
  "medicine", "medication", "dewormer", "flea", "tick", "toothpaste", "shoes",
  "shirt", "jacket", "gloves", "towel", "bib", "blanket", "plush",
];

const BRAND_HINTS = [
  "9 Lives", "ACANA", "Almo Nature", "Applaws", "Blue Buffalo", "Blue Wilderness",
  "Canidae", "Cesar", "Diamond Naturals", "Eukanuba", "Fancy Feast",
  "Farmina", "Friskies", "Freshpet", "Fromm", "Fussie Cat",
  "Hill's Prescription Diet", "Hill's Science Diet", "IAMS", "Instinct",
  "Merrick", "Meow Mix", "Natural Balance", "Nature's Recipe", "Nulo",
  "NutriSource", "Open Farm", "Orijen", "Pedigree", "Primal",
  "Purina Beneful", "Purina Cat Chow", "Purina Dog Chow", "Purina ONE",
  "Purina Pro Plan", "Purina", "Rachael Ray Nutrish", "Redford Naturals",
  "Royal Canin", "Sheba", "Solid Gold", "Stella & Chewy's",
  "Taste of the Wild", "The Honest Kitchen", "Tiki Cat", "Vital Essentials",
  "Wellness CORE", "Wellness", "Weruva", "Whiskas", "Ziwi Peak",
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:farmandfleet-sitemap-catalog -- --output=.tmp/farmandfleet-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap-index=url     Farm & Fleet sitemap index URL (default ${DEFAULT_SITEMAP_INDEX_URL})
  --sitemap=url[,url]     Explicit Farm & Fleet product sitemap URL(s), comma-separated. Overrides sitemap-index.
  --max-sitemaps=N        Number of product sitemap files to read, 1-${MAX_SITEMAPS} (default ${MAX_SITEMAPS})
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.farmandfleet\.com\/sitemap\.xml\.gz$/i.test(sitemapIndexUrl)) {
    usage("--sitemap-index must be https://www.farmandfleet.com/sitemap.xml.gz.");
  }
  for (const url of explicitSitemapUrls) {
    if (!/^https:\/\/www\.farmandfleet\.com\/sitemap_products\d+\.xml\.gz$/i.test(url)) {
      usage("--sitemap values must be Farm & Fleet product sitemap URLs like https://www.farmandfleet.com/sitemap_products01.xml.gz.");
    }
  }
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/www\.farmandfleet\.com\/products\/\d+-[^/]+\.html$/i.test(url)) return "";
  return url;
}

function cleanImageUrl(value) {
  const url = decodeXml(value);
  if (!/^https:\/\/products\.blains\.com\//i.test(url)) return "";
...
async function fetchText(url) {
  ...
  if (!response.ok) {
    throw new Error(`Farm & Fleet sitemap fetch failed ${response.status}: ${url}`);
  }
...
function extractSitemapLocs(xml) {
  return [...String(xml || "").matchAll(/<loc>([\s\S]*?)<\/loc>/gi)]
    ...
    .filter((url) => /^https:\/\/www\.farmandfleet\.com\/sitemap_products\d+\.xml\.gz$/i.test(url))
...
function parseProductUrls(xml, sitemapUrl) {
  ...
  for (const match of String(xml || "").matchAll(/<url>([\s\S]*?)<\/url>/gi)) {
    ...
    const captionName = cleanProductName(block.match(/<image:caption>([\s\S]*?)<\/image:caption>/i)?.[1]);
    const name = captionName || productNameFromUrl(sourceUrl);
    if (!name) continue;
    rows.push({
      sourceUrl,
      name,
...
function normalizeTarget(row) {
  ...
  return {
    ...
    cacheKey,
    source: "farmandfleet_sitemap_export",
...
async function resolveSitemapUrls() {
  ...
  if (discovered.length === 0) {
    throw new Error("Farm & Fleet sitemap index did not expose product sitemap URLs.");
  }
  return discovered;
}
...
async function main() {
  ...
  const manifest = {
    ...
    source: "farmandfleet_product_sitemap",
    sitemapIndexUrl,
    sitemapUrls,
    explicitSitemapUrls,
    includeExisting,
  ...
  console.log(`Farm & Fleet sitemaps fetched: ${sitemapsFetched}`);
  console.log(`Farm & Fleet products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Farm & Fleet sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Farm & Fleet sitemap catalog target export failed:", err.message);
  process.exit(1);
});
