...
const source = fs.readFileSync(path.join(root, "scripts/backfill-merrick-official-products.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Merrick official product backfill guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("require(\"dotenv\").config({ quiet: true })") &&
    ...
    source.includes("Set MERRICK_PRODUCT_IMPORT_SERVICE_KEY, PRODUCT_LOOKUP_SERVICE_KEY, or Supabase service key") &&
    source.includes("if (!dryRun && !MERRICK_PRODUCT_IMPORT_SERVICE_KEY)"),
  "official Merrick product import must be env-quiet and service-key gated for writes"
);

assert(
  source.includes("DEFAULT_INPUT = \".tmp/merrick-sitemap-catalog-targets.json\"") &&
    source.includes("DEFAULT_EXPORT_PARSED = \".tmp/merrick-official-product-data.json\"") &&
...
assert(
  source.includes("merrickpetcare\\.com\\/shop") &&
    ...
    source.includes("field--name-field-text-nutrition") &&
    source.includes("<h3[^>]*>\\s*Ingredients:?\\s*<\\/h3>") &&
    source.includes("field--name-field-ingredients-pdf") &&
  ...
  "script must parse official Merrick ingredient text without product-lookup or browser scraping"
);
...
assert(
  ...
  "script must parse official Merrick guaranteed analysis and calorie fields into the nutrient-panel contract"
);

assert(
  source.includes("async function fetchExistingCacheKeys()") &&
    source.includes("/rest/v1/product_data") &&
    source.includes("brand.ilike.*Merrick*") &&
    source.includes("Existing Merrick keys skipped") &&
    source.includes("includeExisting"),
  "script must skip existing Merrick product_data aliases unless explicitly included"
);
...
assert(
  ...
  "script must write official Merrick ingredients through service-role product_data RPCs and verify nutrient writes"
);

assert(
  source.includes("source: \"merrick_official_product_data\"") &&
...
assert(
  packageJson.includes('"backfill:merrick-official-products": "node scripts/backfill-merrick-official-products.js"') &&
    packageJson.includes('"test:merrick-official-product-backfill": "node scripts/test-merrick-official-product-backfill-guards.js"') &&
    packageJson.includes("npm run test:merrick-official-product-backfill"),
  "package scripts must expose Merrick official product backfill and include its guard in test:guards"
);

console.log("Merrick official product backfill guard passed");
