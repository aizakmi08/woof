...
const source = fs.readFileSync(path.join(root, "scripts/export-orijen-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`ORIJEN sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  ...
  "exporter must read the official ORIJEN en-US sitemap, derive titles from product path slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("chunks and shreds") &&
    source.includes("wet cat food") &&
    source.includes("real meat") &&
    source.includes("if (") &&
    source.includes("freeze dried food medallions") &&
    source.includes("epic bites freeze dried food") &&
    source.includes("CORE_FOOD_TERMS") &&
    source.includes("\"amazing grains\"") &&
    source.includes("\"wild reserve\"") &&
    source.includes("\"freshprey\"") &&
    source.includes("\"guardian 8\"") &&
    ...
    source.includes("function inferOrijenPetTypes(row)") &&
    ...
    source.includes("if (!/^https:\\/\\/www\\.orijenpetfoods\\.com\\/en-US\\/(dogs|cats)\\/(dog-food|cat-food)\\//i.test(row.sourceUrl)) return false") &&
  ...
  "exporter must strictly filter ORIJEN sitemap rows to complete dry/wet/fresh dog and cat foods while rejecting fdt freeze-dried treats, accessories, and non-food rows"
);
...
assert(
  source.includes("source: \"orijen_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"orijen_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:orijen-sitemap-catalog": "node scripts/export-orijen-sitemap-targets.js"') &&
    packageJson.includes('"test:orijen-sitemap-catalog-export": "node scripts/test-orijen-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:orijen-sitemap-catalog-export"),
  "package scripts must expose ORIJEN sitemap exporter and include its guard in test:guards"
);

console.log("ORIJEN sitemap catalog export guard passed");
