...
const DEFAULT_SITEMAP_URL = "https://www.naturalbalanceinc.com/sitemap.xml";
const DEFAULT_OUTPUT = ".tmp/naturalbalance-sitemap-catalog-targets.json";
const REST_PAGE_SIZE = 1000;
const USER_AGENT = "Mozilla/5.0 Woof catalog coverage export";
const BRAND = "Natural Balance";
...
const DEFAULT_PRODUCT_FORMS = new Set(["dry", "wet"]);
const PRODUCT_LINE_TERMS = [
  "limited ingredient", "limited ingredient reserve", "original ultra",
  "platefulls", "delectable delights", "fat cats", "fat dogs",
];
const CORE_FOOD_TERMS = [
  "recipe", "canned", "stew", "gravy", "dry", "wet", "chicken", "salmon",
  "duck", "turkey", "lamb", "beef", "fish", "tuna", "venison", "bison",
  "vegetarian", "green pea", "sweet potato", "brown rice", "kitten",
  "indoor", "all life stage", "reduced calorie",
];
const TREAT_TERMS = [
  "treat", "treats", "mini rewards", "crunchy biscuits", "jumpin stix",
  "soft chewy", "biscuit", "biscuits", "chew", "chews", "reward", "rewards",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:naturalbalance-sitemap-catalog -- --output=.tmp/naturalbalance-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit Natural Balance sitemap URL (${DEFAULT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.naturalbalanceinc\.com\/sitemap\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://www.naturalbalanceinc.com/sitemap.xml.");
  }
}
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .join("")
    .replace(/\bCatatouille\b/g, "Catatouille")
    .replace(/\bOfishally\b/g, "O'Fishally")
    .replace(/\bPurrrfect\b/g, "Purrfect")
    .replace(/\bWooferole\b/g, "Wooferole")
    .replace(/\bDu(c|k)ken Itas\b/gi, "Ducken-itas")
    .replace(/\s+([,])/g, "$1")
    .trim();
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/www\.naturalbalanceinc\.com\/(dog|cat)-recipes\/(dry|wet|treat)\/[a-z0-9-]+\/[a-z0-9-]+$/i.test(url)) {
    return "";
  }
  return url;
}
...
function parsedProductPath(url) {
  try {
    ...
    const [speciesSegment, form, productLine, slug] = segments;
    const petType = speciesSegment === "dog-recipes" ? "dog" : speciesSegment === "cat-recipes" ? "cat" : "";
    if (!petType || !form || !productLine || !slug) return null;
    return { petType, form, productLine, slug };
  } catch {
    return null;
  }
}
...
function productNameFromSlug(slug) {
  return cleanProductName(String(slug || "").replace(/--+/g, "-").replace(/-/g, " "));
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Natural Balance sitemap fetch failed ${response.status}: ${url}`);
  ...
  return [...String(xml || "").matchAll(/<url>([\s\S]*?)<\/url>/gi)]
    .map((match) => {
      ...
      return {
        ...
        expectedPetType: parsed.petType,
...
function inferNaturalBalancePetTypes(row) {
  const inferred = inferPetTypes({
    brand: BRAND,
    product_name: `${row.expectedPetType} ${row.name} ${row.sourceUrl}`,
    cache_key: normalizeCacheKey(`${row.expectedPetType} ${row.name} ${row.sourceUrl}`),
    source: "naturalbalance_sitemap",
  }, { includeAmbiguous: false });
  if (inferred.includes(row.expectedPetType)) return [row.expectedPetType];
  return [row.expectedPetType].filter(Boolean);
}

function isCandidateFoodRow(row) {
  const text = normalizeText(`${row.name} ${row.sourceUrl} ${row.retailerProductId} ${row.productLine} ${row.form}`);
  const petTypes = inferNaturalBalancePetTypes(row);
  if (!/^https:\/\/www\.naturalbalanceinc\.com\/(dog|cat)-recipes\/(dry|wet|treat)\//i.test(row.sourceUrl)) return false;
  if (petTypes.length !== 1) return false;
  if (!DEFAULT_PRODUCT_FORMS.has(row.form) && !(includeTreats && row.form === "treat")) return false;
  if (row.form === "treat" && !includeTreats) return false;
  ...
  const hasFoodLine = PRODUCT_LINE_TERMS.some((term) => hasTerm(text, term));
  const coreFood = CORE_FOOD_TERMS.some((term) => hasTerm(text, term));
  return hasFoodLine && (coreFood || includeTreats);
}
...
function normalizeTarget(row) {
  ...
  return {
    ...
    cacheKey,
    source: "naturalbalance_sitemap_export",
...
async function main() {
  ...
  const manifest = {
    ...
    source: "naturalbalance_product_sitemap",
    sitemapUrl,
    includeExisting,
    includeTreats,
    skipExistingScan,
  ...
  console.log(`Natural Balance products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Natural Balance sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Natural Balance sitemap catalog target export failed:", err.message);
  process.exit(1);
});
