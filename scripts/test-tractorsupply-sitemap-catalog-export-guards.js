...
const source = fs.readFileSync(path.join(root, "scripts/export-tractorsupply-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Tractor Supply sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://www.tractorsupply.com/sitemap.xml") &&
    source.includes("sitemap_product_") &&
  ...
  "exporter must discover Tractor Supply product sitemaps, derive titles from slugs, and avoid product-page scraping or product-lookup calls"
);

assert(
  source.includes("function parseProductRows(xml, sitemapUrl)") &&
    source.includes("extractFirstImageUrl") &&
  ...
  "exporter must derive cleaned slug titles, image URLs, product ids, and source sitemap provenance"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"horse feed\"") &&
    source.includes("\"poultry feed\"") &&
    source.includes("\"automatic feeder\"") &&
    source.includes("BRAND_HINTS") &&
    !source.includes("\"Tractor Supply\"") &&
    source.includes("\"4health\"") &&
    source.includes("\"Paws & Claws\"") &&
    source.includes("\"Retriever\"") &&
    source.includes("\"Loyall Life\"") &&
    ...
    source.includes("[\"nutrena loyall life\", \"Loyall Life\"]") &&
    source.includes("[\"paws claws\", \"Paws & Claws\"]") &&
  ...
  "exporter must strictly filter Tractor Supply's mixed farm-retail sitemap to complete dog/cat food-like rows and reject supplement/topper/treat/livestock rows by default"
);
...
assert(
  source.includes("source: \"tractorsupply_sitemap_export\"") &&
    source.includes("sourceQuality: \"retailer_sitemap_slug\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"tractorsupply_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:tractorsupply-sitemap-catalog": "node scripts/export-tractorsupply-sitemap-targets.js"') &&
    packageJson.includes('"test:tractorsupply-sitemap-catalog-export": "node scripts/test-tractorsupply-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:tractorsupply-sitemap-catalog-export"),
  "package scripts must expose Tractor Supply sitemap exporter and include its guard in test:guards"
);

console.log("Tractor Supply sitemap catalog export guard passed");
