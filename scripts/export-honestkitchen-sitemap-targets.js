...
const DEFAULT_OUTPUT = ".tmp/honestkitchen-sitemap-catalog-targets.json";
...
const TREAT_TERMS = [
  ...
  "goat s milk n cookies", "goats milk n cookies", "smitten", "smittens",
];
...
const SUPPLEMENTAL_FOOD_TERMS = [
  ...
  "perfect form", "digestive supplement", "cheddar crumble",
  "cheddar crumbles", "pumpkin spice latte", "functional pour overs",
  "immune support", "heart health", "hip and joint", "skin and coat",
  "protein plus grain free beef topper", "protein plus grain free fish topper",
];

const NON_FOOD_TERMS = [
  "toy", "toys", "bundle", "bowl", "scoop", "mat", "collar", "leash",
  "harness", "bed", "crate", "carrier", "litter", "shampoo", "wipes",
  "cleaner", "flea", "tick", "grooming", "brush", "comb",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:honestkitchen-sitemap-catalog -- --output=.tmp/honestkitchen-sitemap-catalog-targets.json
...
Options:
  ...
  --sitemap=url           Explicit The Honest Kitchen product sitemap URL
...
function assertConfig() {
  ...
  if (explicitSitemapUrl && !/^https:\/\/www\.thehonestkitchen\.com\/sitemap_products_\d+\.xml\?from=\d+&to=\d+$/i.test(explicitSitemapUrl)) {
    usage("--sitemap must be a The Honest Kitchen products sitemap URL.");
  }
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`The Honest Kitchen sitemap fetch failed ${response.status}: ${url}`);
...
function extractProductSitemapLocs(xml) {
  return [...String(xml || "").matchAll(/<loc>([\s\S]*?)<\/loc>/gi)]
    ...
    .filter((url) => /^https:\/\/www\.thehonestkitchen\.com\/sitemap_products_\d+\.xml\?from=\d+&to=\d+$/i.test(url))
...
async function resolveSitemapUrls() {
  ...
  if (urls.length === 0) throw new Error("No The Honest Kitchen product sitemaps found.");
  return urls;
}
...
function isCandidateFoodRow(row) {
  ...
  const coreFood = CORE_FOOD_TERMS.some((term) => hasTerm(text, term)) ||
    /\/products\/.*-(dry|wet|dehydrated)-(dog|cat|puppy)-food$/i.test(row.sourceUrl);
...
function normalizeTarget(row) {
  ...
  const petTypes = inferPetTypes({
    brand: BRAND,
    product_name: `${name} ${row.sourceUrl}`,
    cache_key: normalizeCacheKey(`${name} ${row.sourceUrl}`),
    source: "honestkitchen_sitemap",
  ...
  return {
    ...
    cacheKey,
    source: "honestkitchen_sitemap_export",
...
async function main() {
  ...
  const manifest = {
    ...
    source: "honestkitchen_product_sitemap",
    sitemapIndexUrl,
    sitemapUrls,
    explicitSitemapUrl,
    includeExisting,
  ...
  console.log(`The Honest Kitchen products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing The Honest Kitchen sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("The Honest Kitchen sitemap catalog target export failed:", err.message);
  process.exit(1);
});
