...
const DEFAULT_SITEMAP_INDEX_URL = "https://www.tractorsupply.com/sitemap.xml";
const DEFAULT_OUTPUT = ".tmp/tractorsupply-sitemap-catalog-targets.json";
...
const TREAT_TERMS = [
  ...
  "trachea", "tendon", "sausage", "stick", "sticks", "antler",
];
...
const NON_FOOD_TERMS = [
  ...
  "aquarium", "parakeet", "reptile", "small animal", "hamster", "rabbit hutch",
  "hay", "bedding", "catfish", "fish food", "pond fish",
  "horse feed", "cattle feed", "calf feed", "goat feed", "sheep feed",
  "poultry feed", "chicken feed", "duck feed", "deer feed", "wild bird",
  "screen protector", "phone case", "automatic feeder", "slow feeder",
  "food bowl", "food mat", "food storage", "can saver",
];

const BRAND_HINTS = [
  "9Lives", "ACANA", "Against the Grain", "Beneful", "Bil-Jac",
  "Blue Buffalo", "Canidae", "Cesar", "Chicken Soup for the Soul",
  "Diamond Naturals", "Diamond", "Eagle Pack", "Earthborn Holistic",
  "Eukanuba", "Fancy Feast", "4health", "Freshpet", "Friskies",
  "Fromm", "Gentle Giants", "Hill's Prescription Diet",
  "Hill's Science Diet", "I and Love and You", "Iams", "Instinct",
  "Kit & Kaboodle", "Loyall Life", "Meow Mix", "Merrick", "Natural Balance",
  "Nature's Recipe", "NutriSource", "Nutrena", "Nulo", "Nutro",
  "Open Farm", "Orijen", "Paws & Claws", "Pedigree", "Purina Dog Chow",
  "Purina ONE", "Purina Pro Plan", "Purina", "Rachael Ray Nutrish",
  "Retriever", "Royal Canin", "Sheba", "Solid Gold", "Sportmix",
  "Stella & Chewy's", "Taste of the Wild", "Tiki Cat", "Tiki Dog",
  "True Acre Foods", "Victor", "Wellness", "Weruva", "Whiskas",
  "Wholesomes", "Whole Earth Farms", "Zignature",
...
const BRAND_PREFIX_ALIASES = [
  ...
  ["nutrena loyall life", "Loyall Life"],
  ...
  ["true acre foods", "True Acre Foods"],
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:tractorsupply-sitemap-catalog -- --output=.tmp/tractorsupply-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap-index=url     Tractor Supply sitemap index URL (${DEFAULT_SITEMAP_INDEX_URL})
  --sitemap=url           Explicit Tractor Supply product sitemap URL
...
function assertConfig() {
  ...
  if (!Number.isFinite(maxSitemaps) || maxSitemaps < 0) {
    usage("--max-sitemaps must be a non-negative number.");
  }
  if (!/^https:\/\/www\.tractorsupply\.com\/sitemap\.xml$/i.test(sitemapIndexUrl)) {
    usage("--sitemap-index must be https://www.tractorsupply.com/sitemap.xml.");
  }
  if (explicitSitemapUrl && !/^https:\/\/www\.tractorsupply\.com\/sitemap_product_\d+\.xml$/i.test(explicitSitemapUrl)) {
    usage("--sitemap must be a Tractor Supply product sitemap URL.");
  }
}
...
function normalizeCacheKey(text) {
  return String(text || "")
    ...
    .replace(/[-/&+]/g, " ")
    .replace(/[^a-z0-9\s]/g, "")
    .replace(/\b\d+(\s+)?(lb|lbs|pound|pounds|oz|ounce|ounces|kg|g|gram|grams|can|cans|bag|bags|case|pack|ct|count|tray|trays|pouch|pouches)\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .replace(/\bPaws Claws\b/g, "Paws & Claws")
    .replace(/\bW\b/g, "with")
...
function cleanUrl(value) {
  ...
  if (parsed.protocol !== "https:" || parsed.hostname !== "www.tractorsupply.com") return "";
  if (!/^\/tsc\/product\/[^/?#]+$/i.test(parsed.pathname)) return "";
...
function productIdFromUrl(url) {
  try {
    const slug = new URL(url).pathname.split("/").filter(Boolean).pop() || "";
    const numericSuffix = slug.match(/-(\d{5,})$/);
    return numericSuffix ? numericSuffix[1] : slug;
  } catch {
    return "";
  }
}
...
async function fetchText(url) {
  ...
  const timer = setTimeout(() => controller.abort(new Error(`timeout fetching ${url}`)), 120_000);
  try {
    ...
    if (!response.ok) {
      throw new Error(`Tractor Supply sitemap fetch failed ${response.status}: ${url}`);
    }
...
function extractProductSitemapLocs(xml) {
  return [...String(xml || "").matchAll(/<loc>([\s\S]*?)<\/loc>/gi)]
    ...
    .filter((url) => /^https:\/\/www\.tractorsupply\.com\/sitemap_product_\d+\.xml$/i.test(url))
...
async function resolveSitemapUrls() {
  ...
  if (urls.length === 0) {
    throw new Error("Tractor Supply sitemap index did not expose product sitemap URLs.");
  }
  return maxSitemaps > 0 ? urls.slice(0, maxSitemaps) : urls;
}

function extractFirstImageUrl(urlBlock) {
  const match = String(urlBlock || "").match(/<image:loc>([\s\S]*?)<\/image:loc>/i);
...
function parseProductRows(xml, sitemapUrl) {
  return [...String(xml || "").matchAll(/<url>([\s\S]*?)<\/url>/gi)]
    .map((match) => {
      ...
      const sourceUrl = locMatch ? cleanUrl(locMatch[1]) : "";
      if (!sourceUrl) return null;
      return {
        sourceUrl,
        name: productNameFromUrl(sourceUrl),
        productId: productIdFromUrl(sourceUrl),
        imageUrl: extractFirstImageUrl(urlBlock),
        sitemapUrl,
      };
    })
    .filter(Boolean);
...
function normalizeTarget(row) {
  ...
  return {
    ...
    cacheKey,
    source: "tractorsupply_sitemap_export",
...
async function main() {
  ...
  const manifest = {
    ...
    source: "tractorsupply_product_sitemap",
    sitemapIndexUrl,
    sitemapUrls,
    explicitSitemapUrl,
    maxSitemaps,
  ...
  console.log(`Tractor Supply products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Tractor Supply sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Tractor Supply sitemap catalog target export failed:", err.message);
  process.exit(1);
});
