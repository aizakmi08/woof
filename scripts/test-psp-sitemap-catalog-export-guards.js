...
const source = fs.readFileSync(path.join(root, "scripts/export-psp-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`PSP sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
    source.includes("https://www.petsuppliesplus.com/sitemap.xml") &&
    source.includes("const DEFAULT_SITEMAP_URL = \"https://edge.sitecorecloud.io/petsupplies9cf0-petsuppliesplus-prod-d6e8/media/Project/PetSuppliesPlus/psp/Sitemaps/psp-custom/sitemap.xml\"") &&
    source.includes("const configuredSitemapUrl = explicitSitemapUrl || DEFAULT_SITEMAP_URL") &&
    source.includes("edge.sitecorecloud.io") &&
  ...
  "exporter must default to the reachable PSP Sitecore sitemap and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("function parseProductUrls(xml, sitemapUrl)") &&
    ...
    source.includes("skuFromUrl(sourceUrl)") &&
    source.includes("case of|pack of") &&
    source.includes("oz|fl\\s*-?\\s*oz|lb|lbs|kg|g|pound|ounce"),
  "exporter must derive cleaned product titles, SKU/barcode hints, product ids, and source sitemap provenance from sitemap URLs"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("if (!includeTreats && pathName.includes(\"/treats/\")) return false") &&
    source.includes("return coreFood || treat"),
  "exporter must strictly filter PSP sitemap URLs to dog/cat food-like products and preserve brand hints before backfill"
);
...
assert(
  source.includes("source: \"psp_sitemap_export\"") &&
    ...
    source.includes("barcode: row.sku || \"\"") &&
    source.includes("searchTerms: [...new Set([name, row.sourceUrl, row.sku].filter(Boolean))]") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"psp_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:psp-sitemap-catalog": "node scripts/export-psp-sitemap-targets.js"') &&
    packageJson.includes('"test:psp-sitemap-catalog-export": "node scripts/test-psp-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:psp-sitemap-catalog-export"),
  "package scripts must expose PSP sitemap exporter and include its guard in test:guards"
);

console.log("PSP sitemap catalog export guard passed");
