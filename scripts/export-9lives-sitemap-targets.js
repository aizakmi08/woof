...
const DEFAULT_SITEMAP_URL = "https://www.9lives.com/sitemap.xml";
const DEFAULT_PRODUCT_SITEMAP_URL = "https://www.9lives.com/product-sitemap.xml";
const DEFAULT_OUTPUT = ".tmp/9lives-sitemap-catalog-targets.json";
...
const CORE_FOOD_TERMS = [
  "wet cat food", "dry cat food", "pate", "shreds", "bites", "variety pack",
  "kitten", "senior", "daily essentials", "indoor essentials", "urinary tract",
  "proteinplus", "morris catch", "real tuna", "real chicken", "real beef",
  "ocean fish", "seafood", "poultry", "favorites", "gravy", "sauce",
];
const REJECT_TERMS = [
  "treat", "treats", "snack", "snacks", "chew", "chews", "jerky",
  "topper", "toppers", "complement", "complements", "supplement",
  "supplements", "broth", "bisque", "toy", "toys", "litter",
  "catnip", "accessory", "accessories",
];
const NON_FOOD_TERMS = [
  "bowl", "mat", "collar", "leash", "harness", "bed", "crate", "carrier",
  "shampoo", "wipes", "cleaner", "flea", "tick", "grooming", "brush", "comb",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:9lives-sitemap-catalog -- --output=.tmp/9lives-sitemap-catalog-targets.json

Options:
  --output=path.json          Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url               Explicit 9Lives sitemap index URL (${DEFAULT_SITEMAP_URL})
  --product-sitemap=url       Explicit 9Lives product sitemap URL (${DEFAULT_PRODUCT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.9lives\.com\/sitemap\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://www.9lives.com/sitemap.xml.");
  }
  if (!/^https:\/\/www\.9lives\.com\/product-sitemap\.xml$/i.test(productSitemapUrl)) {
    usage("--product-sitemap must be https://www.9lives.com/product-sitemap.xml.");
  }
}
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .replace(/\bProteinplus\b/g, "ProteinPlus")
    .replace(/\s+([,])/g, "$1")
    .trim();
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/www\.9lives\.com\/product\/[a-z0-9-]+$/i.test(url)) return "";
  return url;
}
...
function formFromSlug(slug) {
  ...
  if (hasTerm(text, "dry cat food")) return "dry";
  if (hasTerm(text, "wet cat food")) return "wet";
  return "";
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`9Lives sitemap fetch failed ${response.status}: ${url}`);
...
function infer9LivesPetTypes(row) {
  const inferred = inferPetTypes({
    brand: BRAND,
    product_name: `cat ${row.name} ${row.sourceUrl}`,
    cache_key: normalizeCacheKey(`cat ${BRAND} ${row.name} ${row.sourceUrl}`),
    source: "9lives_sitemap",
  }, { includeAmbiguous: false });
  if (inferred.includes("cat")) return ["cat"];
  return ["cat"];
}

function isCandidateFoodRow(row) {
  const text = normalizeText(`${row.name} ${row.sourceUrl} ${row.retailerProductId} ${row.form}`);
  const petTypes = infer9LivesPetTypes(row);
  if (!/^https:\/\/www\.9lives\.com\/product\//i.test(row.sourceUrl)) return false;
  if (petTypes.length !== 1 || petTypes[0] !== "cat") return false;
...
async function main() {
  ...
  if (productSitemaps.length !== 1) throw new Error(`9Lives product sitemap missing from index: ${productSitemapUrl}`);
  ...
  console.log(`9Lives products scanned: ${scannedProducts}`);
  console.log(`Product sitemaps scanned: ${productSitemaps.length}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing 9Lives sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("9Lives sitemap catalog target export failed:", err.message);
  process.exit(1);
});
