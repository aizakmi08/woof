...
const DEFAULT_SITEMAP_URL = "https://victorpetfood.com/sitemap.xml";
const DEFAULT_OUTPUT = ".tmp/victor-sitemap-catalog-targets.json";
...
const DOG_SLUGS = new Set([
  ...
  "grain-free-lamb-meal-sweet-potato-recipe",
...
const CORE_FOOD_TERMS = [
  "hi pro plus", "hi-pro plus", "high energy", "multi pro", "multi-pro", "professional", "elite canine",
  "yukon river", "lamb meal", "ocean fish", "beef meal", "brown rice",
  "performance", "nutra pro", "senior healthy weight", "active dog puppy",
  "hero canine", "fit feline", "feline", "cat", "formula", "recipe",
];

const TREAT_TERMS = [
  "treat", "treats", "crunchy", "snack", "snacks", "biscuit", "biscuits",
  "chew", "chews", "jerky",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:victor-sitemap-catalog -- --output=.tmp/victor-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit VICTOR sitemap URL (${DEFAULT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/victorpetfood\.com\/sitemap\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://victorpetfood.com/sitemap.xml.");
  }
}
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .join("")
    .replace(/\bVictor\b/g, "VICTOR")
    .replace(/\bHi Pro\b/g, "Hi-Pro")
    .replace(/\bMulti Pro\b/g, "Multi-Pro")
    .replace(/\bNutra Pro\b/g, "Nutra Pro")
    .replace(/\s+([,])/g, "$1")
    .trim();
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/victorpetfood\.com\/products\/[a-z0-9-]+$/i.test(url)) return "";
  return url;
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`VICTOR sitemap fetch failed ${response.status}: ${url}`);
...
function inferVictorPetTypes(row) {
  const slug = String(row.retailerProductId || "").toLowerCase();
  if (DOG_SLUGS.has(slug)) return ["dog"];
  if (CAT_SLUGS.has(slug)) return ["cat"];
  return inferPetTypes({
    brand: BRAND,
    product_name: `${row.name} ${row.sourceUrl}`,
    cache_key: normalizeCacheKey(`${row.name} ${row.sourceUrl}`),
    source: "victor_sitemap",
...
function isCandidateFoodRow(row) {
  ...
  const slug = String(row.retailerProductId || "").toLowerCase();
  const petTypes = inferVictorPetTypes(row);
  if (!/^https:\/\/victorpetfood\.com\/products\//i.test(row.sourceUrl)) return false;
  if (CATEGORY_SLUGS.has(slug)) return false;
  if (petTypes.length !== 1) return false;
  if (!DOG_SLUGS.has(slug) && !CAT_SLUGS.has(slug)) return false;
...
async function main() {
  ...
  console.log(`VICTOR products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing VICTOR sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("VICTOR sitemap catalog target export failed:", err.message);
  process.exit(1);
});
