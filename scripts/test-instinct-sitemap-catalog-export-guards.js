...
const source = fs.readFileSync(path.join(root, "scripts/export-instinct-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Instinct sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  ...
  "exporter must read the official Instinct product sitemap, derive names from product slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"raw meals freeze dried\"") &&
    source.includes("\"freshly crafted meals\"") &&
    source.includes("\"pate split cup\"") &&
  ...
  "exporter must strictly filter Instinct sitemap rows to complete foods and reject toppers, mixers, multivitamins, treats, and non-food rows by default"
);
...
assert(
  source.includes("source: \"instinct_sitemap_export\"") &&
    source.includes("sourceQuality: \"official_brand_sitemap_slug\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"instinct_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:instinct-sitemap-catalog": "node scripts/export-instinct-sitemap-targets.js"') &&
    packageJson.includes('"test:instinct-sitemap-catalog-export": "node scripts/test-instinct-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:instinct-sitemap-catalog-export"),
  "package scripts must expose Instinct sitemap exporter and include its guard in test:guards"
);

console.log("Instinct sitemap catalog export guard passed");
