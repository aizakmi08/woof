...
const source = fs.readFileSync(path.join(root, "scripts/export-petsupplies-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`PetSupplies sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://www.petsupplies.com/sitemap.xml") &&
  ...
  "exporter must discover PetSupplies Shopify product sitemaps, derive titles from sitemap image titles, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"Bully Performance\"") &&
    source.includes("\"Hi-Tor\"") &&
    source.includes("\"Hunters Special\"") &&
    source.includes("\"Triumph\"") &&
    source.includes("\"The Honest Kitchen\"") &&
    source.includes("BRAND_PREFIX_ALIASES") &&
    source.includes("[\"bully performance\", \"Bully Performance\"]") &&
    source.includes("[\"hi tor\", \"Hi-Tor\"]") &&
    source.includes("[\"hunters special\", \"Hunters Special\"]") &&
    source.includes("[\"stella chewy s\", \"Stella & Chewy's\"]") &&
    source.includes("[\"triumph\", \"Triumph\"]") &&
  ...
  "exporter must strictly filter PetSupplies's mixed Shopify sitemap to complete dog/cat food rows and reject treat/supplement/accessory rows by default"
);
...
assert(
  source.includes("source: \"petsupplies_sitemap_export\"") &&
    source.includes("sourceQuality: \"retailer_sitemap_title\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"petsupplies_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:petsupplies-sitemap-catalog": "node scripts/export-petsupplies-sitemap-targets.js"') &&
    packageJson.includes('"test:petsupplies-sitemap-catalog-export": "node scripts/test-petsupplies-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:petsupplies-sitemap-catalog-export"),
  "package scripts must expose PetSupplies sitemap exporter and include its guard in test:guards"
);

console.log("PetSupplies sitemap catalog export guard passed");
