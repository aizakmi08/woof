...
const source = fs.readFileSync(path.join(root, "scripts/export-canidae-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Canidae sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://canidae.com/xmlsitemap.php?type=products&page=1") &&
    source.includes("const BRAND = \"Canidae\"") &&
  ...
  "exporter must read the official Canidae product sitemap, derive titles from sitemap slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"pure farm to bowl\"") &&
    source.includes("\"balanced bowl\"") &&
    source.includes("\"all life stages\"") &&
    source.includes("\"under the sun\"") &&
    source.includes("TREAT_TERMS") &&
    source.includes("\"jerky\"") &&
    source.includes("SUPPLEMENTAL_FOOD_TERMS") &&
    source.includes("\"protein topper\"") &&
    source.includes("\"chunky stew toppers\"") &&
    source.includes("NON_FOOD_TERMS") &&
    source.includes("\"test product\"") &&
    !source.includes("\"bowl\",") &&
    source.includes("function inferCanidaePetTypes(row)") &&
    source.includes("hasTerm(text, \"pure farm to bowl\")") &&
    source.includes("hasTerm(text, \"under the sun\")") &&
  ...
  "exporter must strictly filter Canidae sitemap rows to complete foods and reject toppers, test products, treats, supplements, and non-food rows by default"
);
...
assert(
  source.includes("source: \"canidae_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"canidae_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:canidae-sitemap-catalog": "node scripts/export-canidae-sitemap-targets.js"') &&
    packageJson.includes('"test:canidae-sitemap-catalog-export": "node scripts/test-canidae-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:canidae-sitemap-catalog-export"),
  "package scripts must expose Canidae sitemap exporter and include its guard in test:guards"
);

console.log("Canidae sitemap catalog export guard passed");
