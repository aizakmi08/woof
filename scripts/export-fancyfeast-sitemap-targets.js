...
const DEFAULT_OUTPUT = ".tmp/fancyfeast-sitemap-catalog-targets.json";
const REST_PAGE_SIZE = 1000;
const USER_AGENT = "Mozilla/5.0 Woof catalog coverage export";
const BRAND = "Fancy Feast";
...
const CORE_FOOD_TERMS = [
  "wet cat food", "dry cat food", "classic pate", "pate", "gravy lovers",
  "timeless favorites", "medleys", "gourmet naturals", "savory centers",
  "delights with cheddar", "petites", "gems", "mousse", "grilled",
  "minced", "sliced", "flaked", "chunky", "marinated morsels",
  "variety pack", "kitten", "senior", "gravy", "sauce", "morsels",
  "chicken", "turkey", "salmon", "tuna", "beef", "whitefish",
  "ocean fish", "seafood", "poultry", "liver", "giblets", "shrimp",
  "crab", "sole", "cod", "trout", "mackerel", "florentine",
  "primavera", "tuscany", "milanese", "ragu", "carne asada",
  "barbacoa", "filet mignon",
];
const REJECT_TERMS = [
  "treat", "treats", "savory cravings", "chefs cuts", "chef s cuts",
  "appetizer", "appetizers", "topper", "toppers", "complement",
  "complements", "broth", "broths", "bisque", "bisques", "puree",
  "lickable", "layered delights", "supplement", "supplements",
  "snack", "snacks", "catnip", "toy", "toys", "litter", "accessory",
  "accessories",
];
const NON_FOOD_TERMS = [
  "faq", "article", "leash", "bowl", "mat", "collar", "harness", "bed",
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:fancyfeast-sitemap-catalog -- --output=.tmp/fancyfeast-sitemap-catalog-targets.json
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .join("")
    .replace(/\bFancy Feast\b/g, "Fancy Feast")
    .replace(/\bPate\b/g, "Pate")
    .replace(/\bPetites\b/g, "Petites")
    .replace(/\bGems\b/g, "Gems")
    .replace(/\s+([,])/g, "$1")
    .trim();
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/www\.purina\.com\/cats\/shop\/fancy-feast-[a-z0-9-]+-(wet|dry)-cat-food(?:-[a-z0-9-]+)?$/i.test(url)) return "";
  return url;
}
...
function parsedProductPath(url) {
  try {
    ...
    if (segments.length !== 3 || segments[0] !== "cats" || segments[1] !== "shop") return null;
    const slug = segments[2];
    if (!/^fancy-feast-/.test(slug) || !/-(wet|dry)-cat-food(?:-|$)/.test(slug)) return null;
    return { slug };
  } catch {
    return null;
  }
...
function productNameFromPath(parsed) {
  return cleanProductName(String(parsed.slug || "")
    .replace(/^fancy-feast-/, "")
    .replace(/-(wet|dry)-cat-food(?:-|$)/, " ")
    .replace(/\b(\d+)-cans\b/g, "$1 Cans")
...
function formFromSlug(slug) {
  if (/-dry-cat-food(?:-|$)/.test(slug)) return "dry";
  if (/-wet-cat-food(?:-|$)/.test(slug)) return "wet";
  return "";
}
...
function inferFancyFeastPetTypes(row) {
...
function isCandidateFoodRow(row) {
  ...
  const petTypes = inferFancyFeastPetTypes(row);
  if (!/^https:\/\/www\.purina\.com\/cats\/shop\/fancy-feast-/i.test(row.sourceUrl)) return false;
  if (!/-(wet|dry)-cat-food(?:-|$)/i.test(row.retailerProductId)) return false;
...
function normalizeTarget(row) {
  ...
  return {
    ...
    cacheKey,
    source: "fancyfeast_sitemap_export",
...
async function main() {
  ...
  const manifest = {
    ...
    source: "fancyfeast_product_sitemap",
    sitemapUrl,
    includeExisting,
    skipExistingScan,
    scannedProducts,
  ...
  console.log(`Fancy Feast products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Fancy Feast sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Fancy Feast sitemap catalog target export failed:", err.message);
  process.exit(1);
});
