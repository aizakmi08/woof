...
const source = fs.readFileSync(path.join(root, "scripts/export-nutro-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Nutro sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://www.nutro.com/sitemap.xml") &&
    source.includes("const BRAND = \"Nutro\"") &&
    ...
    source.includes("products\\/(dry|wet|wet-grain-free)") &&
    !source.includes("product-lookup") &&
    !source.includes("cheerio") &&
    !source.includes("puppeteer"),
  "exporter must read the official Nutro sitemap, derive titles from product slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"natural choice\"") &&
    source.includes("\"nutro so simple\"") &&
    ...
    source.includes("function inferNutroPetTypes(row)") &&
    ...
    source.includes("if (!/^https:\\/\\/www\\.nutro\\.com\\/products\\/(dry|wet|wet-grain-free)\\//i.test(row.sourceUrl)) return false") &&
  ...
  "exporter must strictly filter Nutro sitemap rows to explicit dog/cat foods and reject supplements, treats, and non-food rows by default"
);
...
assert(
  source.includes("source: \"nutro_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"nutro_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:nutro-sitemap-catalog": "node scripts/export-nutro-sitemap-targets.js"') &&
    packageJson.includes('"test:nutro-sitemap-catalog-export": "node scripts/test-nutro-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:nutro-sitemap-catalog-export"),
  "package scripts must expose Nutro sitemap exporter and include its guard in test:guards"
);

console.log("Nutro sitemap catalog export guard passed");
