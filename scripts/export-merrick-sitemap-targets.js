...
const DEFAULT_OUTPUT = ".tmp/merrick-sitemap-catalog-targets.json";
...
const CORE_FOOD_TERMS = [
  ...
  "backcountry", "grain free", "healthy grains", "limited ingredient",
  "limited ingredient diet", "lil plates", "purrfect bistro",
  "fresh selects", "kitchen comforts", "slow cooked bbq", "chunky",
  "raw infused", "bone broth coated", "variety pack", "small breed",
  "large breed", "puppy", "kitten", "senior", "adult",
  "healthy weight", "dry", "wet", "pate", "stew", "dinner", "plate",
  "pot pie", "cookout",
  "gravy", "chicken", "turkey", "salmon", "tuna", "beef", "lamb",
  "venison", "duck", "bison", "whitefish", "ocean fish", "trout",
  "rabbit", "cod", "pork", "buffalo", "fish", "liver", "potato",
  "carrot", "peas", "spinach", "barley", "brown rice", "rice",
  "oatmeal", "sweet potato",
];
const REJECT_TERMS = [
  "treat", "treats", "lickable", "puree", "purees", "topper", "toppers",
  "complement", "complements", "supplement", "supplements",
  "bisque", "appetizer", "appetizers", "snack", "snacks", "catnip",
  "food topper", "cat food topper", "dog cat food topper",
  "beef broth", "chicken broth", "turkey bone broth",
  "creamy", "alfredo", "bolognese",
...
const NON_FOOD_TERMS = [
  ...
  "grooming", "brush", "comb", "canada", "fr ca", "authors",
  "pet food finder", "where to buy", "terms", "privacy", "cookie",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:merrick-sitemap-catalog -- --output=.tmp/merrick-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit Merrick sitemap index URL (${DEFAULT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.merrickpetcare\.com\/sitemaps\/default\/sitemap\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://www.merrickpetcare.com/sitemaps/default/sitemap.xml.");
  }
}
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .join("")
    .replace(/\bMerrick\b/g, "Merrick")
    .replace(/\bBbq\b/g, "BBQ")
    .replace(/\bLil\b/g, "Lil'")
    .replace(/\s+([,])/g, "$1")
    .trim();
}
...
function formFromSlug(slug) {
  ...
  if (hasTerm(text, "fresh dog food")) return "fresh";
  if (hasTerm(text, "dry dog food") || hasTerm(text, "dry cat food")) return "dry";
  if (hasTerm(text, "wet dog food") || hasTerm(text, "wet cat food")) return "wet";
  if (hasTerm(text, "pate") || hasTerm(text, "stew") || hasTerm(text, "gravy")) return "wet";
  return "";
}

function petTypeFromSlug(slug) {
  const text = normalizeText(slug);
...
function parsedProductPath(url) {
  try {
    ...
    if (segments.length !== 2 || segments[0] !== "shop") return null;
    const slug = segments[1];
    if (!/^[a-z0-9-]+$/.test(slug)) return null;
    const petType = petTypeFromSlug(slug);
    const form = formFromSlug(slug);
    if (!petType || !form) return null;
    return { slug, petType, form };
  } catch {
    return null;
  }
}
...
function productNameFromPath(parsed) {
  return productNameFromSlug(parsed.slug);
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Merrick sitemap fetch failed ${response.status}: ${url}`);
...
async function parseProductRows(indexXml) {
  const sitemapLocs = parseLocs(indexXml)
    .filter((url) => /^https:\/\/www\.merrickpetcare\.com\/shop\/[a-z0-9-]+$/i.test(url));
  const rows = [];
...
function inferMerrickPetTypes(row) {
...
function isCandidateFoodRow(row) {
  ...
  const petTypes = inferMerrickPetTypes(row);
  if (!/^https:\/\/www\.merrickpetcare\.com\/shop\/[a-z0-9-]+$/i.test(row.sourceUrl)) return false;
...
async function main() {
  ...
  const manifest = {
    ...
    source: "merrick_product_sitemap",
    sitemapUrl,
    includeExisting,
    skipExistingScan,
    scannedProducts,
  ...
  console.log(`Merrick products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Merrick sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Merrick sitemap catalog target export failed:", err.message);
  process.exit(1);
});
