...
const DEFAULT_SITEMAP_INDEX_URL = "https://solidgoldpet.com/sitemap.xml";
const DEFAULT_OUTPUT = ".tmp/solidgold-sitemap-catalog-targets.json";
...
const CORE_FOOD_TERMS = [
  "dry food", "wet food", "dry dog food", "dry cat food", "wet dog food",
  "wet cat food", "shreds wet food", "pate with", "paté with",
  "grain free wet food", "sensitive stomach dry food", "variety wet food",
];

const TREAT_TERMS = [
  "treat", "treats", "chew", "chews", "mousse treat", "squeeze",
  "squeezeable", "jerky", "biscuit", "biscuits",
];

const SUPPLEMENTAL_FOOD_TERMS = [
  "supplement", "supplements", "powder", "meal topper", "topper",
  "toppers", "bone broth", "broth topper", "stool eating deterrent",
  "urinary tract health", "calming", "gut health chews", "joint support",
  "allergy support", "multivitamin", "digestive health chews",
  "pre & probiotic", "pre and probiotic", "functional cups",
  "meal or topper", "topper or meal", "meatball topper",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:solidgold-sitemap-catalog -- --output=.tmp/solidgold-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap-index=url     Solid Gold sitemap index URL (${DEFAULT_SITEMAP_INDEX_URL})
  --sitemap=url           Explicit Solid Gold product sitemap URL
...
function assertConfig() {
  ...
  if (!/^https:\/\/solidgoldpet\.com\/sitemap\.xml$/i.test(sitemapIndexUrl)) {
    usage("--sitemap-index must be https://solidgoldpet.com/sitemap.xml.");
  }
  if (explicitSitemapUrl && !/^https:\/\/solidgoldpet\.com\/sitemap_products_\d+\.xml\?from=\d+&to=\d+$/i.test(explicitSitemapUrl)) {
    usage("--sitemap must be a Solid Gold products sitemap URL.");
  }
}
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .join("")
    .replace(/\bNutrientboost\b/g, "NutrientBoost")
    .replace(/\bSeameal\b/g, "SeaMeal")
    .replace(/\bPaté\b/g, "Paté")
    .replace(/'S\b/g, "'s")
    .replace(/\s+([,])/g, "$1")
    .trim();
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/solidgoldpet\.com\/products\/[a-z0-9-%]+$/i.test(url)) return "";
  return url;
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Solid Gold sitemap fetch failed ${response.status}: ${url}`);
...
function extractProductSitemapLocs(xml) {
  return [...String(xml || "").matchAll(/<loc>([\s\S]*?)<\/loc>/gi)]
    ...
    .filter((url) => /^https:\/\/solidgoldpet\.com\/sitemap_products_\d+\.xml\?from=\d+&to=\d+$/i.test(url))
...
async function resolveSitemapUrls() {
  ...
  if (urls.length === 0) throw new Error("No Solid Gold product sitemaps found.");
  return urls;
}
...
function normalizeTarget(row) {
  ...
  const petTypes = inferSolidGoldPetTypes(row);
  ...
  return {
    ...
    cacheKey,
    source: "solidgold_sitemap_export",
...
async function main() {
  ...
  const manifest = {
    ...
    source: "solidgold_product_sitemap",
    sitemapIndexUrl,
    sitemapUrls,
    explicitSitemapUrl,
    includeExisting,
  ...
  console.log(`Solid Gold products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Solid Gold sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Solid Gold sitemap catalog target export failed:", err.message);
  process.exit(1);
});
