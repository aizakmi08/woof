...
const source = fs.readFileSync(path.join(root, "scripts/export-meowmix-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Meow Mix sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://www.meowmix.com/sitemap.xml") &&
    source.includes("const BRAND = \"Meow Mix\"") &&
    ...
    source.includes("www\\.meowmix\\.com\\/cat-food") &&
    source.includes("candidate.startsWith(`${sourceUrl}/`)") &&
    !source.includes("product-lookup") &&
    !source.includes("cheerio") &&
    !source.includes("puppeteer"),
  "exporter must read the official Meow Mix sitemap, derive titles from leaf cat-food paths, reject category parents, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"gravy bursts\"") &&
    source.includes("\"chunks in gravy\"") &&
    source.includes("\"pate in gravy\"") &&
    source.includes("\"tenders in sauce\"") &&
    source.includes("\"tender centers\"") &&
    source.includes("REJECT_TERMS") &&
    source.includes("\"treat\"") &&
    source.includes("\"irresistibles\"") &&
    source.includes("\"topper\"") &&
    source.includes("\"supplement\"") &&
    source.includes("\"catnip\"") &&
    source.includes("NON_FOOD_TERMS") &&
    source.includes("function inferMeowMixPetTypes(row)") &&
    ...
    source.includes("if (!/^https:\\/\\/www\\.meowmix\\.com\\/cat-food\\//i.test(row.sourceUrl)) return false") &&
  ...
  "exporter must strictly filter Meow Mix sitemap rows to core leaf cat foods while rejecting treats, toppers, supplements, accessories, and non-food rows"
);
...
assert(
  source.includes("source: \"meowmix_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"meowmix_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:meowmix-sitemap-catalog": "node scripts/export-meowmix-sitemap-targets.js"') &&
    packageJson.includes('"test:meowmix-sitemap-catalog-export": "node scripts/test-meowmix-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:meowmix-sitemap-catalog-export"),
  "package scripts must expose Meow Mix sitemap exporter and include its guard in test:guards"
);

console.log("Meow Mix sitemap catalog export guard passed");
