...
const DEFAULT_SITEMAP_URL = "https://www.meowmix.com/sitemap.xml";
const DEFAULT_OUTPUT = ".tmp/meowmix-sitemap-catalog-targets.json";
const REST_PAGE_SIZE = 1000;
const USER_AGENT = "Mozilla/5.0 Woof catalog coverage export";
const BRAND = "Meow Mix";
...
const CORE_FOOD_TERMS = [
  "cat food", "dry", "wet", "gravy bursts", "chunks in gravy", "pate in gravy",
  "tenders in sauce", "variety pack", "multipack", "kitten", "hairball",
  "indoor", "original choice", "seafood medley", "tender centers",
  "salmon", "chicken", "tuna", "beef", "turkey", "giblets", "shrimp",
  "crab", "ocean whitefish", "seafood", "poultry", "surf n turf",
];
const REJECT_TERMS = [
  "treat", "treats", "irresistibles", "snack", "snacks", "chew", "chews",
  "topper", "toppers", "complement", "complements", "supplement",
  "supplements", "broth", "bisque", "toy", "toys", "litter", "catnip",
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:meowmix-sitemap-catalog -- --output=.tmp/meowmix-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit Meow Mix sitemap URL (${DEFAULT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.meowmix\.com\/sitemap\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://www.meowmix.com/sitemap.xml.");
  }
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/www\.meowmix\.com\/cat-food\/(dry|gravy-bursts|wet)(\/[a-z0-9-]+)+$/i.test(url)) return "";
  return url;
}
...
function parsedProductPath(url) {
  try {
    ...
    const [catFood, form, ...rest] = segments;
    if (catFood !== "cat-food" || !/^(dry|gravy-bursts|wet)$/.test(form) || rest.length < 1) return null;
    return { form, rest, slug: rest[rest.length - 1] };
  } catch {
    return null;
  }
}
...
function formFromPath(parsed) {
  if (parsed.form === "dry") return "dry";
  if (parsed.form === "wet" || parsed.form === "gravy-bursts") return "wet";
  return "";
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Meow Mix sitemap fetch failed ${response.status}: ${url}`);
...
function parseProductRows(xml) {
  const allProductUrls = parseLocs(xml).map(cleanUrl).filter(Boolean);
  return allProductUrls
    .map((sourceUrl) => {
      const parsed = parsedProductPath(sourceUrl);
      if (!parsed) return null;
      if (allProductUrls.some((candidate) => candidate !== sourceUrl && candidate.startsWith(`${sourceUrl}/`))) return null;
...
function inferMeowMixPetTypes(row) {
...
function isCandidateFoodRow(row) {
  ...
  const petTypes = inferMeowMixPetTypes(row);
  if (!/^https:\/\/www\.meowmix\.com\/cat-food\//i.test(row.sourceUrl)) return false;
...
async function main() {
  ...
  const manifest = {
    ...
    source: "meowmix_product_sitemap",
    sitemapUrl,
    includeExisting,
    skipExistingScan,
    scannedProducts,
  ...
  console.log(`Meow Mix products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Meow Mix sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Meow Mix sitemap catalog target export failed:", err.message);
  process.exit(1);
});
