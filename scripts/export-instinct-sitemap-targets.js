...
const DEFAULT_OUTPUT = ".tmp/instinct-sitemap-catalog-targets.json";
...
const CORE_FOOD_TERMS = [
  ...
  "frozen lamb patties dog food", "frozen beef patties dog food",
  "frozen chicken patties dog food", "freshraw beef medallions dog food",
  "freshraw chicken medallions dog food", "freshraw turkey medallions dog food",
  "raw meals freeze dried", "freeze dried chicken dog food",
  "freeze dried beef dog food", "freeze dried lamb dog food",
  "freeze dried pollock dog food", "freeze dried puppy chicken dog food",
  "freeze dried chicken cat food", "freeze dried kitten chicken cat food",
  "freeze dried pollock cat food", "freshly crafted meals",
  "pate split cup", "flaked split cup", "original variety pack wet cat food",
  "small breed chicken wet dog food", "small breed beef wet dog food",
  "original puppy chicken wet dog food",
];

const TREAT_TERMS = [
  "treat", "treats", "biscuits", "jerky", "chew", "chews",
];

const SUPPLEMENTAL_FOOD_TERMS = [
  "topper", "toppers", "raw boost mixers", "raw boost shakers",
  "healthy cravings", "mobility support", "digestive health", "gut health",
  "skin coat health", "tranquility", "healthy energy", "multivitamin",
  "duos", "mixer", "mixers", "food topper",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:instinct-sitemap-catalog -- --output=.tmp/instinct-sitemap-catalog-targets.json
...
function productNameFromUrl(url) {
  ...
  if (!id) return "";
  return cleanProductName(id
    .replace(/-\d+$/g, "")
    .replace(/-/g, " ")
    .replace(/\binstinct\b/gi, "")
    .replace(/\b5 5 oz can pack of 12\b/gi, "")
    .replace(/\b5 5 oz can\b/gi, "")
    .replace(/\s+/g, " ")
    .trim());
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Instinct sitemap fetch failed ${response.status}: ${url}`);
...
function normalizeTarget(row) {
  ...
  const petTypes = inferInstinctPetTypes(row);
  ...
  return {
    ...
    cacheKey,
    source: "instinct_sitemap_export",
...
async function main() {
  ...
  const manifest = {
    ...
    source: "instinct_product_sitemap",
    sitemapUrl,
    includeExisting,
    includeTreats,
    skipExistingScan,
  ...
  console.log(`Instinct products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Instinct sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Instinct sitemap catalog target export failed:", err.message);
  process.exit(1);
});
