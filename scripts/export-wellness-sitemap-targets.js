...
const DEFAULT_OUTPUT = ".tmp/wellness-sitemap-catalog-targets.json";
...
const CORE_FOOD_TERMS = [
  "wellness core", "wellness complete health", "wellness simple",
  "appetizing entrees", "purrfect duos", "signature selects",
  "tiny tasters", "mini meals", "petite entrees", "stews",
  "protein bowls", "tasty textures", "healthy indulgence",
  "complete health plus", "core plus", "grain free", "grained",
  "wholesome grains", "digestive health",
];

const TREAT_TERMS = [
  "treat", "treats", "kittles", "crunchy puppy bites", "wellbars",
  "tiny trainers", "brainiac puppy treats", "rewarding life",
];

const SUPPLEMENTAL_FOOD_TERMS = [
  "bowl boosters", "topper", "toppers", "supplement", "supplements",
  "hip joint", "calming", "multi benefit", "functional topper",
  "lickable treats", "bare turkey", "bare beef", "mothers solutions",
];

const NON_FOOD_TERMS = [
  "toy", "toys", "bowl", "mat", "collar", "leash", "harness", "bed",
  "crate", "carrier", "litter", "shampoo", "wipes", "cleaner", "flea",
  "tick", "grooming", "brush", "comb", "whimzees", "old mother hubbard",
];

const CAT_LINE_TERMS = [
  "cat", "kitten", "tiny tasters", "appetizing entrees", "purrfect duos",
  "signature selects", "tasty textures", "healthy indulgence", "good kitty",
  "morsels", "minced", "sliced", "gravies", "flaked", "whipped wonders",
  "hairball", "indoor", "core plus digestive health", "core plus healthy weight",
  "core plus skin coat", "complete health grilled",
];

const DOG_LINE_TERMS = [
  "dog", "puppy", "mini meals", "petite entrees", "stews", "protein bowls",
  "complete health plus", "small breed", "large breed", "toy breed",
  "core digestive health chicken brown rice", "core digestive health small breed",
  "core digestive health whitefish brown rice", "core digestive health age advantage",
  "core digestive health large breed", "complete health grained lamb barley",
  "complete health grained whitefish sweet potato", "complete health grained senior chicken barley",
  "complete health grained chicken oatmeal", "complete health grained puppy",
  "complete health grained healthy weight chicken peas", "complete health grained beef barley",
  "complete health grain sensitive skin stomach", "simple grained", "simple grain free",
  "core grain free original", "core grain free reduced fat", "core grain free ocean",
  "core grain free wild game", "core grain free lamb", "core grain free turkey duck",
  "core wholesome grains", "core healthy weight chicken chicken meal",
  "core healthy joints", "complete health 95", "complete health pate puppy",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:wellness-sitemap-catalog -- --output=.tmp/wellness-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit Wellness Salsify product sitemap URL (${DEFAULT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.wellnesspetfood\.com\/salsify-products-sitemap\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://www.wellnesspetfood.com/salsify-products-sitemap.xml.");
  }
}
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .replace(/\bPurrfect\b/g, "Purrfect")
    .replace(/\bMousse\b/g, "Mousse")
    .replace(/\b2 8oz\b/g, "2.8 oz")
    .replace(/\s+([,])/g, "$1")
    .trim();
}
...
function productNameFromSlug(slug) {
  const cleanedSlug = String(slug || "")
    .replace(/^cat-wellness-/, "wellness-")
    .replace(/^dog-wellness-/, "wellness-")
    .replace(/^wellness-/, "")
    .replace(/-2$/, "")
    .replace(/egglamb/g, "egg-lamb")
    .replace(/-/g, " ");
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Wellness sitemap fetch failed ${response.status}: ${url}`);
...
function inferWellnessPetTypes(row) {
  const idText = normalizeText(row.retailerProductId);
  const text = normalizeText(`${row.name} ${row.sourceUrl} ${row.retailerProductId}`);

  if (idText.startsWith("cat wellness")) return ["cat"];
  if (idText.startsWith("dog wellness")) return ["dog"];
  if (hasTerm(text, "tiny tasters")) return ["cat"];
  if (hasTerm(text, "mini meals")) return ["dog"];
  if (hasTerm(text, "petite entrees")) return ["dog"];
  if (hasTerm(text, "protein bowls")) return ["dog"];
  if (hasTerm(text, "stews")) return ["dog"];

  const inferred = inferPetTypes({
    brand: BRAND,
    product_name: `${row.name} ${row.sourceUrl}`,
    cache_key: normalizeCacheKey(`${row.name} ${row.sourceUrl}`),
    source: "wellness_sitemap",
  }, { includeAmbiguous: false });
  if (inferred.length > 0) return inferred;

  const cat = CAT_LINE_TERMS.some((term) => hasTerm(text, term));
  const dog = DOG_LINE_TERMS.some((term) => hasTerm(text, term));
...
function isCandidateFoodRow(row) {
  const text = normalizeText(`${row.name} ${row.sourceUrl} ${row.retailerProductId}`);
  if (!/^(wellness|cat-wellness|dog-wellness)-/i.test(row.retailerProductId)) return false;
  if (inferWellnessPetTypes(row).length === 0) return false;
...
function normalizeTarget(row) {
  ...
  const petTypes = inferWellnessPetTypes(row);
  ...
  return {
    ...
    cacheKey,
    source: "wellness_sitemap_export",
...
async function main() {
  ...
  const manifest = {
    ...
    source: "wellness_salsify_product_sitemap",
    sitemapUrl,
    includeExisting,
    includeTreats,
    skipExistingScan,
  ...
  console.log(`Wellness products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Wellness sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Wellness sitemap catalog target export failed:", err.message);
  process.exit(1);
});
