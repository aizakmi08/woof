...
const source = fs.readFileSync(path.join(root, "scripts/backfill-hillspet-official-products.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Hill's official product backfill guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("require(\"dotenv\").config({ quiet: true })") &&
    ...
    source.includes("Set HILLSPET_PRODUCT_IMPORT_SERVICE_KEY, PRODUCT_LOOKUP_SERVICE_KEY, or Supabase service key") &&
    source.includes("if (!dryRun && !HILLSPET_PRODUCT_IMPORT_SERVICE_KEY)"),
  "official Hill's product import must be env-quiet and service-key gated for writes"
);

assert(
  source.includes("DEFAULT_INPUT = \".tmp/hillspet-sitemap-catalog-targets.json\"") &&
    source.includes("DEFAULT_EXPORT_PARSED = \".tmp/hillspet-official-product-data.json\"") &&
...
assert(
  source.includes("www\\.hillspet\\.com") &&
    source.includes("function extractAccordionPanel(html, titlePattern)") &&
    source.includes("function parseIngredients(html)") &&
    source.includes("extractAccordionPanel(html, /^Ingredients$/i)") &&
  ...
  "script must parse official Hill's product-page ingredient accordions without product-lookup or browser scraping"
);

assert(
  source.includes("function parseNutrientPanel(html, sourceUrl)") &&
    source.includes("Average Nutrient") &&
  ...
  "script must parse official Hill's nutrient/calorie fields into the nutrient-panel contract"
);

assert(
  source.includes("async function fetchExistingCacheKeys()") &&
    source.includes("/rest/v1/product_data") &&
    source.includes("brand.ilike.*Hill*") &&
    source.includes("Existing Hill's keys skipped") &&
    source.includes("includeExisting"),
  "script must skip existing Hill's product_data aliases unless explicitly included"
);
...
assert(
  ...
  "script must write official Hill's ingredients through service-role product_data RPCs and verify nutrient writes"
);

assert(
  source.includes("source: \"hillspet_official_product_data\"") &&
...
assert(
  packageJson.includes('"backfill:hillspet-official-products": "node scripts/backfill-hillspet-official-products.js"') &&
    packageJson.includes('"test:hillspet-official-product-backfill": "node scripts/test-hillspet-official-product-backfill-guards.js"') &&
    packageJson.includes("npm run test:hillspet-official-product-backfill"),
  "package scripts must expose Hill's official product backfill and include its guard in test:guards"
);

console.log("Hill's official product backfill guard passed");
