...
const WELLNESS_PRODUCT_IMPORT_SERVICE_KEY =
  process.env.WELLNESS_PRODUCT_IMPORT_SERVICE_KEY ||
  process.env.PRODUCT_LOOKUP_SERVICE_KEY ||
  process.env.SUPABASE_SERVICE_ROLE_KEY ||
  process.env.SUPABASE_SERVICE_KEY ||
  "";
const DEFAULT_INPUT = ".tmp/wellness-sitemap-catalog-targets.json";
const DEFAULT_EXPORT_PARSED = ".tmp/wellness-official-product-data.json";
const DEFAULT_REPORT = ".tmp/wellness-official-product-backfill-report.jsonl";
const USER_AGENT = "Mozilla/5.0 Woof official Wellness product import";
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run backfill:wellness-official-products -- --dry-run
  WELLNESS_PRODUCT_IMPORT_SERVICE_KEY=... npm run backfill:wellness-official-products

Options:
  --input=path.json           Wellness catalog target manifest (default ${DEFAULT_INPUT})
...
function assertConfig() {
  ...
  if (!fs.existsSync(inputPath)) usage(`Missing Wellness input manifest: ${path.relative(root, inputPath)}`);
  if (!dryRun && !WELLNESS_PRODUCT_IMPORT_SERVICE_KEY) {
    usage("Set WELLNESS_PRODUCT_IMPORT_SERVICE_KEY, PRODUCT_LOOKUP_SERVICE_KEY, or Supabase service key for official Wellness product writes.");
  }
...
function normalizeText(value) {
  return asciiFold(value)
    .toLowerCase()
    .replace(/&amp;/g, "&")
    .replace(/grain[-\s]*free/g, "grain free")
    .replace(/[''™®]/g, " ")
    .replace(/[-_/,%+]+/g, " ")
    .replace(/[^a-z0-9\s&]/g, " ")
    .replace(/\bamp\b/g, " ")
...
function inferForm(sourceUrl, fallback = "") {
  ...
  if (/\b(wet|can|canned|pate|morsels|minced|sliced|shredded|shreds|flaked|stew|stews|mousse|gravy|gravies|pouch|sauce|entree|entrees|duos|purrfect|signature selects|healthy indulgence|tiny tasters|tasty textures|mini meals|petite entrees|protein bowls|complete health 95)\b/.test(text)) return "wet";
  if (/\b(dry|kibble|deboned|meal|grained|wholesome grains|grain free indoor|grain free salmon herring indoor|grain free large breed|grain free small breed|simple grain free|core digestive health|core grain free|core plus|simple limited ingredient|complete health plus)\b/.test(text)) return "dry";
  return "";
}

function isVarietyPack(target) {
  const text = normalizeText(`${target.name} ${target.cacheKey} ${target.sourceUrl}`);
  return /\bvariety\s+packs?\b/.test(text);
}
...
function modalBlock(html, id) {
  const raw = String(html || "");
  const start = raw.indexOf(`id="${id}"`);
  if (start < 0) return "";
  const nextButton = raw.indexOf("<button type=\"button\" data-bs-toggle=\"modal\"", start + id.length);
  const nextModal = raw.indexOf("<div class=\"modal fade\" id=", start + id.length);
  const endCandidates = [nextButton, nextModal].filter((index) => index > start);
  const end = endCandidates.length ? Math.min(...endCandidates) : start + 12000;
  return raw.slice(start, end);
}

function firstImageAlt(block) {
  return decodeHtml((String(block || "").match(/<img[^>]+alt="([\s\S]*?)"/i) || [])[1] || "");
}

function rawIngredientText(html) {
  const block = modalBlock(html, "ingredientsModal");
  return firstImageAlt(block);
}

function formulaLabels(text) {
  return text.match(/(?:^|\.\s+)[A-ZÀ-ÖØ-Ý][^:.]{3,130}\bRecipe(?:\s+(?:in|with|infused|for)\b[^:]{0,70})?:/gi) || [];
}

function parseIngredients(html) {
  const text = rawIngredientText(html);
  if (!text || /\b(guaranteed analysis|feeding guidelines|daily feeding|calorie content)\b/i.test(text)) return [];
  if (formulaLabels(text).length > 1) return [];
...
function parseNutrientPanel(html, sourceUrl) {
  ...
  const text = stripTags(modalBlock(html, "guaranteedAnalysisModal"));
  for (const row of text.matchAll(/([A-Za-z0-9*+\-\s]+?)\s+(?:(?:\((?:min|max)\.?\))|Min|Max|Not Less Than|Not More Than)\s+([\d.]+\s*%)/gi)) {
    const field = nutrientField(row[1]);
    const value = parsePercent(row[2].match(/([\d.]+)\s*%/)?.[1]);
...
function parseOfficialProductName(html) {
  ...
  return cleanProductName(product.name || stripTags(h1));
}
...
function parseImageUrl(html) {
  ...
  if (decoded.startsWith("/")) return `https://www.wellnesspetfood.com${decoded}`.slice(0, 500);
...
async function fetchText(url, attempt = 1) {
  ...
  if (response.status === 429 || response.status >= 500) {
    if (attempt >= 5) throw new Error(`Wellness official page fetch failed ${response.status}: ${url}`);
    const retryAfter = Number(response.headers.get("retry-after"));
    await sleep(Number.isFinite(retryAfter) ? retryAfter * 1000 : 1000 * attempt * attempt);
    return fetchText(url, attempt + 1);
  }
  if (!response.ok) throw new Error(`Wellness official page fetch failed ${response.status}: ${url}`);
...
async function fetchExistingCacheKeys() {
  ...
  const pageSize = 1000;
  while (true) {
    const url = `${base}?select=cache_key,product_name,brand&or=(brand.ilike.*Wellness*,product_name.ilike.*Wellness*,cache_key.ilike.*wellness*)&offset=${offset}&limit=${pageSize}`;
...
async function parseTarget(target) {
  if (isVarietyPack(target)) {
    return { target: null, reason: "variety_pack_multi_formula", sourceUrl: target.sourceUrl };
  }
  const html = await fetchText(target.sourceUrl);
  if (formulaLabels(rawIngredientText(html)).length > 1) {
    return { target: null, reason: "multi_formula_ingredient_text", sourceUrl: target.sourceUrl };
  }
...
function exportParsed(targets, metadata) {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      source: "wellness_official_product_data",
...
function loadVerifiedReportKeys() {
  ...
  for (const line of fs.readFileSync(reportPath, "utf8").split(/\r?\n/)) {
    ...
    try {
      ...
      if (entry?.event === "wellness_official_product_result" && entry?.status === "verified_saved" && entry?.cache_key) {
...
async function writeTarget(target) {
  ...
  const response = await fetch(`${supabaseBase()}/rest/v1/rpc/${rpc}`, {
    method: "POST",
    headers: restHeaders(WELLNESS_PRODUCT_IMPORT_SERVICE_KEY),
...
async function processWrites(targets) {
  ...
  async function worker() {
    while (nextIndex < selected.length) {
      ...
      appendReport({
        event: "wellness_official_product_result",
...
async function main() {
  ...
  for (const target of planned) {
    ...
    try {
      ...
      console.warn(`Skipped Wellness target: ${target.name} (${error?.message || error})`);
      continue;
    }
    if (!result.target) {
      skipped.parse += 1;
  ...
  console.log(`Wellness official input targets: ${inputTargets.length}`);
  console.log(`Existing Wellness keys skipped: ${skipped.existing}`);
