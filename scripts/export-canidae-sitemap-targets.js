...
const DEFAULT_OUTPUT = ".tmp/canidae-sitemap-catalog-targets.json";
...
const CORE_FOOD_TERMS = [
  ...
  "puppy food", "cat food", "dog food", "all life stages",
  "pure farm to bowl", "pure petite", "balanced bowl", "goodness",
  "under the sun", "grain free", "ancient grains", "wholesome grains",
  "minced", "morsels", "pate", "formula",
];
...
const SUPPLEMENTAL_FOOD_TERMS = [
  "topper", "toppers", "protein topper", "chunky stew toppers",
  "bone broth", "supplement", "supplements", "meal booster",
  "mix in", "stew toppers",
];

const NON_FOOD_TERMS = [
  "test product", "toy", "toys", "mat", "collar", "leash",
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:canidae-sitemap-catalog -- --output=.tmp/canidae-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit Canidae products sitemap URL (${DEFAULT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/canidae\.com\/xmlsitemap\.php\?type=products&page=1$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://canidae.com/xmlsitemap.php?type=products&page=1.");
  }
}
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .join("")
    .replace(/\bCanidae\b/g, "CANIDAE")
    .replace(/\b13oz\b/gi, "13 oz")
    .replace(/\b22oz\b/gi, "22 oz")
    .replace(/\s+([,])/g, "$1")
    .trim();
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Canidae sitemap fetch failed ${response.status}: ${url}`);
...
function inferCanidaePetTypes(row) {
  const text = normalizeText(`${row.name} ${row.sourceUrl} ${row.retailerProductId}`);
  if (hasTerm(text, "cat food") || hasTerm(text, "cats") || hasTerm(text, "cat wet")) return ["cat"];
  if (
    hasTerm(text, "dog food") ||
    hasTerm(text, "dogs") ||
    hasTerm(text, "puppy") ||
    hasTerm(text, "pure farm to bowl") ||
    hasTerm(text, "under the sun")
  ) {
    return ["dog"];
  }
...
function isCandidateFoodRow(row) {
  const text = normalizeText(`${row.name} ${row.sourceUrl} ${row.retailerProductId}`);
  if (inferCanidaePetTypes(row).length === 0) return false;
...
function normalizeTarget(row) {
  ...
  const petTypes = inferCanidaePetTypes(row);
...
async function main() {
  ...
  const manifest = {
    ...
    source: "canidae_product_sitemap",
    sitemapUrl,
    includeExisting,
    includeTreats,
    skipExistingScan,
  ...
  console.log(`Canidae products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Canidae sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Canidae sitemap catalog target export failed:", err.message);
  process.exit(1);
});
