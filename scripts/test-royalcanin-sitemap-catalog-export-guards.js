...
const source = fs.readFileSync(path.join(root, "scripts/export-royalcanin-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Royal Canin sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://www.royalcanin.com/us/en-us/sitemap/sitemap-products.xml") &&
    ...
    source.includes("retail-products") &&
    !source.includes("product-lookup") &&
    !source.includes("cheerio") &&
    !source.includes("puppeteer"),
  "exporter must read the official Royal Canin product sitemap, derive titles from retail-product slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"loaf in sauce\"") &&
    source.includes("\"thin slices in gravy\"") &&
    source.includes("\"dental care\"") &&
    source.includes("\"hairball care\"") &&
    ...
    source.includes("function inferRoyalCaninPetTypes(row)") &&
    source.includes("/\\/us\\/dogs\\/products\\/retail-products\\//i.test(row.sourceUrl)") &&
    source.includes("/\\/us\\/cats\\/products\\/retail-products\\//i.test(row.sourceUrl)") &&
    source.includes("function isCandidateFoodRow(row)") &&
    source.includes("if (!/\\/retail-products\\//i.test(row.sourceUrl)) return false") &&
  ...
  "exporter must strictly filter Royal Canin sitemap rows to retail dog/cat foods and reject supplements, treats, and non-food rows by default"
);
...
assert(
  source.includes("source: \"royalcanin_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"royalcanin_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:royalcanin-sitemap-catalog": "node scripts/export-royalcanin-sitemap-targets.js"') &&
    packageJson.includes('"test:royalcanin-sitemap-catalog-export": "node scripts/test-royalcanin-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:royalcanin-sitemap-catalog-export"),
  "package scripts must expose Royal Canin sitemap exporter and include its guard in test:guards"
);

console.log("Royal Canin sitemap catalog export guard passed");
