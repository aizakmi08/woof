...
const BRAND = "Canidae";
const DEFAULT_PRODUCTS_SITEMAP_URL = "https://canidae.com/xmlsitemap.php?type=products&page=1";
const DEFAULT_OUTPUT = ".tmp/nutrient-panel-canidae-targets.json";
...
const sitemapUrl = sitemapArg ? sitemapArg.split("=").slice(1).join("=").trim() : DEFAULT_PRODUCTS_SITEMAP_URL;
const limit = limitArg ? Number(limitArg.split("=")[1]) : 0;
const delayMs = delayArg ? Number(delayArg.split("=")[1]) : 150;

const REJECT_TERMS = [
  "broth", "bundle", "case of", "chew", "chews", "multipack", "snack",
  "snacks", "supplement", "supplements", "topper", "toppers", "treat",
  "treats", "variety pack",
];
const PROTEIN_TOKENS = new Set([
  "beef", "bison", "boar", "chicken", "duck", "fish", "lamb", "mackerel",
  "pork", "quail", "rabbit", "salmon", "sardine", "trout", "tuna", "turkey",
  "venison", "whitefish",
]);
const DISTINCTIVE_STOP_TOKENS = new Set([
  "adult", "ages", "all", "and", "breeds", "canidae", "cat", "cats", "dog",
  "dogs", "dry", "farm", "food", "foods", "for", "free", "grain",
  "ingredient", "ingredients", "life", "limited", "pate", "pet", "pets",
  "premium", "protein", "pure", "recipe", "recipes", "small", "stages",
  "the", "to", "wet", "with",
]);
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:canidae-nutrient-panels -- --output=.tmp/nutrient-panel-canidae-targets.json

Options:
  --output=path.json      Write a nutrient-panel backfill manifest (default ${DEFAULT_OUTPUT})
  --sitemap-url=url       Explicit Canidae product sitemap URL (${DEFAULT_PRODUCTS_SITEMAP_URL})
  --limit=N               Stop after N matched nutrient panels are collected (default 0, no limit)
  --delay-ms=N            Delay between page/JSON requests (default 150)
...
function assertConfig() {
  ...
  if (!/^https:\/\/canidae\.com\/xmlsitemap\.php\?type=products&page=1$/i.test(sitemapUrl)) {
    usage("--sitemap-url must be https://canidae.com/xmlsitemap.php?type=products&page=1.");
  }
...
async function fetchText(url, accept = "text/html,application/xhtml+xml;q=0.9,*/*;q=0.8") {
  ...
  if (!response.ok) throw new Error(`Canidae fetch failed ${response.status}: ${url}`);
  return response.text();
}

function productIdFromHtml(html) {
  const match = String(html || "").match(/window\.stencilBootstrap\("product",\s*"\\?\{\\?"productId\\?":(\d+)/);
...
function titleFromHtml(html, fallbackUrl) {
  ...
  for (const match of jsonLd) {
    try {
      ...
      if (parsed?.["@type"] === "Product" && parsed.name) return decodeHtml(parsed.name);
    } catch {
      // Keep looking for another JSON-LD block.
    }
  }
  const title = String(html || "").match(/<title>([\s\S]*?)<\/title>/i);
  if (title) return decodeHtml(title[1]).replace(/\s+\|\s+Canidae.*$/i, "");
  return decodeHtml(new URL(fallbackUrl).pathname.split("/").filter(Boolean).pop() || "").replace(/-/g, " ");
}
...
function inferProductFormFromText(value) {
  const text = normalizeText(value);
  const wet = /\b(wet|can|canned|pate|morsels|minced|broth)\b/.test(text);
...
function distinctiveTokens(value) {
  return normalizeText(value)
    .split(/\s+/)
    .filter((token) => token.length > 2 && !DISTINCTIVE_STOP_TOKENS.has(token) && !PROTEIN_TOKENS.has(token))
    .sort();
}
...
function candidateFromPage(sourceUrl, html) {
  ...
  if (REJECT_TERMS.some((term) => hasTerm(normalized, term))) return null;
  const petType = inferPetTypeFromText(text);
  const form = inferProductFormFromText(text);
  const proteins = proteinTokens(text);
  const distinctive = distinctiveTokens(title);
  if (!productId || !petType || !form || proteins.length === 0 || distinctive.length === 0) return null;
  return {
    sourceUrl,
    productId,
    productJsonUrl: `https://cdn11.bigcommerce.com/s-lnkn0tcvqc/content/pdp/products/${productId}.json`,
    productName: title,
    petType,
    form,
...
async function main() {
  ...
  for (const url of urls) {
    ...
    let candidate;
    try {
      candidate = candidateFromPage(url, await fetchText(url));
    } catch {
      skipped.nonFood++;
      continue;
    }
    ...
    const parsed = parseNutrientPanel(JSON.parse(await fetchText(candidate.productJsonUrl, "application/json,*/*;q=0.8")), candidate.sourceUrl);
    if (!parsed.panel) {
    ...
    const target = {
      ...
      petType: candidate.petType,
      source: "canidae_official_product_json",
      sourceUrl: candidate.sourceUrl,
  ...
  console.log(`Wrote ${targets.length} Canidae nutrient panel targets to ${path.relative(root, outputPath)}`);
