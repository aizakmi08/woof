...
const DEFAULT_SITEMAP_INDEX_URL = "https://www.meijer.com/sitemap.xml";
const DEFAULT_OUTPUT = ".tmp/meijer-sitemap-catalog-targets.json";
...
const NON_FOOD_TERMS = [
  ...
  "aquarium", "fish food", "catfish", "glofish", "bird", "parakeet",
  "reptile", "small animal", "hamster", "rabbit", "hay", "bedding",
  "hot dog", "hot dogs", "corn dog", "corn dogs", "chili dog",
  "pretzel dog", "mini dog", "meat stick", "chicken meal deal",
  "cat eye", "cat-eye", "dogfish", "dogwood", "cat6", "cat 6",
  "food storage", "can saver", "placemat", "placemats",
];

const BRAND_HINTS = [
  "9Lives", "ACANA", "Against the Grain", "Alpo", "Beneful", "Bil-Jac",
  "Blue Buffalo", "Canidae", "Cesar", "Diamond Naturals", "Diamond",
  "Eukanuba", "Fancy Feast", "Freshpet", "Friskies", "Fromm",
  "Hill's Prescription Diet", "Hill's Science Diet", "Iams", "Instinct",
  "Kit & Kaboodle", "Meijer", "Meow Mix", "Merrick", "Natural Balance",
  "Nature's Recipe", "Nulo", "Nutro", "Open Farm", "Orijen", "Pedigree",
  "Purina Alpo", "Purina Cat Chow", "Purina Dog Chow", "Purina ONE",
  "Purina Pro Plan", "Purina", "Rachael Ray Nutrish", "Royal Canin",
  "Sheba", "Solid Gold", "Stella & Chewy's", "Taste of the Wild",
  "Tiki Cat", "Tiki Dog", "Wellness", "Weruva", "Whiskas", "Zignature",
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:meijer-sitemap-catalog -- --output=.tmp/meijer-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap-index=url     Meijer sitemap index URL (${DEFAULT_SITEMAP_INDEX_URL})
  --sitemap=url           Explicit Meijer product sitemap URL
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.meijer\.com\/sitemap\.xml$/i.test(sitemapIndexUrl)) {
    usage("--sitemap-index must be https://www.meijer.com/sitemap.xml.");
  }
  if (explicitSitemapUrl && !/^https:\/\/www\.meijer\.com\/content\/dam\/meijer\/applications\/sitemap\/products\/product\.\d+\.sitemap\.xml$/i.test(explicitSitemapUrl)) {
    usage("--sitemap must be a Meijer product sitemap URL.");
  }
}
...
function normalizeCacheKey(text) {
  return String(text || "")
    ...
    .replace(/[-/&+]/g, " ")
    .replace(/[^a-z0-9\s]/g, "")
    .replace(/\b\d+(\s+)?(lb|lbs|pound|pounds|oz|ounce|ounces|kg|g|gram|grams|can|cans|bag|bags|case|pack|ct|count|tray|trays|pouch|pouches|box|boxes)\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}
...
function cleanUrl(value) {
  ...
  if (parsed.protocol !== "https:" || parsed.hostname !== "www.meijer.com") return "";
  if (!/^\/shopping\/product\/[^/?#]+\/\d+\.html$/i.test(parsed.pathname)) return "";
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Meijer sitemap fetch failed ${response.status}: ${url}`);
...
function extractProductSitemapLocs(xml) {
  return [...String(xml || "").matchAll(/<loc>([\s\S]*?)<\/loc>/gi)]
    ...
    .filter((url) => /^https:\/\/www\.meijer\.com\/content\/dam\/meijer\/applications\/sitemap\/products\/product\.\d+\.sitemap\.xml$/i.test(url))
...
async function resolveSitemapUrls() {
  ...
  if (urls.length === 0) throw new Error("Meijer sitemap index did not expose product sitemap URLs.");
...
async function main() {
  ...
  console.log(`Meijer products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Meijer sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Meijer sitemap catalog target export failed:", err.message);
  process.exit(1);
});
