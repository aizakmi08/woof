...
const DEFAULT_SITEMAP_INDEX_URL = "https://www.heartypet.com/sitemap.xml";
const DEFAULT_OUTPUT = ".tmp/heartypet-sitemap-catalog-targets.json";
...
const TREAT_TERMS = [
  ...
  "sticks", "bakery", "oven-baked", "oven baked",
];
...
const SUPPLEMENTAL_FOOD_TERMS = [
  ...
  "itch", "skin and coat", "omega mender", "happy traveler",
];
...
const NON_FOOD_TERMS = [
  ...
  "costume", "aquarium", "fish food", "bird", "avian", "parakeet",
...
const BRAND_HINTS = [
  ...
  "Blue Buffalo", "Canidae", "Chicken Soup for the Soul", "Dave's Pet Food",
  "Dave's", "Diamond Naturals", "Earthborn Holistic", "Evanger's",
  "Farmina", "FirstMate", "Fromm", "Fussie Cat", "Go! Solutions",
  "Health Extension", "Holistic Select", "Instinct", "Lotus",
  "Merrick", "Natural Balance", "Nature's Logic", "Nulo", "NutriSource",
  "Open Farm", "Orijen", "Primal", "Redbarn", "Royal Canin",
  "Solid Gold", "Stella & Chewy's", "Taste of the Wild",
  "The Honest Kitchen", "Tiki Cat", "Tiki Dog", "Victor", "Wellness",
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:heartypet-sitemap-catalog -- --output=.tmp/heartypet-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap-index=url     Heartypet sitemap index URL (${DEFAULT_SITEMAP_INDEX_URL})
  --sitemap=url           Explicit Heartypet product sitemap URL
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.heartypet\.com\/sitemap\.xml$/i.test(sitemapIndexUrl)) {
    usage("--sitemap-index must be https://www.heartypet.com/sitemap.xml.");
  }
  if (explicitSitemapUrl && !/^https:\/\/www\.heartypet\.com\/sitemap_products_\d+\.xml\?from=\d+&to=\d+$/i.test(explicitSitemapUrl)) {
    usage("--sitemap must be a Heartypet products sitemap URL.");
  }
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/www\.heartypet\.com\/products\/[a-z0-9-]+$/i.test(url)) return "";
  return url;
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Heartypet sitemap fetch failed ${response.status}: ${url}`);
...
function extractProductSitemapLocs(xml) {
  return [...String(xml || "").matchAll(/<loc>([\s\S]*?)<\/loc>/gi)]
    ...
    .filter((url) => /^https:\/\/www\.heartypet\.com\/sitemap_products_\d+\.xml\?from=\d+&to=\d+$/i.test(url))
...
async function resolveSitemapUrls() {
  ...
  if (urls.length === 0) throw new Error("No Heartypet product sitemaps found.");
...
function normalizeTarget(row) {
  ...
  return {
    ...
    cacheKey,
    source: "heartypet_sitemap_export",
...
async function main() {
  ...
  const manifest = {
    ...
    source: "heartypet_product_sitemap",
    sitemapIndexUrl,
    sitemapUrls,
    explicitSitemapUrl,
    includeExisting,
  ...
  console.log(`Heartypet products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Heartypet sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Heartypet sitemap catalog target export failed:", err.message);
  process.exit(1);
});
