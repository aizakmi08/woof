...
const ORIJEN_PRODUCT_IMPORT_SERVICE_KEY =
  process.env.ORIJEN_PRODUCT_IMPORT_SERVICE_KEY ||
  process.env.PRODUCT_LOOKUP_SERVICE_KEY ||
  process.env.SUPABASE_SERVICE_ROLE_KEY ||
  process.env.SUPABASE_SERVICE_KEY ||
  "";
const DEFAULT_INPUT = ".tmp/orijen-sitemap-catalog-targets.json";
const DEFAULT_EXPORT_PARSED = ".tmp/orijen-official-product-data.json";
const DEFAULT_REPORT = ".tmp/orijen-official-product-backfill-report.jsonl";
const USER_AGENT = "Mozilla/5.0 Woof official ORIJEN product import";
const SOURCE_BRAND = "ORIJEN";
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run backfill:orijen-official-products -- --dry-run
  ORIJEN_PRODUCT_IMPORT_SERVICE_KEY=... npm run backfill:orijen-official-products

Options:
  --input=path.json           ORIJEN catalog target manifest (default ${DEFAULT_INPUT})
...
function assertConfig() {
  ...
  if (!fs.existsSync(inputPath)) usage(`Missing ORIJEN input manifest: ${path.relative(root, inputPath)}`);
  if (!dryRun && !ORIJEN_PRODUCT_IMPORT_SERVICE_KEY) {
    usage("Set ORIJEN_PRODUCT_IMPORT_SERVICE_KEY, PRODUCT_LOOKUP_SERVICE_KEY, or Supabase service key for official ORIJEN product writes.");
  }
...
function parseImageUrl(html) {
  ...
  if (decoded.startsWith("/")) return `https://www.orijenpetfoods.com${decoded}`.slice(0, 500);
...
function slugSuffix(sourceUrl) {
  try {
    ...
    return normalizeText(slug.replace(/-/g, " "))
      .replace(/\b(orijen|dog|cat|food|formula|recipe)\b/g, " ")
      .replace(/\s+/g, " ")
      .trim()
      .split(/\s+/)
      .slice(0, 8)
...
async function fetchText(url, accept = "text/html,application/xhtml+xml;q=0.9,*/*;q=0.8", attempt = 1) {
  ...
  if (response.status === 403 || response.status === 429 || response.status >= 500) {
    if (attempt >= 7) throw new Error(`ORIJEN official fetch failed ${response.status}: ${url}`);
  ...
  if (!response.ok) throw new Error(`ORIJEN official fetch failed ${response.status}: ${url}`);
...
async function fetchExistingCacheKeys() {
  ...
  const pageSize = 1000;
  while (true) {
    const url = `${base}?select=cache_key,product_name,brand&or=(brand.ilike.*ORIJEN*,product_name.ilike.*ORIJEN*,cache_key.ilike.*orijen*)&offset=${offset}&limit=${pageSize}`;
...
function exportParsed(targets, metadata) {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      source: "orijen_official_product_data",
...
function loadVerifiedReportKeys() {
  ...
  for (const line of fs.readFileSync(reportPath, "utf8").split(/\r?\n/)) {
    ...
    try {
      ...
      if (entry?.event === "orijen_official_product_result" && entry?.status === "verified_saved" && entry?.cache_key) {
...
async function writeTarget(target) {
  ...
  const response = await fetch(`${supabaseBase()}/rest/v1/rpc/${rpc}`, {
    method: "POST",
    headers: restHeaders(ORIJEN_PRODUCT_IMPORT_SERVICE_KEY),
...
async function processWrites(targets) {
  ...
  async function worker() {
    while (nextIndex < selected.length) {
      ...
      appendReport({
        event: "orijen_official_product_result",
...
async function main() {
  ...
  for (const target of planned) {
    ...
    try {
      ...
      console.warn(`Skipped ORIJEN target: ${target.brand} ${target.name} (${error?.message || error})`);
      continue;
    }
    if (!result.target) {
      skipped.parse += 1;
  ...
  console.log(`ORIJEN official input targets: ${inputTargets.length}`);
  console.log(`Existing ORIJEN keys skipped: ${skipped.existing}`);
