...
const source = fs.readFileSync(path.join(root, "scripts/export-stellaandchewys-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Stella & Chewy's sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  ...
  "exporter must discover Stella & Chewy's Shopify product sitemaps, derive titles from sitemap image titles, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"frozen raw dinner patties\"") &&
    source.includes("\"freeze-dried raw dinner morsels\"") &&
    source.includes("\"raw coated kibble\"") &&
    source.includes("\"carnivore cravings\"") &&
    source.includes("\"shredrs\"") &&
    source.includes("TREAT_TERMS") &&
    source.includes("\"wild weenies\"") &&
    source.includes("\"carnivore crunch\"") &&
    source.includes("SUPPLEMENTAL_FOOD_TERMS") &&
    source.includes("\"meal mixers\"") &&
    source.includes("\"dinner dust\"") &&
    source.includes("\"bone broth\"") &&
    source.includes("\"solutions\"") &&
  ...
  "exporter must strictly filter Stella & Chewy's sitemap rows to complete foods and reject treats, toppers, supplements, broths, and non-food rows by default"
);
...
assert(
  source.includes("source: \"stellaandchewys_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"stellaandchewys_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:stellaandchewys-sitemap-catalog": "node scripts/export-stellaandchewys-sitemap-targets.js"') &&
    packageJson.includes('"test:stellaandchewys-sitemap-catalog-export": "node scripts/test-stellaandchewys-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:stellaandchewys-sitemap-catalog-export"),
  "package scripts must expose Stella & Chewy's sitemap exporter and include its guard in test:guards"
);

console.log("Stella & Chewy's sitemap catalog export guard passed");
