...
const source = fs.readFileSync(path.join(root, "scripts/export-petfoodexpress-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Pet Food Express sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://www.petfood.express/xmlsitemap.php") &&
    source.includes("type=products&page=") &&
  ...
  "exporter must discover BigCommerce product sitemaps, derive titles from slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"Pet Food Express\"") === false &&
    source.includes("\"JustFoodForDogs\"") &&
    source.includes("\"Fussie Cat\"") &&
    source.includes("\"Open Farm\"") &&
    source.includes("BRAND_PREFIX_ALIASES") &&
    source.includes("[\"b-f-f\", \"Weruva\"]") &&
  ...
  "exporter must strictly filter Pet Food Express's mixed sitemap to complete dog/cat food-like rows and reject supplement/topper/treat rows by default"
);
...
assert(
  source.includes("source: \"petfoodexpress_sitemap_export\"") &&
    source.includes("sourceQuality: \"retailer_sitemap_slug\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"petfoodexpress_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:petfoodexpress-sitemap-catalog": "node scripts/export-petfoodexpress-sitemap-targets.js"') &&
    packageJson.includes('"test:petfoodexpress-sitemap-catalog-export": "node scripts/test-petfoodexpress-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:petfoodexpress-sitemap-catalog-export"),
  "package scripts must expose Pet Food Express sitemap exporter and include its guard in test:guards"
);

console.log("Pet Food Express sitemap catalog export guard passed");
