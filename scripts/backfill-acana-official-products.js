...
const ACANA_PRODUCT_IMPORT_SERVICE_KEY =
  process.env.ACANA_PRODUCT_IMPORT_SERVICE_KEY ||
  process.env.PRODUCT_LOOKUP_SERVICE_KEY ||
  process.env.SUPABASE_SERVICE_ROLE_KEY ||
  process.env.SUPABASE_SERVICE_KEY ||
  "";
const DEFAULT_INPUT = ".tmp/acana-sitemap-catalog-targets.json";
const DEFAULT_EXPORT_PARSED = ".tmp/acana-official-product-data.json";
const DEFAULT_REPORT = ".tmp/acana-official-product-backfill-report.jsonl";
const USER_AGENT = "Mozilla/5.0 Woof official ACANA product import";
const SOURCE_BRAND = "ACANA";
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run backfill:acana-official-products -- --dry-run
  ACANA_PRODUCT_IMPORT_SERVICE_KEY=... npm run backfill:acana-official-products

Options:
  --input=path.json           ACANA catalog target manifest (default ${DEFAULT_INPUT})
...
function assertConfig() {
  ...
  if (!fs.existsSync(inputPath)) usage(`Missing ACANA input manifest: ${path.relative(root, inputPath)}`);
  if (!dryRun && !ACANA_PRODUCT_IMPORT_SERVICE_KEY) {
    usage("Set ACANA_PRODUCT_IMPORT_SERVICE_KEY, PRODUCT_LOOKUP_SERVICE_KEY, or Supabase service key for official ACANA product writes.");
  }
...
function cleanUrl(value) {
  try {
    ...
    if (!/^https:\/\/www\.acana\.com\/en-US\/(dogs|cats)\/(dog-food|cat-food)\/[^?#]+\/[a-z0-9-]+\.html$/i.test(url)) return "";
...
function parseImageUrl(html) {
  ...
  if (decoded.startsWith("/")) return `https://www.acana.com${decoded}`.slice(0, 500);
  return /^https:\/\//i.test(decoded) ? decoded.slice(0, 500) : "";
}

function ingredientsBlock(html) {
  return String(html || "").match(/<div[^>]+class=["'][^"']*\bingredients-list\b[^"']*["'][^>]*>([\s\S]*?)<\/div>\s*<div[^>]+class=["'][^"']*\banalysis\b/i)?.[1] || "";
}

function parseIngredients(html) {
  const block = ingredientsBlock(html);
  const firstParagraph = block.match(/<p[^>]*>([\s\S]*?)<\/p>/i)?.[1] || "";
  const text = stripTags(firstParagraph)
    .replace(/\s+(METABOLIZABLE ENERGY|ME Calculated|AAFCO|FEEDING INSTRUCTIONS).*$/i, "")
    .trim();
...
function parseNutrientPanel(html, sourceUrl) {
  ...
  const text = stripTags(ingredientsBlock(raw) || raw);
  let match = text.match(/(?:METABOLIZABLE ENERGY|ME Calculated)\D{0,80}([\d,]+(?:\.\d+)?)\s*kcal\s*\/\s*kg/i);
  if (match) panel.calories_per_kg = Number(match[1].replace(/,/g, ""));
  match = text.match(/([\d,]+(?:\.\d+)?)\s*kcal\s*(?:per|\/)\s*(?:8\s*fl\.?\s*oz\.?|8\s*oz\.?|cup)/i);
...
function slugSuffix(sourceUrl) {
  try {
    ...
    return normalizeText(slug.replace(/-/g, " "))
      .replace(/\b(acana|dog|cat|food|formula|recipe)\b/g, " ")
      .replace(/\s+/g, " ")
      .trim()
      .split(/\s+/)
      .slice(0, 8)
...
async function fetchText(url, accept = "text/html,application/xhtml+xml;q=0.9,*/*;q=0.8", attempt = 1) {
  ...
  if (response.status === 403 || response.status === 429 || response.status >= 500) {
    if (attempt >= 7) throw new Error(`ACANA official fetch failed ${response.status}: ${url}`);
  ...
  if (!response.ok) throw new Error(`ACANA official fetch failed ${response.status}: ${url}`);
...
async function fetchExistingCacheKeys() {
  ...
  const pageSize = 1000;
  while (true) {
    const url = `${base}?select=cache_key,product_name,brand&or=(brand.ilike.*ACANA*,product_name.ilike.*ACANA*,cache_key.ilike.*acana*)&offset=${offset}&limit=${pageSize}`;
...
function exportParsed(targets, metadata) {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      source: "acana_official_product_data",
...
function loadVerifiedReportKeys() {
  ...
  for (const line of fs.readFileSync(reportPath, "utf8").split(/\r?\n/)) {
    ...
    try {
      ...
      if (entry?.event === "acana_official_product_result" && entry?.status === "verified_saved" && entry?.cache_key) {
...
async function writeTarget(target) {
  ...
  const response = await fetch(`${supabaseBase()}/rest/v1/rpc/${rpc}`, {
    method: "POST",
    headers: restHeaders(ACANA_PRODUCT_IMPORT_SERVICE_KEY),
...
async function processWrites(targets) {
  ...
  async function worker() {
    while (nextIndex < selected.length) {
      ...
      appendReport({
        event: "acana_official_product_result",
...
async function main() {
  ...
  for (const target of planned) {
    ...
    try {
      ...
      console.warn(`Skipped ACANA target: ${target.brand} ${target.name} (${error?.message || error})`);
      continue;
    }
    if (!result.target) {
      skipped.parse += 1;
  ...
  console.log(`ACANA official input targets: ${inputTargets.length}`);
  console.log(`Existing ACANA keys skipped: ${skipped.existing}`);
