...
const DEFAULT_OUTPUT = ".tmp/royalcanin-sitemap-catalog-targets.json";
...
const CORE_FOOD_TERMS = [
  "adult", "puppy", "kitten", "mother babycat", "mother babydog",
  "loaf in sauce", "thin slices in gravy", "morsels in gravy",
  "ultra soft mousse", "canned in gel", "digestive care", "weight care",
  "urinary care", "dental care", "hairball care", "hair skin care",
  "sensitive digestion", "sensitive skin care", "appetite control care",
  "fit and active", "indoor", "large", "medium", "small", "x small",
  "giant", "starter", "instinctive", "exigent", "spayed neutered",
];

const TREAT_TERMS = [
  "treat", "treats", "chew", "chews", "biscuit", "biscuits",
];

const SUPPLEMENTAL_FOOD_TERMS = [
  "probiotics", "supplement", "supplements", "topper", "toppers",
  "meal booster", "vitamin", "vitamins",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:royalcanin-sitemap-catalog -- --output=.tmp/royalcanin-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit Royal Canin product sitemap URL (${DEFAULT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.royalcanin\.com\/us\/en-us\/sitemap\/sitemap-products\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://www.royalcanin.com/us/en-us/sitemap/sitemap-products.xml.");
  }
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Royal Canin sitemap fetch failed ${response.status}: ${url}`);
...
function inferRoyalCaninPetTypes(row) {
  if (/\/us\/dogs\/products\/retail-products\//i.test(row.sourceUrl)) return ["dog"];
  if (/\/us\/cats\/products\/retail-products\//i.test(row.sourceUrl)) return ["cat"];
  return inferPetTypes({
    brand: BRAND,
    product_name: `${row.name} ${row.sourceUrl}`,
    cache_key: normalizeCacheKey(`${row.name} ${row.sourceUrl}`),
    source: "royalcanin_sitemap",
  }, { includeAmbiguous: false });
}

function isCandidateFoodRow(row) {
  const text = normalizeText(`${row.name} ${row.sourceUrl} ${row.retailerProductId}`);
  if (!/\/retail-products\//i.test(row.sourceUrl)) return false;
  if (inferRoyalCaninPetTypes(row).length === 0) return false;
...
function normalizeTarget(row) {
  ...
  const petTypes = inferRoyalCaninPetTypes(row);
  ...
  return {
    ...
    cacheKey,
    source: "royalcanin_sitemap_export",
...
async function main() {
  ...
  const manifest = {
    ...
    source: "royalcanin_product_sitemap",
    sitemapUrl,
    includeExisting,
    includeTreats,
    skipExistingScan,
  ...
  console.log(`Royal Canin products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Royal Canin sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Royal Canin sitemap catalog target export failed:", err.message);
  process.exit(1);
});
