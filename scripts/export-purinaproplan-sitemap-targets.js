...
const DEFAULT_OUTPUT = ".tmp/purinaproplan-sitemap-catalog-targets.json";
const REST_PAGE_SIZE = 1000;
const USER_AGENT = "Mozilla/5.0 Woof catalog coverage export";
const BRAND = "Purina Pro Plan";
...
const CORE_FOOD_TERMS = [
  ...
  "complete essentials", "specialized nutrition", "specialized",
  "sensitive skin stomach", "sport", "development", "performance",
  "shredded blend", "grain free", "large breed", "small breed",
  "toy breed", "small bites", "puppy", "kitten", "senior", "adult",
  "adult seven plus", "indoor", "hairball", "urinary", "urinary tract",
  "weight management", "healthy metabolism", "prime plus", "bright mind",
  "live clear", "liveclear", "all life stages", "high protein",
  "veterinary diets", "gastroenteric", "dietetic management", "st ox",
  "advantedge", "digestive support", "senior support", "savory selects",
  "ultra soft mousse", "variety pack", "pate",
  "gravy", "chicken", "turkey", "salmon", "tuna", "beef", "lamb",
  "venison", "duck", "bison", "whitefish", "ocean fish", "trout",
  "liver", "cod", "shrimp", "sole", "barley", "brown rice", "rice",
  "oat meal", "oatmeal", "sweet potato",
];
const REJECT_TERMS = [
  ...
  "fortiflora", "calming care", "hydracare", "joint care",
  "skin care soft chews", "soft chews", "chewable tablet",
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:purinaproplan-sitemap-catalog -- --output=.tmp/purinaproplan-sitemap-catalog-targets.json
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .join("")
    .replace(/\bPurina Pro Plan\b/g, "Purina Pro Plan")
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/www\.purina\.com\/(cats|dogs)\/shop\/pro-plan-[a-z0-9-]+$/i.test(url)) return "";
  return url;
}
...
function formFromSlug(slug, petType) {
  ...
  if (petType === "dog" && /-dry-dog(?:-|$)/.test(slug)) return "dry";
  if (/-dry$/.test(slug)) return "dry";
  return "";
}
...
function parsedProductPath(url) {
  try {
    ...
    const form = formFromSlug(slug, petType);
    if (!/^pro-plan-/.test(slug) || !form) return null;
...
function inferPurinaProPlanPetTypes(row) {
...
function isCandidateFoodRow(row) {
  ...
  const petTypes = inferPurinaProPlanPetTypes(row);
  if (!/^https:\/\/www\.purina\.com\/(cats|dogs)\/shop\/pro-plan-/i.test(row.sourceUrl)) return false;
  if (!formFromSlug(row.retailerProductId, row.petType)) return false;
...
function normalizeTarget(row) {
  ...
  return {
    ...
    cacheKey,
    source: "purinaproplan_sitemap_export",
...
async function main() {
  ...
  const manifest = {
    ...
    source: "purinaproplan_product_sitemap",
    sitemapUrl,
    includeExisting,
    skipExistingScan,
    scannedProducts,
  ...
  console.log(`Purina Pro Plan products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Purina Pro Plan sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Purina Pro Plan sitemap catalog target export failed:", err.message);
  process.exit(1);
});
