...
const source = fs.readFileSync(path.join(root, "scripts/export-earthborn-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Earthborn sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  ...
  "exporter must read the official Earthborn product sitemap and avoid product-page scraping or product-lookup calls"
);
...
assert(
  ...
  "exporter must keep only official US complete dog/cat food rows and reject treats, international rows, supplements, and non-food rows"
);

assert(
  source.includes("source: \"earthborn_sitemap_export\"") &&
    ...
    source.includes("form: formFromPath(row)") &&
    source.includes("productType: `${row.line}_food`") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"earthborn_sitemap\"") &&
...
assert(
  packageJson.includes('"export:earthborn-sitemap-catalog": "node scripts/export-earthborn-sitemap-targets.js"') &&
    packageJson.includes('"test:earthborn-sitemap-catalog-export": "node scripts/test-earthborn-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:earthborn-sitemap-catalog-export"),
  "package scripts must expose Earthborn sitemap exporter and include its guard in test:guards"
);

console.log("Earthborn sitemap catalog export guard passed");
