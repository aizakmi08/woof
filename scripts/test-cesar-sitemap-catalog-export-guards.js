...
const source = fs.readFileSync(path.join(root, "scripts/export-cesar-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Cesar sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://www.cesar.com/sitemap.xml") &&
    source.includes("const BRAND = \"Cesar\"") &&
    source.includes("--sitemap=url") &&
    source.includes("--include-existing") &&
    source.includes("--include-treats") &&
    source.includes("--include-toppers") &&
    ...
    source.includes("www\\.cesar\\.com\\/products\\/(wet|dry|treats)") &&
    !source.includes("product-lookup") &&
    !source.includes("cheerio") &&
    !source.includes("puppeteer"),
  "exporter must read the official Cesar sitemap, derive titles from product slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"classic loaf\"") &&
    source.includes("\"filets gravy\"") &&
    source.includes("\"home delights\"") &&
    source.includes("\"wholesome bowls\"") &&
    source.includes("\"warm bowls\"") &&
    source.includes("TREAT_TERMS") &&
    source.includes("\"softies\"") &&
    source.includes("\"chewy\"") &&
    source.includes("SUPPLEMENTAL_FOOD_TERMS") &&
    source.includes("\"topper\"") &&
    source.includes("\"simply crafted\"") &&
    source.includes("NON_FOOD_TERMS") &&
    source.includes("\"litter\"") &&
    source.includes("function inferCesarPetTypes(row)") &&
    source.includes("return [\"dog\"]") &&
    source.includes("function parsedProductPath(url)") &&
    source.includes("if (!DEFAULT_PRODUCT_FORMS.has(row.form) && !(includeTreats && row.form === \"treats\")) return false") &&
    source.includes("if (row.form === \"treats\" && !includeTreats) return false") &&
    ...
    source.includes("if (!/^https:\\/\\/www\\.cesar\\.com\\/products\\/(wet|dry|treats)\\//i.test(row.sourceUrl)) return false") &&
    source.includes("if (!includeToppers && SUPPLEMENTAL_FOOD_TERMS.some((term) => hasTerm(text, term))) return false") &&
    source.includes("if (!includeTreats && TREAT_TERMS.some((term) => hasTerm(text, term))) return false"),
  "exporter must strictly filter Cesar sitemap rows to explicit wet/dry dog foods while rejecting treats, toppers, complements, supplements, and non-food rows by default"
);
...
assert(
  source.includes("source: \"cesar_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"cesar_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:cesar-sitemap-catalog": "node scripts/export-cesar-sitemap-targets.js"') &&
    packageJson.includes('"test:cesar-sitemap-catalog-export": "node scripts/test-cesar-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:cesar-sitemap-catalog-export"),
  "package scripts must expose Cesar sitemap exporter and include its guard in test:guards"
);

console.log("Cesar sitemap catalog export guard passed");
