...
const DEFAULT_SITEMAP_URL = "https://www.diamondpet.com/page-sitemap1.xml";
const DEFAULT_OUTPUT = ".tmp/diamond-sitemap-catalog-targets.json";
...
const LINE_LABELS = new Map([
  ...
  ["diamond-naturals-grain-free", "Diamond Naturals Grain-Free"],
  ["diamond-care", "Diamond Care"],
  ["diamond-pro89", "Diamond Pro89"],
]);

const REJECT_TERMS = [
  "formula quiz", "quiz results", "where to buy", "blog", "article",
  "faqs", "about", "privacy", "contest", "category", "photo",
  "treat", "treats", "snack", "snacks", "chew", "chews", "biscuit",
  "supplement", "supplements", "topper", "toppers", "broth", "toy",
  "toys", "apparel", "litter", "accessory", "accessories",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:diamond-sitemap-catalog -- --output=.tmp/diamond-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit Diamond page sitemap URL (${DEFAULT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.diamondpet\.com\/page-sitemap1\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://www.diamondpet.com/page-sitemap1.xml.");
  }
}
...
function titleCaseProductName(value) {
  const smallWords = new Set(["and", "or", "of", "for", "in", "with", "to", "the", "all"]);
  return String(value || "")
    .split(/(\s+)/)
    .map((part, index) => {
    ...
    .join("")
    .replace(/\bPro89\b/g, "Pro89")
    .replace(/\bSkin Coat\b/g, "Skin & Coat")
    .trim();
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/www\.diamondpet\.com\/(?:dog|cat)\/[a-z0-9-]+\/[a-z0-9-]+$/i.test(url)) return "";
  return url;
}
...
function parseProductUrl(url) {
  try {
    ...
    const speciesPath = parts[0];
    const line = parts[1];
    const slug = parts[2];
    const petType = speciesPath === "dog" ? "dog" : speciesPath === "cat" ? "cat" : "";
    if (!petType || !LINE_LABELS.has(line) || !slug) return null;
    return { petType, line, slug };
  } catch {
    return null;
  }
}
...
function formFromSlug(slug) {
  ...
  if (hasTerm(text, "dinner")) return "wet";
  return "dry";
}

function nameFromPath(row) {
  const line = LINE_LABELS.get(row.line) || BRAND;
  const baseName = cleanProductName(row.slug)
    .replace(/\bDog\b/g, "Dog")
    .replace(/\bCat\b/g, "Cat");
  return baseName.toLowerCase().startsWith(line.toLowerCase())
    ? baseName
    : `${line} ${baseName}`;
}
...
function addSpeciesQualifier(name, petType) {
  ...
  if (petType === "cat" && !/\b(cat|cats|kitten|kittens|feline)\b/.test(text)) return `${name} Cat Food`;
  if (petType === "dog" && !/\b(dog|dogs|puppy|puppies|canine)\b/.test(text)) return `${name} Dog Food`;
  return name;
}
...
function inferDiamondPetTypes(target) {
...
function normalizeTarget(url) {
  ...
  const name = addSpeciesQualifier(nameFromPath(row), row.petType);
  const draft = { ...row, name, sourceUrl: url };
  const petTypes = inferDiamondPetTypes(draft);
  ...
  return {
    ...
    cacheKey,
    source: "diamond_sitemap_export",
    sourceQuality: "official_brand_sitemap_slug",
    sourceUrl: url,
    imageUrl: "",
    barcode: "",
    retailerProductId: row.slug,
    form: formFromSlug(row.slug),
    productType: `${row.line}_${formFromSlug(row.slug)}_food`,
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Diamond sitemap fetch failed ${response.status}: ${url}`);
...
async function main() {
  ...
  console.log(`Diamond sitemap URLs scanned: ${rawUrls.length}`);
  console.log(`Candidate Diamond product URLs: ${urls.length}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Diamond sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Diamond sitemap catalog target export failed:", err.message);
  process.exit(1);
});
