...
const TASTEOFTHEWILD_PRODUCT_IMPORT_SERVICE_KEY =
  process.env.TASTEOFTHEWILD_PRODUCT_IMPORT_SERVICE_KEY ||
  process.env.PRODUCT_LOOKUP_SERVICE_KEY ||
  process.env.SUPABASE_SERVICE_ROLE_KEY ||
  process.env.SUPABASE_SERVICE_KEY ||
  "";
const DEFAULT_INPUT = ".tmp/tasteofthewild-sitemap-catalog-targets.json";
const DEFAULT_EXPORT_PARSED = ".tmp/tasteofthewild-official-product-data.json";
const DEFAULT_REPORT = ".tmp/tasteofthewild-official-product-backfill-report.jsonl";
const USER_AGENT = "Mozilla/5.0 Woof official Taste of the Wild product import";
const SOURCE_BRAND = "Taste of the Wild";
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run backfill:tasteofthewild-official-products -- --dry-run
  TASTEOFTHEWILD_PRODUCT_IMPORT_SERVICE_KEY=... npm run backfill:tasteofthewild-official-products

Options:
  --input=path.json           Taste of the Wild catalog target manifest (default ${DEFAULT_INPUT})
...
function assertConfig() {
  ...
  if (!fs.existsSync(inputPath)) usage(`Missing Taste of the Wild input manifest: ${path.relative(root, inputPath)}`);
  if (!dryRun && !TASTEOFTHEWILD_PRODUCT_IMPORT_SERVICE_KEY) {
    usage("Set TASTEOFTHEWILD_PRODUCT_IMPORT_SERVICE_KEY, PRODUCT_LOOKUP_SERVICE_KEY, or Supabase service key for official Taste of the Wild product writes.");
  }
...
function cleanProductName(value) {
  return decodeHtml(value)
    .replace(/\bTaste\s+of\s+the\s+Wild\b/gi, "")
    .replace(/\u2122|\u00AE|\u00A9/g, "")
    .replace(/\s+\|\s+Taste\s+of\s+the\s+Wild.*$/i, "")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 220);
}
...
function inferForm(sourceUrl, fallback = "") {
  ...
  if (/\b(freeze dried|medallions|patties)\b/.test(text)) return "fresh";
  return "dry";
}
...
function parseImageUrl(html) {
  ...
  if (decoded.startsWith("/")) return `https://www.tasteofthewildpetfood.com${decoded}`.slice(0, 500);
  return /^https:\/\//i.test(decoded) ? decoded.slice(0, 500) : "";
}

function ingredientsBlock(html) {
  return String(html || "").match(/<div[^>]+id=["']collapseIngredients["'][^>]*>([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<div[^>]+class=["'][^"']*\btab-pane\b/i)?.[1] || "";
}
...
function parseIngredients(html) {
  ...
  const ingredients = [];
  const seen = new Set();
  for (const match of block.matchAll(/<a[^>]+id=["']pill-tab-igred-all-[^"']+["'][^>]*>([\s\S]*?)<\/a>/gi)) {
    const ingredient = cleanIngredient(stripTags(match[1]));
  ...
  const text = stripTags(block)
    .replace(/^.*?\bAll Ingredients\b/i, "")
    .replace(/\s+(Guaranteed Analysis|Calorie Content|AAFCO|FEEDING INSTRUCTIONS).*$/i, "")
    .trim();
...
function parseNutrientPanel(html, sourceUrl) {
  ...
  const analysisBlock = text.slice(index, end > index ? end : index + 4000);
  ...
  let match = text.match(/Calorie Content\D{0,140}?([\d,]+(?:\.\d+)?)\s*kcal\s*\/?\s*kg/i);
  if (match) panel.calories_per_kg = Number(match[1].replace(/,/g, ""));
  match = text.match(/Calorie Content[\s\S]{0,160}?\(([\d,]+(?:\.\d+)?)\s*kcal\s*\/?\s*cup\)/i);
...
function slugSuffix(sourceUrl) {
  try {
    ...
    return normalizeText(slug.replace(/-/g, " "))
      .replace(/\b(tasteofthewild|taste|wild|dog|cat|food|formula|recipe)\b/g, " ")
      .replace(/\s+/g, " ")
      .trim()
      .split(/\s+/)
      .slice(0, 8)
...
async function fetchText(url, accept = "text/html,application/xhtml+xml;q=0.9,*/*;q=0.8", attempt = 1) {
  ...
  if (response.status === 403 || response.status === 429 || response.status >= 500) {
    if (attempt >= 7) throw new Error(`Taste of the Wild official fetch failed ${response.status}: ${url}`);
  ...
  if (!response.ok) throw new Error(`Taste of the Wild official fetch failed ${response.status}: ${url}`);
...
async function fetchExistingCacheKeys() {
  ...
  const pageSize = 1000;
  while (true) {
    const url = `${base}?select=cache_key,product_name,brand&or=(brand.ilike.*Taste%20of%20the%20Wild*,product_name.ilike.*Taste%20of%20the%20Wild*,cache_key.ilike.*taste%20of%20the%20wild*)&offset=${offset}&limit=${pageSize}`;
...
async function parseTarget(target) {
  ...
  return {
    target: {
      ...
      brand: SOURCE_BRAND,
      cacheKey: buildCacheKey(SOURCE_BRAND, officialProductName) || target.cacheKey,
...
function exportParsed(targets, metadata) {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      source: "tasteofthewild_official_product_data",
...
function loadVerifiedReportKeys() {
  ...
  for (const line of fs.readFileSync(reportPath, "utf8").split(/\r?\n/)) {
    ...
    try {
      ...
      if (entry?.event === "tasteofthewild_official_product_result" && entry?.status === "verified_saved" && entry?.cache_key) {
...
async function writeTarget(target) {
  ...
  const response = await fetch(`${supabaseBase()}/rest/v1/rpc/${rpc}`, {
    method: "POST",
    headers: restHeaders(TASTEOFTHEWILD_PRODUCT_IMPORT_SERVICE_KEY),
...
async function processWrites(targets) {
  ...
  async function worker() {
    while (nextIndex < selected.length) {
      ...
      appendReport({
        event: "tasteofthewild_official_product_result",
...
async function main() {
  ...
  for (const target of planned) {
    ...
    try {
      ...
      console.warn(`Skipped Taste of the Wild target: ${target.brand} ${target.name} (${error?.message || error})`);
      continue;
    }
    if (!result.target) {
      skipped.parse += 1;
  ...
  console.log(`Taste of the Wild official input targets: ${inputTargets.length}`);
  console.log(`Existing Taste of the Wild keys skipped: ${skipped.existing}`);
