...
const DEFAULT_SITEMAP_URL = "https://www.acana.com/en-US/sitemap_0.xml";
const DEFAULT_OUTPUT = ".tmp/acana-sitemap-catalog-targets.json";
...
const CORE_FOOD_TERMS = [
  ...
  "large breed", "small breed", "highest protein", "wholesome grains",
  "healthy grains", "classics", "singles", "butcher's favorites",
  "premium pate", "premium chunks", "chunks in broth", "pate",
  "red meat", "free run poultry", "wild prairie", "wild atlantic",
  "pacifica", "grasslands", "ranchlands", "meadowland", "homestead harvest",
  "bountiful catch", "indoor entree", "first feast", "market medley",
  "light and fit", "sport and agility", "freshwater fish", "coldwater fish",
  "duck", "lamb", "beef", "pork", "salmon", "tuna", "chicken",
  "turkey", "fish", "poultry", "grains", "recipe", "blend",
];
const REJECT_TERMS = [
  "treat", "treats", "lickables", "jerky", "jerky bites",
  "freeze dried treats", "fdt", "chewy strips", "chewy tenders",
  "chew", "chews", "snack", "snacks", "topper", "toppers",
  "supplement", "supplements", "catnip", "litter",
];
const NON_FOOD_TERMS = [
  "search", "store", "where to buy", "article", "articles", "faq",
  "club acana", "about", "contact", "privacy", "terms", "accessory",
  "accessories", "toy", "toys", "collar", "leash", "harness", "bed",
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:acana-sitemap-catalog -- --output=.tmp/acana-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit ACANA sitemap URL (${DEFAULT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.acana\.com\/en-US\/sitemap_0\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://www.acana.com/en-US/sitemap_0.xml.");
  }
}
...
function formFromPath(nameSlug, productId) {
  ...
  if (
    hasTerm(text, "chunks in broth") ||
    hasTerm(text, "premium chunks") ||
    hasTerm(text, "premium pate") ||
    hasTerm(text, "pate") ||
    hasTerm(text, "bone broth") ||
    productId.includes("pate") ||
    productId.includes("chunks")
  ) {
    return "wet";
  }
  if (hasTerm(text, "freeze dried patties")) return "fresh";
  return "dry";
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`ACANA sitemap fetch failed ${response.status}: ${url}`);
...
function inferAcanaPetTypes(row) {
...
function isCandidateFoodRow(row) {
  ...
  const petTypes = inferAcanaPetTypes(row);
  if (!/^https:\/\/www\.acana\.com\/en-US\/(dogs|cats)\/(dog-food|cat-food)\//i.test(row.sourceUrl)) return false;
...
async function main() {
  ...
  console.log(`ACANA products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing ACANA sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("ACANA sitemap catalog target export failed:", err.message);
  process.exit(1);
});
