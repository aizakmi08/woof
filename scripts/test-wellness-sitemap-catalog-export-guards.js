...
const source = fs.readFileSync(path.join(root, "scripts/export-wellness-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Wellness sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://www.wellnesspetfood.com/salsify-products-sitemap.xml") &&
    source.includes("const BRAND = \"Wellness\"") &&
  ...
  "exporter must read the official Wellness Salsify product sitemap, derive titles from sitemap slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"tiny tasters\"") &&
    source.includes("\"mini meals\"") &&
    source.includes("\"petite entrees\"") &&
    source.includes("\"signature selects\"") &&
    source.includes("\"purrfect duos\"") &&
    source.includes("TREAT_TERMS") &&
    source.includes("\"kittles\"") &&
    source.includes("\"tiny trainers\"") &&
    source.includes("SUPPLEMENTAL_FOOD_TERMS") &&
    source.includes("\"bowl boosters\"") &&
    source.includes("\"supplement\"") &&
    source.includes("NON_FOOD_TERMS") &&
    source.includes("\"whimzees\"") &&
    source.includes("\"old mother hubbard\"") &&
    source.includes("function inferWellnessPetTypes(row)") &&
    source.includes("if (hasTerm(text, \"tiny tasters\")) return [\"cat\"]") &&
    source.includes("if (hasTerm(text, \"mini meals\")) return [\"dog\"]") &&
  ...
  "exporter must strictly filter Wellness sitemap rows to complete foods and reject treats, toppers, supplements, WHIMZEES, Old Mother Hubbard, and non-food rows by default"
);
...
assert(
  source.includes("source: \"wellness_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"wellness_salsify_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:wellness-sitemap-catalog": "node scripts/export-wellness-sitemap-targets.js"') &&
    packageJson.includes('"test:wellness-sitemap-catalog-export": "node scripts/test-wellness-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:wellness-sitemap-catalog-export"),
  "package scripts must expose Wellness sitemap exporter and include its guard in test:guards"
);

console.log("Wellness sitemap catalog export guard passed");
