...
const DEFAULT_SITEMAP_URL = "https://www.sheba.com/sitemap.xml";
const DEFAULT_OUTPUT = ".tmp/sheba-sitemap-catalog-targets.json";
...
const includeComplements = process.argv.includes("--include-complements");
...
const CORE_FOOD_TERMS = [
  "wet cat food", "perfect portions", "premium pate", "cuts gravy",
  "cuts in gravy", "gravy indulgence", "grilled", "kitten pate",
  "adult wet cat food", "entree", "variety pack", "multipack", "twin pack",
  "pate", "gravy", "chicken", "beef", "turkey", "salmon", "tuna",
  "whitefish", "trout", "seafood", "mixed grill", "roasted", "tender",
  "sustainable", "signature",
];
const TREAT_TERMS = [
  "treat", "treats", "meaty tender sticks", "tender sticks", "stick", "sticks",
  "bisque", "puree", "lickable", "snack", "snacks",
];
const SUPPLEMENTAL_FOOD_TERMS = [
  "broth", "broths", "filets broth", "meal complement", "complement",
  "complements", "topper", "toppers", "supplement", "supplements",
  "meal booster", "probiotic", "probiotics",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:sheba-sitemap-catalog -- --output=.tmp/sheba-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit Sheba sitemap URL (${DEFAULT_SITEMAP_URL})
  ...
  --include-complements   Include broth/complement rows in addition to core food URLs
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.sheba\.com\/sitemap\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://www.sheba.com/sitemap.xml.");
  }
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/www\.sheba\.com\/products\/(wet|treats)\/[a-z0-9-]+$/i.test(url)) return "";
  return url;
}
...
function parsedProductPath(url) {
  try {
    ...
    if (products !== "products" || !form || !slug) return null;
    return { form, slug };
  } catch {
    return null;
  }
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Sheba sitemap fetch failed ${response.status}: ${url}`);
...
function inferShebaPetTypes(row) {
  const inferred = inferPetTypes({
    brand: BRAND,
    product_name: `cat ${row.name} ${row.sourceUrl}`,
    cache_key: normalizeCacheKey(`cat ${row.name} ${row.sourceUrl}`),
    source: "sheba_sitemap",
...
function isCandidateFoodRow(row) {
  ...
  const petTypes = inferShebaPetTypes(row);
  if (!/^https:\/\/www\.sheba\.com\/products\/(wet|treats)\//i.test(row.sourceUrl)) return false;
  if (petTypes.length !== 1 || petTypes[0] !== "cat") return false;
  if (row.form !== "wet" && !(includeTreats && row.form === "treats")) return false;
  if (row.form === "treats" && !includeTreats) return false;
  if (NON_FOOD_TERMS.some((term) => hasTerm(text, term))) return false;
  if (!includeComplements && SUPPLEMENTAL_FOOD_TERMS.some((term) => hasTerm(text, term))) return false;
...
async function main() {
  ...
  console.log(`Sheba products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Sheba sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Sheba sitemap catalog target export failed:", err.message);
  process.exit(1);
});
