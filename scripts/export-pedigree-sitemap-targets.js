...
const DEFAULT_SITEMAP_URL = "https://www.pedigree.com/sitemap.xml";
const DEFAULT_OUTPUT = ".tmp/pedigree-sitemap-catalog-targets.json";
const REST_PAGE_SIZE = 1000;
const USER_AGENT = "Mozilla/5.0 Woof catalog coverage export";
const BRAND = "Pedigree";
...
const DEFAULT_PRODUCT_FORMS = new Set(["wet", "dry"]);
const PRODUCT_PATH_FORMS = new Set(["wet", "dry", "wet-can", "wet-pouch"]);
const CORE_FOOD_TERMS = [
  "dog food", "wet dog food", "dry dog food", "choice cuts", "chopped ground dinner",
  "complete nutrition", "tender bites", "high protein", "healthy weight",
  "puppy", "small dog", "small breed", "adult", "big dogs", "large breed",
  "variety pack", "multipack", "homestyle meals", "morsels sauce", "gravy",
  "grilled steak", "roasted chicken", "chicken", "beef", "lamb", "turkey",
  "duck", "bison", "bacon", "filet mignon", "prime rib", "t-bone",
  "country stew", "rice", "vegetable", "dinner", "recipe", "stew",
];
const TREAT_TERMS = [
  "drizzlers", "dentastix", "dental", "treat", "treats", "chewy chunx",
  "jumbone", "marrobone", "biscuit", "biscuits", "twisty", "oral",
  "chew", "chews", "snack", "snacks",
];
const SUPPLEMENTAL_FOOD_TERMS = [
  "topper", "toppers", "complement", "complements", "supplement",
  "supplements", "meal booster", "probiotic", "probiotics",
];
const NON_FOOD_TERMS = [
  "toy", "toys", "bowl holder", "mat", "collar", "leash", "harness", "bed",
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:pedigree-sitemap-catalog -- --output=.tmp/pedigree-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit Pedigree sitemap URL (${DEFAULT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.pedigree\.com\/sitemap\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://www.pedigree.com/sitemap.xml.");
  }
}
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .join("")
    .replace(/\bPedigree\b/g, "Pedigree")
    .replace(/\bCt\b/g, "ct")
    .replace(/\s+([,])/g, "$1")
    .trim();
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/www\.pedigree\.com\/products\/(dry|wet|wet-can|wet-pouch)\/[a-z0-9-]+$/i.test(url)) return "";
  return url;
}
...
function parsedProductPath(url) {
  try {
    ...
    const [products, pathForm, slug] = segments;
    if (products !== "products" || !PRODUCT_PATH_FORMS.has(pathForm) || !slug) return null;
    return { pathForm, form: pathForm === "dry" ? "dry" : "wet", slug };
  } catch {
    return null;
  }
}
...
function productNameFromSlug(slug) {
  return cleanProductName(String(slug || "")
    .replace(/^pedigreechoice-/, "choice-")
    .replace(/^pedigree-/, "")
    .replace(/\b(\d+)ct\b/g, "$1 ct")
    .replace(/--+/g, "-")
    .replace(/-/g, " "));
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Pedigree sitemap fetch failed ${response.status}: ${url}`);
...
function inferPedigreePetTypes(row) {
  const inferred = inferPetTypes({
    brand: BRAND,
    product_name: `dog ${row.name} ${row.sourceUrl}`,
    cache_key: normalizeCacheKey(`dog ${row.name} ${row.sourceUrl}`),
    source: "pedigree_sitemap",
  }, { includeAmbiguous: false });
  if (inferred.includes("dog")) return ["dog"];
  return ["dog"];
}

function isCandidateFoodRow(row) {
  const text = normalizeText(`${row.name} ${row.sourceUrl} ${row.retailerProductId} ${row.form}`);
  const petTypes = inferPedigreePetTypes(row);
  if (!/^https:\/\/www\.pedigree\.com\/products\/(dry|wet|wet-can|wet-pouch)\//i.test(row.sourceUrl)) return false;
  if (petTypes.length !== 1 || petTypes[0] !== "dog") return false;
  if (!DEFAULT_PRODUCT_FORMS.has(row.form)) return false;
...
function normalizeTarget(row) {
  ...
  return {
    ...
    cacheKey,
    source: "pedigree_sitemap_export",
...
async function main() {
  ...
  const manifest = {
    ...
    source: "pedigree_product_sitemap",
    sitemapUrl,
    includeExisting,
    skipExistingScan,
    scannedProducts,
  ...
  console.log(`Pedigree products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Pedigree sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Pedigree sitemap catalog target export failed:", err.message);
  process.exit(1);
});
