...
const WERUVA_PRODUCT_IMPORT_SERVICE_KEY =
  process.env.WERUVA_PRODUCT_IMPORT_SERVICE_KEY ||
  process.env.PRODUCT_LOOKUP_SERVICE_KEY ||
  process.env.SUPABASE_SERVICE_ROLE_KEY ||
  process.env.SUPABASE_SERVICE_KEY ||
  "";
const DEFAULT_INPUT = ".tmp/weruva-sitemap-catalog-targets.json";
const DEFAULT_EXPORT_PARSED = ".tmp/weruva-official-product-data.json";
const DEFAULT_REPORT = ".tmp/weruva-official-product-backfill-report.jsonl";
const USER_AGENT = "Mozilla/5.0 Woof official Weruva product import";
...
const NUTRIENT_FIELD_ALIASES = [
  ...
  [/\bcrude\s+moisture\b|\bmoisture\b/i, "moisture_pct"],
  [/\bash\b/i, "ash_pct"],
  [/\bcalcium\b/i, "calcium_pct"],
  [/\bphosphorus\b/i, "phosphorus_pct"],
  [/\bomega\s*3\b|\bomega[-\s]*3\b/i, "omega_3_pct"],
  [/\bomega\s*6\b|\bomega[-\s]*6\b/i, "omega_6_pct"],
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run backfill:weruva-official-products -- --dry-run
  WERUVA_PRODUCT_IMPORT_SERVICE_KEY=... npm run backfill:weruva-official-products

Options:
  --input=path.json           Weruva catalog target manifest (default ${DEFAULT_INPUT})
...
function assertConfig() {
  ...
  if (!fs.existsSync(inputPath)) usage(`Missing Weruva input manifest: ${path.relative(root, inputPath)}`);
  if (!dryRun && !WERUVA_PRODUCT_IMPORT_SERVICE_KEY) {
    usage("Set WERUVA_PRODUCT_IMPORT_SERVICE_KEY, PRODUCT_LOOKUP_SERVICE_KEY, or Supabase service key for official Weruva product writes.");
  }
...
function inferPetType(sourceUrl, fallback = "") {
  ...
  if (/\bdogs?\b|\bdoggy\b|\bwoof\b/.test(text)) return "dog";
  if (/\bcats?\b|\bkitty\b|\bkitten\b|\bmeow\b|\bpurr\b/.test(text)) return "cat";
  return "";
}

function inferForm(sourceUrl, fallback = "") {
  const text = normalizeText(`${sourceUrl} ${fallback}`);
  if (/\b(variety pack|multipack|multi pack|sampler|assortment)\b/.test(text)) return "variety_pack";
  if (/\b(dry|kibble)\b/.test(text)) return "dry";
  if (/\b(pouch|pouches)\b/.test(text)) return "pouch";
  if (/\b(can|canned|pate|gravy|stew|au jus|mousse|shreds|minced|puree|in sauce|wet)\b/.test(text)) return "wet";
  return "";
}
...
function normalizeTarget(entry) {
  ...
  const brand = cleanProductName(entry?.brand || SOURCE_BRAND);
...
function isMultiPack(target) {
  return /\b(variety\s+pack|multipack|multi\s+pack|sampler|assortment|bundle|combo\s+pack|value\s+pack)\b/i.test(`${target.name} ${target.sourceUrl} ${target.cacheKey} ${target.form}`);
}
...
function parseJsonLd(html) {
  ...
  for (const match of String(html || "").matchAll(/<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)) {
    try {
      ...
      // Ignore analytics or malformed JSON-LD blocks.
    }
  }
  return products;
}
...
function parseImageUrl(html) {
  ...
  const raw =
    images.find((image) => typeof image === "string") ||
    String(html || "").match(/<meta\s+property=["']og:image["']\s+content=["']([^"']+)["']/i)?.[1] ||
    "";
  const decoded = decodeHtml(raw);
  if (decoded.startsWith("//")) return `https:${decoded}`.slice(0, 500);
  if (decoded.startsWith("/")) return `https://www.weruva.com${decoded}`.slice(0, 500);
  return /^https:\/\//i.test(decoded) ? decoded.slice(0, 500) : "";
}

function parseSpecItems(html) {
  const specs = {};
  for (const match of String(html || "").matchAll(/<li[^>]+class=["'][^"']*product-spec__item[^"']*["'][^>]*>([\s\S]*?)<\/li>/gi)) {
    const item = match[1];
    const labelHtml = item.match(/<div[^>]+class=["'][^"']*product-spec__label[^"']*["'][^>]*>([\s\S]*?)<\/div>/i)?.[1] || "";
    const valueHtml = item.match(/<div[^>]+class=["'][^"']*product-spec__value[^"']*["'][^>]*>([\s\S]*?)<\/div>/i)?.[1] || "";
    const label = normalizeText(stripTags(labelHtml));
    const value = stripTags(valueHtml);
    if (label && value) specs[label] = value;
  }
  for (const match of String(html || "").matchAll(/<details[^>]+class=["'][^"']*disclosure[^"']*["'][^>]*>([\s\S]*?)<\/details>/gi)) {
    const item = match[1];
    const labelHtml = item.match(/<h2[^>]*class=["'][^"']*disclosure__title[^"']*["'][^>]*>([\s\S]*?)<\/h2>/i)?.[1] || "";
    const valueHtml = item.match(/<div[^>]+class=["'][^"']*disclosure__content[^"']*["'][^>]*>([\s\S]*?)<\/div>/i)?.[1] || "";
    const label = normalizeText(stripTags(labelHtml));
    const value = stripTags(valueHtml);
...
function parseIngredientsFromSpecs(specs) {
  const text = specValue(specs, [/^ingredients?$/i])
    .replace(/\s+(Guaranteed Analysis|Caloric Content|Calorie Content).*$/i, "")
    .trim();
...
function parseNutrientPanel(specs, sourceUrl) {
  const text = specValue(specs, [/^guaranteed analysis$/i, /^analysis$/i]);
  const panel = { basis: "as-fed", source_url: sourceUrl };
  for (const match of text.matchAll(/([A-Za-z0-9\-\s]+?)\s*(?:\((?:min|max)\))?\s*:?\s*([\d.]+)\s*%/gi)) {
    const field = nutrientField(match[1]);
    const value = parsePercent(`${match[2]}%`);
    if (field && value != null && panel[field] == null) panel[field] = value;
  }
  const calorieText = specValue(specs, [/^caloric content$/i, /^calorie content$/i]);
  let match = calorieText.match(/([\d,]+(?:\.\d+)?)\s*kcal\s*(?:ME\s*)?\/\s*kg/i);
  if (match) panel.calories_per_kg = Number(match[1].replace(/,/g, ""));
  match = calorieText.match(/kcal\s+per\s+kg\s*:?\s*([\d,]+(?:\.\d+)?)/i);
  if (match) panel.calories_per_kg = Number(match[1].replace(/,/g, ""));
  match = calorieText.match(/([\d,]+(?:\.\d+)?)\s*kcal\s*(?:ME\s*)?\/\s*cup/i);
  if (match) panel.calories_per_cup = Number(match[1].replace(/,/g, ""));
  match = calorieText.match(/kcal\s+per\s+cup\s*:?\s*([\d,]+(?:\.\d+)?)/i);
  if (match) panel.calories_per_cup = Number(match[1].replace(/,/g, ""));
...
function slugSuffix(sourceUrl) {
  try {
    ...
    return normalizeText(slug.replace(/-/g, " "))
      .replace(/\b(weruva|soulistic|cat person|food)\b/g, " ")
      .replace(/\s+/g, " ")
      .trim()
      .split(/\s+/)
      .slice(0, 8)
...
async function fetchText(url, accept = "text/html,application/xhtml+xml;q=0.9,*/*;q=0.8", attempt = 1) {
  ...
  if (response.status === 403 || response.status === 429 || response.status >= 500) {
    if (attempt >= 7) throw new Error(`Weruva official fetch failed ${response.status}: ${url}`);
  ...
  if (!response.ok) throw new Error(`Weruva official fetch failed ${response.status}: ${url}`);
...
async function fetchExistingCacheKeys() {
  ...
  const pageSize = 1000;
  while (true) {
    const url = `${base}?select=cache_key,product_name,brand&or=(brand.ilike.*Weruva*,brand.ilike.*Soulistic*,brand.ilike.*Cat%20Person*,product_name.ilike.*Weruva*,product_name.ilike.*Soulistic*,product_name.ilike.*Cat%20Person*,cache_key.ilike.*weruva*,cache_key.ilike.*soulistic*,cache_key.ilike.*cat%20person*)&offset=${offset}&limit=${pageSize}`;
...
async function parseTarget(target) {
  ...
  const specs = parseSpecItems(html);
  const ingredients = parseIngredientsFromSpecs(specs);
  ...
  const brand = cleanProductName(target.brand || SOURCE_BRAND);
  const petType = target.petType || inferPetType(target.sourceUrl, officialProductName);
  const nutrientPanel = parseNutrientPanel(specs, target.sourceUrl);
  ...
  return {
    target: {
      ...
      brand,
      cacheKey: buildCacheKey(brand, officialProductName) || target.cacheKey,
      petType,
...
function exportParsed(targets, metadata) {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      source: "weruva_official_product_data",
...
function loadVerifiedReportKeys() {
  ...
  for (const line of fs.readFileSync(reportPath, "utf8").split(/\r?\n/)) {
    ...
    try {
      ...
      if (entry?.event === "weruva_official_product_result" && entry?.status === "verified_saved" && entry?.cache_key) {
...
async function writeTarget(target) {
  ...
  const response = await fetch(`${supabaseBase()}/rest/v1/rpc/${rpc}`, {
    method: "POST",
    headers: restHeaders(WERUVA_PRODUCT_IMPORT_SERVICE_KEY),
...
async function processWrites(targets) {
  ...
  async function worker() {
    while (nextIndex < selected.length) {
      ...
      appendReport({
        event: "weruva_official_product_result",
...
async function main() {
  ...
  for (const target of planned) {
    ...
    try {
      ...
      console.warn(`Skipped Weruva target: ${target.brand} ${target.name} (${error?.message || error})`);
      continue;
    }
    if (!result.target) {
      skipped.parse += 1;
  ...
  console.log(`Weruva official input targets: ${inputTargets.length}`);
  console.log(`Existing Weruva keys skipped: ${skipped.existing}`);
