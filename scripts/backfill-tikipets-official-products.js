...
const TIKIPETS_PRODUCT_IMPORT_SERVICE_KEY =
  process.env.TIKIPETS_PRODUCT_IMPORT_SERVICE_KEY ||
  process.env.PRODUCT_LOOKUP_SERVICE_KEY ||
  process.env.SUPABASE_SERVICE_ROLE_KEY ||
  process.env.SUPABASE_SERVICE_KEY ||
  "";
const DEFAULT_INPUT = ".tmp/tikipets-sitemap-catalog-targets.json";
const DEFAULT_EXPORT_PARSED = ".tmp/tikipets-official-product-data.json";
const DEFAULT_REPORT = ".tmp/tikipets-official-product-backfill-report.jsonl";
const USER_AGENT = "Mozilla/5.0 Woof official Tiki Pets product import";
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run backfill:tikipets-official-products -- --dry-run
  TIKIPETS_PRODUCT_IMPORT_SERVICE_KEY=... npm run backfill:tikipets-official-products

Options:
  --input=path.json           Tiki Pets catalog target manifest (default ${DEFAULT_INPUT})
  ...
  --delay-ms=N                Delay between official page requests (default 2500)
...
function assertConfig() {
  ...
  if (!fs.existsSync(inputPath)) usage(`Missing Tiki Pets input manifest: ${path.relative(root, inputPath)}`);
  if (!dryRun && !TIKIPETS_PRODUCT_IMPORT_SERVICE_KEY) {
    usage("Set TIKIPETS_PRODUCT_IMPORT_SERVICE_KEY, PRODUCT_LOOKUP_SERVICE_KEY, or Supabase service key for official Tiki Pets product writes.");
  }
...
function cleanProductName(value) {
  return decodeHtml(value)
    .replace(/\u2122|\u00AE|\u00A9/g, "")
    .replace(/\s+-\s+Tiki Pets.*$/i, "")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 220);
}

function inferBrand(value, petType = "") {
  const text = normalizeText(value);
  if (text.includes("tiki dog") || petType === "dog") return "Tiki Dog";
  return "Tiki Cat";
}

function inferForm(sourceUrl, fallback = "") {
  const text = normalizeText(`${sourceUrl} ${fallback}`);
  if (/\b(dry|kibble|born carnivore)\b/.test(text)) return "dry";
  if (/\b(wet|can|canned|pate|mousse|pouch|broth|luau|grill|after dark|hearty|stew)\b/.test(text)) return "wet";
  return "";
}
...
function normalizeTarget(entry) {
  ...
  const brand = cleanProductName(entry?.brand || inferBrand(`${sourceUrl} ${name}`, petType)) || inferBrand(sourceUrl, petType);
...
function isMultiPack(target) {
  return /\b(variety\s+pack|multipack|multi\s+pack|sampler|bundle)\b/i.test(`${target.name} ${target.sourceUrl} ${target.cacheKey}`);
}
...
function extractById(html, id, nextId = "") {
  const raw = String(html || "");
  const start = raw.search(new RegExp(`<div[^>]+id=["']${id}["'][^>]*>`, "i"));
  if (start < 0) return "";
  if (nextId) {
    const next = raw.search(new RegExp(`<div[^>]+id=["']${nextId}["'][^>]*>`, "i"));
    if (next > start) return raw.slice(start, next);
  }
  return raw.slice(start, start + 12000);
}

function parseOfficialProductName(html, fallback) {
  const h1 = String(html || "").match(/<h1[^>]*>([\s\S]*?)<\/h1>/i)?.[1] || "";
  const title = String(html || "").match(/<title>([\s\S]*?)<\/title>/i)?.[1] || "";
  return cleanProductName(stripTags(h1) || stripTags(title) || fallback);
}
...
function parseImageUrl(html) {
  ...
  if (decoded.startsWith("/")) return `https://tikipets.com${decoded}`.slice(0, 500);
...
function parseIngredients(html) {
  const pane = extractById(html, "ingredients-pane", "analysis-and-calorie-pane");
  const body = pane.match(/<div[^>]+class=["'][^"']*card-body[^"']*["'][^>]*>([\s\S]*?)<\/div>/i)?.[1] || pane;
  const text = stripTags(body)
    .replace(/^Ingredients\s+/i, "")
    .replace(/\s+(Guaranteed Analysis|Calorie Content|Feeding Guidelines).*$/i, "")
    .trim();
...
function parseNutrientPanel(html, sourceUrl) {
  const pane = extractById(html, "analysis-and-calorie-pane", "feeding-guidelines-pane");
  const panel = { basis: "as-fed", source_url: sourceUrl };
  for (const row of pane.matchAll(/<tr[^>]*>([\s\S]*?)<\/tr>/gi)) {
  ...
  const text = stripTags(pane || html);
  for (const match of text.matchAll(/(Crude Protein|Crude Fat|Crude Fiber|Moisture|Calcium|Phosphorus|Omega\s*3[^;:]*|Omega\s*6[^;:]*)\s*(?:\([^)]*\))?\s*([\d.]+)\s*%/gi)) {
    const field = nutrientField(match[1]);
    const value = parsePercent(`${match[2]}%`);
    if (field && value != null && panel[field] == null) panel[field] = value;
  }
  let match = text.match(/([\d,]+(?:\.\d+)?)\s*kcal\s*(?:ME\s*)?\/\s*kg/i);
  if (match) panel.calories_per_kg = Number(match[1].replace(/,/g, ""));
  match = text.match(/([\d,]+(?:\.\d+)?)\s*kcal\s*(?:ME\s*)?\/\s*cup/i);
...
async function fetchText(url, accept = "text/html,application/xhtml+xml;q=0.9,*/*;q=0.8", attempt = 1) {
  ...
  if (response.status === 403 || response.status === 429 || response.status >= 500) {
    if (attempt >= 7) throw new Error(`Tiki Pets official fetch failed ${response.status}: ${url}`);
  ...
  if (!response.ok) throw new Error(`Tiki Pets official fetch failed ${response.status}: ${url}`);
...
async function fetchExistingCacheKeys() {
  ...
  const pageSize = 1000;
  while (true) {
    const url = `${base}?select=cache_key,product_name,brand&or=(brand.ilike.*Tiki*,product_name.ilike.*Tiki*,cache_key.ilike.*tiki*)&offset=${offset}&limit=${pageSize}`;
...
async function parseTarget(target) {
  ...
  return {
    target: {
      ...
      brand: inferBrand(`${target.sourceUrl} ${officialProductName} ${target.brand}`, target.petType),
...
function exportParsed(targets, metadata) {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      source: "tikipets_official_product_data",
...
function loadVerifiedReportKeys() {
  ...
  for (const line of fs.readFileSync(reportPath, "utf8").split(/\r?\n/)) {
    ...
    try {
      ...
      if (entry?.event === "tikipets_official_product_result" && entry?.status === "verified_saved" && entry?.cache_key) {
...
async function writeTarget(target) {
  ...
  const response = await fetch(`${supabaseBase()}/rest/v1/rpc/${rpc}`, {
    method: "POST",
    headers: restHeaders(TIKIPETS_PRODUCT_IMPORT_SERVICE_KEY),
...
async function processWrites(targets) {
  ...
  async function worker() {
    while (nextIndex < selected.length) {
      ...
      appendReport({
        event: "tikipets_official_product_result",
...
async function main() {
  ...
  for (const target of planned) {
    ...
    try {
      ...
      console.warn(`Skipped Tiki Pets target: ${target.name} (${error?.message || error})`);
      continue;
    }
    if (!result.target) {
      skipped.parse += 1;
  ...
  console.log(`Tiki Pets official input targets: ${inputTargets.length}`);
  console.log(`Existing Tiki Pets keys skipped: ${skipped.existing}`);
