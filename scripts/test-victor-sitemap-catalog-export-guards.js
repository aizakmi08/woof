...
const source = fs.readFileSync(path.join(root, "scripts/export-victor-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`VICTOR sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://victorpetfood.com/sitemap.xml") &&
    source.includes("const BRAND = \"VICTOR\"") &&
    ...
    source.includes("victorpetfood\\.com\\/products") &&
    !source.includes("product-lookup") &&
    !source.includes("cheerio") &&
    !source.includes("puppeteer"),
  "exporter must read the official VICTOR sitemap, derive titles from product slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("DOG_SLUGS") &&
    source.includes("\"hi-pro-plus\"") &&
    source.includes("\"grain-free-hero-canine\"") &&
    source.includes("CAT_SLUGS") &&
    source.includes("\"fit-feline\"") &&
    source.includes("\"mers-classic-feline\"") &&
    source.includes("CATEGORY_SLUGS") &&
    source.includes("\"cat-formulas\"") &&
    source.includes("CORE_FOOD_TERMS") &&
    source.includes("\"hi-pro plus\"") &&
    source.includes("\"nutra pro\"") &&
    source.includes("TREAT_TERMS") &&
    source.includes("\"crunchy\"") &&
    source.includes("SUPPLEMENTAL_FOOD_TERMS") &&
    source.includes("\"probiotic\"") &&
    source.includes("NON_FOOD_TERMS") &&
    source.includes("\"litter\"") &&
    source.includes("function inferVictorPetTypes(row)") &&
    source.includes("if (!DOG_SLUGS.has(slug) && !CAT_SLUGS.has(slug)) return false") &&
    ...
    source.includes("if (!/^https:\\/\\/victorpetfood\\.com\\/products\\//i.test(row.sourceUrl)) return false") &&
  ...
  "exporter must strictly filter VICTOR sitemap rows to allowlisted dog/cat foods and reject categories, supplements, treats, and non-food rows by default"
);
...
assert(
  source.includes("source: \"victor_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"victor_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:victor-sitemap-catalog": "node scripts/export-victor-sitemap-targets.js"') &&
    packageJson.includes('"test:victor-sitemap-catalog-export": "node scripts/test-victor-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:victor-sitemap-catalog-export"),
  "package scripts must expose VICTOR sitemap exporter and include its guard in test:guards"
);

console.log("VICTOR sitemap catalog export guard passed");
