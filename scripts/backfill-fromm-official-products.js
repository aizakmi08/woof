...
const FROMM_PRODUCT_IMPORT_SERVICE_KEY =
  process.env.FROMM_PRODUCT_IMPORT_SERVICE_KEY ||
  process.env.PRODUCT_LOOKUP_SERVICE_KEY ||
  process.env.SUPABASE_SERVICE_ROLE_KEY ||
  process.env.SUPABASE_SERVICE_KEY ||
  "";
const DEFAULT_INPUT = ".tmp/fromm-sitemap-catalog-targets.json";
const DEFAULT_EXPORT_PARSED = ".tmp/fromm-official-product-data.json";
const DEFAULT_REPORT = ".tmp/fromm-official-product-backfill-report.jsonl";
const USER_AGENT = "Mozilla/5.0 Woof official Fromm product import";
const BRAND = "Fromm";
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run backfill:fromm-official-products -- --dry-run
  FROMM_PRODUCT_IMPORT_SERVICE_KEY=... npm run backfill:fromm-official-products

Options:
  --input=path.json           Fromm catalog target manifest (default ${DEFAULT_INPUT})
...
function assertConfig() {
  ...
  if (!fs.existsSync(inputPath)) usage(`Missing Fromm input manifest: ${path.relative(root, inputPath)}`);
  if (!dryRun && !FROMM_PRODUCT_IMPORT_SERVICE_KEY) {
    usage("Set FROMM_PRODUCT_IMPORT_SERVICE_KEY, PRODUCT_LOOKUP_SERVICE_KEY, or Supabase service key for official Fromm product writes.");
  }
...
function inferForm(sourceUrl, fallback = "") {
  ...
  if (/\/frozen\//.test(sourceUrl) || /\b(frozen|fresh)\b/.test(text)) return "fresh";
  if (/\/can\//.test(sourceUrl) || /\b(can|canned|wet|pate|diner|frommbalaya|gumbo|gravy|stew)\b/.test(text)) return "wet";
  if (/\/dry\//.test(sourceUrl) || /\b(dry|kibble)\b/.test(text)) return "dry";
  return "";
}
...
function parseIngredients(html) {
  ...
  const marker = source.match(/<!--\s*Ingredients\s*-->|<h3[^>]*>\s*Ingredients\s*<\/h3>/i);
  const markerIndex = marker?.index ?? -1;
  const sectionEnd = markerIndex >= 0 ? source.indexOf("</section>", markerIndex) : -1;
  const block = markerIndex >= 0
    ? source.slice(markerIndex, sectionEnd > markerIndex ? sectionEnd : markerIndex + 12000)
    : source.match(/<div[^>]+class=["'][^"']*\bingredients\b[^"']*["'][^>]*>([\s\S]*?)<\/div>\s*<\/section>/i)?.[1] || "";
  if (!block) return [];
...
function parseImageUrl(html) {
  const raw =
    ...
    String(html || "").match(/<img[^>]+class=["'][^"']*(?:product|Model-preview)[^"']*["'][^>]+src=["']([^"']+)["']/i)?.[1] ||
    "";
  const decoded = decodeHtml(raw).replace(/\\\//g, "/");
  if (decoded.startsWith("//")) return `https:${decoded}`.slice(0, 500);
  if (decoded.startsWith("/")) return `https://frommfamily.com${decoded}`.slice(0, 500);
...
async function fetchText(url, attempt = 1) {
  ...
  if (response.status === 429 || response.status >= 500) {
    if (attempt >= 5) throw new Error(`Fromm official page fetch failed ${response.status}: ${url}`);
    const retryAfter = Number(response.headers.get("retry-after"));
    await sleep(Number.isFinite(retryAfter) ? retryAfter * 1000 : 1000 * attempt * attempt);
    return fetchText(url, attempt + 1);
  }
  if (!response.ok) throw new Error(`Fromm official page fetch failed ${response.status}: ${url}`);
...
async function fetchExistingCacheKeys() {
  ...
  const pageSize = 1000;
  while (true) {
    const url = `${base}?select=cache_key,product_name,brand&or=(brand.ilike.*Fromm*,product_name.ilike.*Fromm*,cache_key.ilike.*fromm*)&offset=${offset}&limit=${pageSize}`;
...
function exportParsed(targets, metadata) {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      source: "fromm_official_product_data",
...
function loadVerifiedReportKeys() {
  ...
  for (const line of fs.readFileSync(reportPath, "utf8").split(/\r?\n/)) {
    ...
    try {
      ...
      if (entry?.event === "fromm_official_product_result" && entry?.status === "verified_saved" && entry?.cache_key) {
...
async function writeTarget(target) {
  ...
  const response = await fetch(`${supabaseBase()}/rest/v1/rpc/${rpc}`, {
    method: "POST",
    headers: restHeaders(FROMM_PRODUCT_IMPORT_SERVICE_KEY),
...
async function processWrites(targets) {
  ...
  async function worker() {
    while (nextIndex < selected.length) {
      ...
      appendReport({
        event: "fromm_official_product_result",
...
async function main() {
  ...
  for (const target of planned) {
    ...
    try {
      ...
      console.warn(`Skipped Fromm target: ${target.name} (${error?.message || error})`);
      continue;
    }
    if (!result.target) {
      skipped.parse += 1;
  ...
  console.log(`Fromm official input targets: ${inputTargets.length}`);
  console.log(`Existing Fromm keys skipped: ${skipped.existing}`);
