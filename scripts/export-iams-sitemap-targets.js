...
const DEFAULT_SITEMAP_URL = "https://www.iams.com/sitemap.xml";
const DEFAULT_OUTPUT = ".tmp/iams-sitemap-catalog-targets.json";
...
const DOG_TERMS = [
  "dog", "dogs", "puppy", "puppies", "large breed", "small breed", "mini chunks",
];

const CAT_TERMS = [
  "cat", "cats", "kitten", "kittens", "hairball", "perfect portions", "healthy enjoyment",
];

const CORE_FOOD_TERMS = [
  "proactive health", "advanced health", "perfect portions", "healthy enjoyment",
  "healthy kitten", "healthy puppy", "healthy aging", "healthy weight",
  "healthy digestion", "mobility support", "skin coat", "high protein",
  "small breed", "large breed", "mini chunks", "dry", "wet", "pate",
  "cuts gravy", "chunks gravy", "classic ground", "variety pack", "food",
];

const TREAT_TERMS = [
  "treat", "treats", "snack", "snacks", "biscuit", "biscuits", "jerky", "chew", "chews",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:iams-sitemap-catalog -- --output=.tmp/iams-sitemap-catalog-targets.json

Options:
  --output=path.json      Write a backfill-compatible target manifest (default ${DEFAULT_OUTPUT})
  --sitemap=url           Explicit IAMS sitemap URL (${DEFAULT_SITEMAP_URL})
...
function assertConfig() {
  ...
  if (!/^https:\/\/www\.iams\.com\/sitemap\.xml$/i.test(sitemapUrl)) {
    usage("--sitemap must be https://www.iams.com/sitemap.xml.");
  }
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/www\.iams\.com\/products\/(dry|wet)\/[a-z0-9-]+$/i.test(url)) return "";
  return url;
}
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`IAMS sitemap fetch failed ${response.status}: ${url}`);
...
function inferIamsPetTypes(row) {
...
function isCandidateFoodRow(row) {
  const text = normalizeText(`${row.name} ${row.sourceUrl} ${row.retailerProductId}`);
  const petTypes = inferIamsPetTypes(row);
  if (!/^https:\/\/www\.iams\.com\/products\/(dry|wet)\//i.test(row.sourceUrl)) return false;
...
async function main() {
  ...
  console.log(`IAMS products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing IAMS sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("IAMS sitemap catalog target export failed:", err.message);
  process.exit(1);
});
