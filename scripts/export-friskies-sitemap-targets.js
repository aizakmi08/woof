...
const DEFAULT_OUTPUT = ".tmp/friskies-sitemap-catalog-targets.json";
const REST_PAGE_SIZE = 1000;
const USER_AGENT = "Mozilla/5.0 Woof catalog coverage export";
const BRAND = "Friskies";
...
const CORE_FOOD_TERMS = [
  "wet cat food", "dry cat food", "pate", "shreds", "prime filets",
  "tasty treasures", "indoor", "extra gravy", "meaty bits",
  "farm favorites", "ocean favorites", "fully loadd", "glazd infuzd",
  "variety pack", "gravy", "sauce", "dinner", "chicken", "turkey",
  "salmon", "tuna", "beef", "whitefish", "ocean fish", "seafood",
  "poultry", "liver", "giblets", "shrimp", "crab", "sardines",
  "surfin turfin", "seafood sensations", "indoor delights",
  "gravy swirld", "tender crunchd", "land sea adventures",
  "party packd",
];
const REJECT_TERMS = [
  "treat", "treats", "party mix", "playfuls", "lil shakes",
  "lil gravies", "bits n broth", "lil soups", "lil lickables",
  "lickable", "puree", "topper", "toppers", "complement",
  "complements", "supplement", "supplements", "broth", "bisque",
  "soup", "soups", "catnip", "pull n play", "snack", "snacks",
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:friskies-sitemap-catalog -- --output=.tmp/friskies-sitemap-catalog-targets.json
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .join("")
    .replace(/\bFriskies\b/g, "Friskies")
    .replace(/\bLoadd\b/g, "Loadd")
    .replace(/\bGlazd\b/g, "Glazd")
    .replace(/\bInfuzd\b/g, "Infuzd")
    .replace(/\bSwirld\b/g, "Swirld")
    .replace(/\bCrunchd\b/g, "Crunchd")
    .replace(/\s+([,])/g, "$1")
    .trim();
}
...
function cleanUrl(value) {
  ...
  if (!/^https:\/\/www\.purina\.com\/cats\/shop\/friskies-[a-z0-9-]+-(wet|dry)-cat-food(?:-[a-z0-9-]+)?$/i.test(url)) return "";
  return url;
}
...
function parsedProductPath(url) {
  try {
    ...
    if (!/^friskies-/.test(slug) || !/-(wet|dry)-cat-food(?:-|$)/.test(slug)) return null;
    return { slug };
  } catch {
    return null;
  }
...
function inferFriskiesPetTypes(row) {
...
function isCandidateFoodRow(row) {
  ...
  const petTypes = inferFriskiesPetTypes(row);
  if (!/^https:\/\/www\.purina\.com\/cats\/shop\/friskies-/i.test(row.sourceUrl)) return false;
...
function normalizeTarget(row) {
  ...
  return {
    ...
    cacheKey,
    source: "friskies_sitemap_export",
...
async function main() {
  ...
  const manifest = {
    ...
    source: "friskies_product_sitemap",
    sitemapUrl,
    includeExisting,
    skipExistingScan,
    scannedProducts,
  ...
  console.log(`Friskies products scanned: ${scannedProducts}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Friskies sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Friskies sitemap catalog target export failed:", err.message);
  process.exit(1);
});
