...
const MERRICK_PRODUCT_IMPORT_SERVICE_KEY =
  process.env.MERRICK_PRODUCT_IMPORT_SERVICE_KEY ||
  process.env.PRODUCT_LOOKUP_SERVICE_KEY ||
  process.env.SUPABASE_SERVICE_ROLE_KEY ||
  process.env.SUPABASE_SERVICE_KEY ||
  "";
const DEFAULT_INPUT = ".tmp/merrick-sitemap-catalog-targets.json";
const DEFAULT_EXPORT_PARSED = ".tmp/merrick-official-product-data.json";
const DEFAULT_REPORT = ".tmp/merrick-official-product-backfill-report.jsonl";
const USER_AGENT = "Mozilla/5.0 Woof official Merrick product import";
const BRAND = "Merrick";
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run backfill:merrick-official-products -- --dry-run
  MERRICK_PRODUCT_IMPORT_SERVICE_KEY=... npm run backfill:merrick-official-products

Options:
  --input=path.json           Merrick catalog target manifest (default ${DEFAULT_INPUT})
...
function assertConfig() {
  ...
  if (!fs.existsSync(inputPath)) usage(`Missing Merrick input manifest: ${path.relative(root, inputPath)}`);
  if (!dryRun && !MERRICK_PRODUCT_IMPORT_SERVICE_KEY) {
    usage("Set MERRICK_PRODUCT_IMPORT_SERVICE_KEY, PRODUCT_LOOKUP_SERVICE_KEY, or Supabase service key for official Merrick product writes.");
  }
...
function inferForm(sourceUrl, fallback = "") {
  ...
  if (/\b(fresh|fresh selects)\b/.test(text)) return "fresh";
  if (/\b(dry|kibble)\b/.test(text)) return "dry";
  if (/\b(wet|can|canned|pate|gravy|stew|dinner|entree|tray)\b/.test(text)) return "wet";
  return "";
}
...
function parseIngredients(html) {
  ...
  let block =
    raw.match(/<h3[^>]*>\s*Ingredients:?\s*<\/h3>\s*<p[^>]*>([\s\S]*?)<\/p>/i)?.[1] ||
    raw.match(/field--name-field-text-nutrition[\s\S]*?<h3[^>]*>\s*Ingredients:?\s*<\/h3>([\s\S]*?)(?:<h3|<\/div>)/i)?.[1] ||
    "";
  if (!block) {
    const field = raw.match(/field--name-field-text-nutrition[\s\S]*?field__item relative">([\s\S]*?)<\/div>\s*<div[^>]+field--name-field-ingredients-pdf/i)?.[1] || "";
    const paragraph = field.match(/<p[^>]*>([\s\S]*?)<\/p>/i)?.[1] || "";
    const paragraphText = stripTags(paragraph);
    if (
      paragraph &&
      paragraphText.split(",").length >= 5 &&
      !/\b(distributed by|daily feeding|recommended daily|calorie content|guaranteed analysis|aafco)\b/i.test(paragraphText)
    ) {
      block = paragraph;
    }
  }
  if (!block) return [];
  return splitIngredientText(block);
}
...
function parseNutrientPanel(html, sourceUrl) {
  ...
  const blocks = [...raw.matchAll(/<h3[^>]*>\s*Guaranteed Analysis:?\s*<\/h3>([\s\S]*?)(?:<h3|<\/div>)/gi)]
    .map((match) => match[1]);
  for (const block of blocks) {
    for (const row of block.matchAll(/<tr[^>]*>([\s\S]*?)<\/tr>/gi)) {
      const cells = [...row[1].matchAll(/<(?:td|th)[^>]*>([\s\S]*?)<\/(?:td|th)>/gi)].map((cell) => stripTags(cell[1]));
    ...
    for (const match of text.matchAll(/\b(Crude\s+Protein|Crude\s+Fat|Crude\s+Fib(?:er|re)|Moisture|Ash|Calcium|Phosphorus|Omega[-\s]*3|Omega[-\s]*6)[^0-9]{0,30}([\d.]+)\s*%/gi)) {
  ...
  let match = calorieText.match(/([\d,]+(?:\.\d+)?)\s*kcal\s*\/\s*kg/i);
  if (match) panel.calories_per_kg = Number(match[1].replace(/,/g, ""));
  match = calorieText.match(/([\d,]+(?:\.\d+)?)\s*kcal\s*\/\s*cup/i);
...
async function fetchText(url, attempt = 1) {
  ...
  if (response.status === 429 || response.status >= 500) {
    if (attempt >= 5) throw new Error(`Merrick official page fetch failed ${response.status}: ${url}`);
    const retryAfter = Number(response.headers.get("retry-after"));
    await sleep(Number.isFinite(retryAfter) ? retryAfter * 1000 : 1000 * attempt * attempt);
    return fetchText(url, attempt + 1);
  }
  if (!response.ok) throw new Error(`Merrick official page fetch failed ${response.status}: ${url}`);
...
async function fetchExistingCacheKeys() {
  ...
  const pageSize = 1000;
  while (true) {
    const url = `${base}?select=cache_key,product_name,brand&or=(brand.ilike.*Merrick*,product_name.ilike.*Merrick*,cache_key.ilike.*merrick*)&offset=${offset}&limit=${pageSize}`;
...
function exportParsed(targets, metadata) {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      source: "merrick_official_product_data",
...
function loadVerifiedReportKeys() {
  ...
  for (const line of fs.readFileSync(reportPath, "utf8").split(/\r?\n/)) {
    ...
    try {
      ...
      if (entry?.event === "merrick_official_product_result" && entry?.status === "verified_saved" && entry?.cache_key) {
...
async function writeTarget(target) {
  ...
  const response = await fetch(`${supabaseBase()}/rest/v1/rpc/${rpc}`, {
    method: "POST",
    headers: restHeaders(MERRICK_PRODUCT_IMPORT_SERVICE_KEY),
...
async function processWrites(targets) {
  ...
  async function worker() {
    while (nextIndex < selected.length) {
      ...
      appendReport({
        event: "merrick_official_product_result",
...
async function main() {
  ...
  for (const target of planned) {
    ...
    try {
      ...
      console.warn(`Skipped Merrick target: ${target.name} (${error?.message || error})`);
      continue;
    }
    if (!result.target) {
      skipped.parse += 1;
  ...
  console.log(`Merrick official input targets: ${inputTargets.length}`);
  console.log(`Existing Merrick keys skipped: ${skipped.existing}`);
