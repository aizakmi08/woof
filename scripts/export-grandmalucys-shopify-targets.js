...
const DEFAULT_OUTPUT = ".tmp/grandmalucys-shopify-catalog-targets.json";
...
const REJECT_TERMS = [
  "sample", "1lb", "1 lb", "pre mix", "pre-mix", "premix",
  "organic", "pumpkin pouch", "singles", "simple replacement",
  "top it", "top-it", "topper", "toppers", "bundle", "3 for",
  "gift", "gift card", "treat", "treats", "snack", "snacks",
  "supplement", "supplements", "chew", "chews", "broth",
];
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:grandmalucys-shopify-catalog -- --output=.tmp/grandmalucys-shopify-catalog-targets.json
...
function isCandidateFoodProduct(product) {
  ...
  if (!KEEP_HANDLES.has(handle)) return false;
...
function inferGrandmaLucysPetTypes(target) {
  const inferred = inferPetTypes({
    brand: BRAND,
    product_name: `dog ${target.name} ${target.sourceUrl}`,
    cache_key: normalizeCacheKey(`dog ${BRAND} ${target.name} ${target.sourceUrl}`),
    source: "grandmalucys_shopify_json",
...
function normalizeTarget(product) {
  ...
  const name = addSpeciesQualifier(cleanProductName(product.title));
  const sourceUrl = `https://www.grandmalucys.com/products/${product.handle}`;
  const draft = { name, sourceUrl };
  const petTypes = inferGrandmaLucysPetTypes(draft);
  ...
  return {
    ...
    cacheKey,
    source: "grandmalucys_shopify_export",
    sourceQuality: "official_brand_shopify_json_allowlist",
    sourceUrl,
    imageUrl: "",
    barcode: "",
    retailerProductId: String(product.id || product.handle || ""),
    form: "fresh",
    productType: "freeze_dried_dog_food",
...
async function fetchJson(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Grandma Lucy's products fetch failed ${response.status}: ${url}`);
...
async function fetchProducts() {
  ...
  if (!Array.isArray(data?.products)) throw new Error("Grandma Lucy's products feed did not return a products array.");
...
async function main() {
  ...
  const manifest = {
    ...
    source: "grandmalucys_shopify_products",
    productsUrl,
    includeExisting,
    skipExistingScan,
  ...
  console.log(`Grandma Lucy's products scanned: ${products.length}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Grandma Lucy's Shopify targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Grandma Lucy's Shopify catalog target export failed:", err.message);
  process.exit(1);
});
