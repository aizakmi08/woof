...
const source = fs.readFileSync(path.join(root, "scripts/export-purinaproplan-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Purina Pro Plan sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://www.purina.com/sitemap.xml") &&
    source.includes("const BRAND = \"Purina Pro Plan\"") &&
    ...
    source.includes("www\\.purina\\.com\\/(cats|dogs)\\/shop\\/pro-plan-") &&
    !source.includes("product-lookup") &&
    !source.includes("cheerio") &&
    !source.includes("puppeteer"),
  "exporter must read the official Purina sitemap index, derive titles from Purina Pro Plan shop slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"complete essentials\"") &&
    source.includes("\"specialized nutrition\"") &&
    source.includes("\"sensitive skin stomach\"") &&
    source.includes("\"sport\"") &&
    source.includes("\"development\"") &&
    source.includes("\"performance\"") &&
    source.includes("\"shredded blend\"") &&
    source.includes("\"puppy\"") &&
    source.includes("\"kitten\"") &&
    source.includes("\"live clear\"") &&
    source.includes("\"veterinary diets\"") &&
    source.includes("\"advantedge\"") &&
    ...
    source.includes("\"fortiflora\"") &&
    source.includes("\"calming care\"") &&
    source.includes("\"hydracare\"") &&
    source.includes("\"soft chews\"") &&
    !source.includes("\"probiotic\"") &&
    !source.includes("\"probiotics\"") &&
    source.includes("NON_FOOD_TERMS") &&
    source.includes("function inferPurinaProPlanPetTypes(row)") &&
    source.includes("return [row.petType]") &&
    source.includes("function parsedProductPath(url)") &&
    source.includes("function formFromSlug(slug, petType)") &&
    source.includes("if (petType === \"dog\" && /-dry-dog(?:-|$)/.test(slug)) return \"dry\"") &&
    source.includes("if (/-dry$/.test(slug)) return \"dry\"") &&
    source.includes("const petType = segments[0] === \"cats\" ? \"cat\" : \"dog\"") &&
    ...
    source.includes("if (!/^https:\\/\\/www\\.purina\\.com\\/(cats|dogs)\\/shop\\/pro-plan-/i.test(row.sourceUrl)) return false") &&
    source.includes("if (!formFromSlug(row.retailerProductId, row.petType)) return false") &&
  ...
  "exporter must strictly filter Purina Pro Plan sitemap rows to core dry/wet cat and dog foods while rejecting treats, toppers, supplements, accessories, and non-food rows without dropping complete foods that mention probiotics"
);
...
assert(
  source.includes("source: \"purinaproplan_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"purinaproplan_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:purinaproplan-sitemap-catalog": "node scripts/export-purinaproplan-sitemap-targets.js"') &&
    packageJson.includes('"test:purinaproplan-sitemap-catalog-export": "node scripts/test-purinaproplan-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:purinaproplan-sitemap-catalog-export"),
  "package scripts must expose Purina Pro Plan sitemap exporter and include its guard in test:guards"
);

console.log("Purina Pro Plan sitemap catalog export guard passed");
