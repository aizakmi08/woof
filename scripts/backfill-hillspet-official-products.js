...
const HILLSPET_PRODUCT_IMPORT_SERVICE_KEY =
  process.env.HILLSPET_PRODUCT_IMPORT_SERVICE_KEY ||
  process.env.PRODUCT_LOOKUP_SERVICE_KEY ||
  process.env.SUPABASE_SERVICE_ROLE_KEY ||
  process.env.SUPABASE_SERVICE_KEY ||
  "";
const DEFAULT_INPUT = ".tmp/hillspet-sitemap-catalog-targets.json";
const DEFAULT_EXPORT_PARSED = ".tmp/hillspet-official-product-data.json";
const DEFAULT_REPORT = ".tmp/hillspet-official-product-backfill-report.jsonl";
const USER_AGENT = "Mozilla/5.0 Woof official Hillspet product import";
const BRAND = "Hill's";
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run backfill:hillspet-official-products -- --dry-run
  HILLSPET_PRODUCT_IMPORT_SERVICE_KEY=... npm run backfill:hillspet-official-products

Options:
  --input=path.json           Hill's catalog target manifest (default ${DEFAULT_INPUT})
...
function assertConfig() {
  ...
  if (!fs.existsSync(inputPath)) usage(`Missing Hill's input manifest: ${path.relative(root, inputPath)}`);
  if (!dryRun && !HILLSPET_PRODUCT_IMPORT_SERVICE_KEY) {
    usage("Set HILLSPET_PRODUCT_IMPORT_SERVICE_KEY, PRODUCT_LOOKUP_SERVICE_KEY, or Supabase service key for official Hill's product writes.");
  }
...
function normalizeText(value) {
  return asciiFold(value)
    .toLowerCase()
    .replace(/&amp;/g, "&")
    .replace(/hill039s|hill39s|hill s|hill's|hills/g, "hills")
    .replace(/[''\u2122\u00AE+]/g, " ")
    .replace(/[-_/,%]+/g, " ")
    .replace(/[^a-z0-9\s&]/g, " ")
    .replace(/\bamp\b/g, " ")
...
function inferBrand(value) {
  ...
  if (text.includes("prescription diet")) return "Hill's Prescription Diet";
  if (text.includes("science diet")) return "Hill's Science Diet";
  if (text.includes("healthy advantage")) return "Hill's Healthy Advantage";
  return BRAND;
}

function inferForm(sourceUrl, fallback = "") {
  const text = normalizeText(`${sourceUrl} ${fallback}`);
  if (/\b(wet|can|canned|pate|pouch|stew|entree|loaf|tray)\b/.test(text)) return "wet";
...
function normalizeTarget(entry) {
  ...
  const brand = cleanProductName(entry?.brand || inferBrand(`${sourceUrl} ${name}`)) || BRAND;
...
function isMultiPack(target) {
  return /\b(variety\s+pack|sample\s+pack|starter\s+kit|multipack|multi\s+pack)\b/i.test(`${target.name} ${target.sourceUrl} ${target.cacheKey}`);
}
...
function extractAccordionPanel(html, titlePattern) {
  const raw = String(html || "");
  for (const match of raw.matchAll(/<span class="cmp-accordion__title">([\s\S]*?)<\/span>([\s\S]*?)(?=<span class="cmp-accordion__title">|<footer\b|<\/main>|$)/gi)) {
    const title = stripTags(match[1]);
    if (titlePattern.test(title)) return match[2] || "";
  }
  return "";
}
...
function parseOfficialProductName(html, fallback) {
  ...
  return cleanProductName(jsonName || stripTags(h1) || fallback);
}
...
function parseImageUrl(html) {
  ...
  if (decoded.startsWith("/")) return `https://www.hillspet.com${decoded}`.slice(0, 500);
...
function parseIngredients(html) {
  const panel = extractAccordionPanel(html, /^Ingredients$/i);
  const text = stripTags(panel)
    .replace(/^Ingredients\s+/i, "")
    .replace(/\s+(Feeding Tips|Key Features|Average Nutrient).*$/i, "")
    .trim();
  return splitIngredients(text)
    ...
    .filter((ingredient) =>
      ...
      !/^(food form|flavor|sizes?)\b/i.test(ingredient) &&
...
function normalizePanel(panel) {
  const clean = { basis: String(panel?.basis || "dry-matter") };
...
function parseNutrientPanel(html, sourceUrl) {
  const panelHtml = extractAccordionPanel(html, /Average Nutrient/i);
  const panel = { basis: "dry-matter", source_url: sourceUrl };
  const nutrientTable = [...panelHtml.matchAll(/<table[^>]*>([\s\S]*?)<\/table>/gi)]
    .map((match) => match[1])
    .find((table) => /Dry\s*Matter/i.test(table)) || "";
  ...
  const text = stripTags(panelHtml || html);
  ...
  match = text.match(/([\d,]+(?:\.\d+)?)\s*kcal\s*\/\s*can/i);
  if (match) panel.calories_per_can = Number(match[1].replace(/,/g, ""));
...
async function fetchText(url, accept = "text/html,application/xhtml+xml;q=0.9,*/*;q=0.8", attempt = 1) {
  ...
  if (response.status === 403 || response.status === 429 || response.status >= 500) {
    if (attempt >= 5) throw new Error(`Hill's official fetch failed ${response.status}: ${url}`);
  ...
  if (!response.ok) throw new Error(`Hill's official fetch failed ${response.status}: ${url}`);
...
async function fetchExistingCacheKeys() {
  ...
  const pageSize = 1000;
  while (true) {
    const url = `${base}?select=cache_key,product_name,brand&or=(brand.ilike.*Hill*,product_name.ilike.*Hill*,cache_key.ilike.*hill*,brand.ilike.*Science%20Diet*,product_name.ilike.*Science%20Diet*)&offset=${offset}&limit=${pageSize}`;
...
async function parseTarget(target) {
  ...
  return {
    target: {
      ...
      brand: inferBrand(`${target.sourceUrl} ${officialProductName} ${target.brand}`),
...
function summarizeParsed(targets) {
  ...
  for (const target of targets) {
    ...
    if (target.nutrientPanel) {
      withNutrients += 1;
      if (
        target.nutrientPanel.calories_per_cup != null ||
        target.nutrientPanel.calories_per_kg != null ||
        target.nutrientPanel.calories_per_can != null
      ) {
        withCalories += 1;
      }
    }
...
function exportParsed(targets, metadata) {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      source: "hillspet_official_product_data",
...
function loadVerifiedReportKeys() {
  ...
  for (const line of fs.readFileSync(reportPath, "utf8").split(/\r?\n/)) {
    ...
    try {
      ...
      if (entry?.event === "hillspet_official_product_result" && entry?.status === "verified_saved" && entry?.cache_key) {
...
async function writeTarget(target) {
  ...
  const response = await fetch(`${supabaseBase()}/rest/v1/rpc/${rpc}`, {
    method: "POST",
    headers: restHeaders(HILLSPET_PRODUCT_IMPORT_SERVICE_KEY),
...
async function processWrites(targets) {
  ...
  async function worker() {
    while (nextIndex < selected.length) {
      ...
      appendReport({
        event: "hillspet_official_product_result",
...
async function main() {
  ...
  for (const target of planned) {
    ...
    try {
      ...
      console.warn(`Skipped Hill's target: ${target.name} (${error?.message || error})`);
      continue;
    }
    if (!result.target) {
      skipped.parse += 1;
  ...
  console.log(`Hill's official input targets: ${inputTargets.length}`);
  console.log(`Existing Hill's keys skipped: ${skipped.existing}`);
