...
const source = fs.readFileSync(path.join(root, "scripts/export-freshpet-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Freshpet sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  ...
  "exporter must read the official Freshpet sitemap, derive titles from product slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"small dog\"") &&
    source.includes("\"large dog\"") &&
    source.includes("CAT_TERMS") &&
    source.includes("\"kitten\"") &&
    source.includes("CORE_FOOD_TERMS") &&
    source.includes("\"complete meal\"") &&
    source.includes("\"fresh from the kitchen\"") &&
    ...
    source.includes("function inferFreshpetPetTypes(row)") &&
    ...
    source.includes("if (!/^https:\\/\\/www\\.freshpet\\.com\\/products\\//i.test(row.sourceUrl)) return false") &&
  ...
  "exporter must strictly filter Freshpet sitemap rows to explicit dog/cat foods and reject supplements, treats, and non-food rows by default"
);
...
assert(
  source.includes("source: \"freshpet_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"freshpet_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:freshpet-sitemap-catalog": "node scripts/export-freshpet-sitemap-targets.js"') &&
    packageJson.includes('"test:freshpet-sitemap-catalog-export": "node scripts/test-freshpet-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:freshpet-sitemap-catalog-export"),
  "package scripts must expose Freshpet sitemap exporter and include its guard in test:guards"
);

console.log("Freshpet sitemap catalog export guard passed");
