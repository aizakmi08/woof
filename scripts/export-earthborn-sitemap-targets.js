...
const DEFAULT_OUTPUT = ".tmp/earthborn-sitemap-catalog-targets.json";
...
function usage(message) {
  ...
  console.error(`
Usage:
  npm run export:earthborn-sitemap-catalog -- --output=.tmp/earthborn-sitemap-catalog-targets.json
...
function titleCaseProductName(value) {
  ...
  return String(value || "")
    ...
    .replace(/\bCatcciatori\b/g, "Catcciatori")
    .replace(/\bFricatssee\b/g, "Fricatssee")
    .trim();
}
...
function parseProductUrl(url) {
  try {
    ...
    const speciesPath = parts[1];
    const line = parts[2];
    const slug = parts[3];
    const petType = speciesPath === "dog-food" ? "dog" : speciesPath === "cat-food" ? "cat" : "";
    if (!petType || !line || !slug) return null;
...
function inferEarthbornPetTypes(target) {
...
function normalizeTarget(url) {
  ...
  const name = addSpeciesQualifier(nameFromSlug(row.slug), row.petType);
  const draft = { ...row, name, sourceUrl: url };
  const petTypes = inferEarthbornPetTypes(draft);
  ...
  return {
    ...
    cacheKey,
    source: "earthborn_sitemap_export",
    sourceQuality: "official_brand_sitemap_slug",
    sourceUrl: url,
    imageUrl: "",
    barcode: "",
    retailerProductId: row.slug,
    form: formFromPath(row),
    productType: `${row.line}_food`,
...
async function fetchText(url) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Earthborn sitemap fetch failed ${response.status}: ${url}`);
...
async function main() {
  ...
  console.log(`Earthborn sitemap URLs scanned: ${rawUrls.length}`);
  console.log(`Candidate Earthborn product URLs: ${urls.length}`);
  console.log(`Existing product_data keys: ${existing.size}`);
  console.log(`Missing Earthborn sitemap targets exported: ${targets.length}`);
  console.log(`Skipped: ${JSON.stringify(skipped)}`);
  console.log(`Output: ${outputPath}`);
}

main().catch((err) => {
  console.error("Earthborn sitemap catalog target export failed:", err.message);
  process.exit(1);
});
