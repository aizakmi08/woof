...
const source = fs.readFileSync(path.join(root, "scripts/export-meijer-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Meijer sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://www.meijer.com/sitemap.xml") &&
    source.includes("product\\.\\d+\\.sitemap\\.xml") &&
    source.includes("\\/content\\/dam\\/meijer\\/applications\\/sitemap\\/products\\/") &&
  ...
  "exporter must discover Meijer product sitemaps, derive titles from slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"glofish\"") &&
    source.includes("\"cat eye\"") &&
    source.includes("BRAND_HINTS") &&
    !source.includes("\"Kroger\"") &&
    source.includes("\"Meijer\"") &&
  ...
  "exporter must strictly filter Meijer's mixed grocery sitemap to complete dog/cat food rows and reject human-food/accessory/supplement/treat rows by default"
);
...
assert(
  source.includes("source: \"meijer_sitemap_export\"") &&
    source.includes("sourceQuality: \"retailer_sitemap_slug\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"meijer_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:meijer-sitemap-catalog": "node scripts/export-meijer-sitemap-targets.js"') &&
    packageJson.includes('"test:meijer-sitemap-catalog-export": "node scripts/test-meijer-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:meijer-sitemap-catalog-export"),
  "package scripts must expose Meijer sitemap exporter and include its guard in test:guards"
);

console.log("Meijer sitemap catalog export guard passed");
