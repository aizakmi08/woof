...
const source = fs.readFileSync(path.join(root, "scripts/export-sheba-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Sheba sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://www.sheba.com/sitemap.xml") &&
    source.includes("const BRAND = \"Sheba\"") &&
    source.includes("--sitemap=url") &&
    source.includes("--include-existing") &&
    source.includes("--include-treats") &&
    source.includes("--include-complements") &&
    ...
    source.includes("www\\.sheba\\.com\\/products\\/(wet|treats)") &&
    !source.includes("product-lookup") &&
    !source.includes("cheerio") &&
    !source.includes("puppeteer"),
  "exporter must read the official Sheba sitemap, derive titles from product slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("if (!/^https:\\/\\/www\\.sheba\\.com\\/products\\/(wet|treats)\\//i.test(row.sourceUrl)) return false") &&
    source.includes("if (!includeComplements && SUPPLEMENTAL_FOOD_TERMS.some((term) => hasTerm(text, term))) return false") &&
    source.includes("if (!includeTreats && TREAT_TERMS.some((term) => hasTerm(text, term))) return false"),
  "exporter must strictly filter Sheba sitemap rows to core wet cat foods while rejecting treats, bisques, purees, broths, complements, supplements, and non-food rows by default"
);
...
assert(
  source.includes("source: \"sheba_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"sheba_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:sheba-sitemap-catalog": "node scripts/export-sheba-sitemap-targets.js"') &&
    packageJson.includes('"test:sheba-sitemap-catalog-export": "node scripts/test-sheba-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:sheba-sitemap-catalog-export"),
  "package scripts must expose Sheba sitemap exporter and include its guard in test:guards"
);

console.log("Sheba sitemap catalog export guard passed");
