...
const source = fs.readFileSync(path.join(root, "scripts/export-friskies-sitemap-targets.js"), "utf8");
const packageJson = fs.readFileSync(path.join(root, "package.json"), "utf8");

function assert(condition, message) {
  if (!condition) {
    console.error(`Friskies sitemap catalog export guard failed: ${message}`);
    process.exit(1);
  }
}
...
assert(
  source.includes("https://www.purina.com/sitemap.xml") &&
    source.includes("const BRAND = \"Friskies\"") &&
    ...
    source.includes("www\\.purina\\.com\\/cats\\/shop\\/friskies-") &&
    !source.includes("product-lookup") &&
    !source.includes("cheerio") &&
    !source.includes("puppeteer"),
  "exporter must read the official Purina sitemap index, derive titles from Friskies shop slugs, and avoid product-page scraping or product-lookup calls"
);
...
assert(
  source.includes("require(\"./catalog-pet-type\")") &&
    ...
    source.includes("\"prime filets\"") &&
    source.includes("\"tasty treasures\"") &&
    source.includes("\"fully loadd\"") &&
    source.includes("\"glazd infuzd\"") &&
    source.includes("REJECT_TERMS") &&
    source.includes("\"party mix\"") &&
    source.includes("\"lil shakes\"") &&
    source.includes("\"lil gravies\"") &&
    source.includes("\"bits n broth\"") &&
    source.includes("\"lil soups\"") &&
    source.includes("\"lil lickables\"") &&
    source.includes("\"topper\"") &&
    source.includes("\"broth\"") &&
    source.includes("NON_FOOD_TERMS") &&
    source.includes("function inferFriskiesPetTypes(row)") &&
    ...
    source.includes("if (!/^https:\\/\\/www\\.purina\\.com\\/cats\\/shop\\/friskies-/i.test(row.sourceUrl)) return false") &&
  ...
  "exporter must strictly filter Friskies sitemap rows to core dry/wet cat foods while rejecting treats, toppers, soups, broths, supplements, accessories, and non-food rows"
);
...
assert(
  source.includes("source: \"friskies_sitemap_export\"") &&
...
assert(
  source.includes("generatedAt: new Date().toISOString()") &&
    source.includes("source: \"friskies_product_sitemap\"") &&
...
assert(
  packageJson.includes('"export:friskies-sitemap-catalog": "node scripts/export-friskies-sitemap-targets.js"') &&
    packageJson.includes('"test:friskies-sitemap-catalog-export": "node scripts/test-friskies-sitemap-catalog-export-guards.js"') &&
    packageJson.includes("npm run test:friskies-sitemap-catalog-export"),
  "package scripts must expose Friskies sitemap exporter and include its guard in test:guards"
);

console.log("Friskies sitemap catalog export guard passed");
