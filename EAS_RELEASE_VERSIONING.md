...
- Local release line: `package.json`, `package-lock.json`, and `app.json` use `1.2.3`.
...
- 2026-08-26 App Store Connect verification: build version `1.2.2`, build `55`, is Ready for Distribution.
- 2026-08-26 EAS verification: the remote iOS build-number counter is `55`; the next production build will auto-increment to a value greater than App Store Connect/TestFlight build `55` for version `1.2.3`.
...
- Remote EAS app version must match the intended release line, currently `1.2.3`.
- The next iOS build number must be greater than App Store Connect/TestFlight build `55`.
...
Use a higher number if App Store Connect already has build `56` or later.
...
4. The submitted TestFlight/App Store build row shows marketing version `1.2.3` and a build number greater than `55`.
