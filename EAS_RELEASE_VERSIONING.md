...
- Local release line: `package.json`, `package-lock.json`, and `app.json` use `1.2.2`.
...
- 2026-08-24 EAS verification: production build `54` completed for version `1.2.2`. The remote iOS build-number counter is `54`; the next production build will auto-increment to build `55` for version `1.2.2`.
...
- Remote EAS app version must match the intended release line, currently `1.2.2`.
- The next iOS build number must be greater than App Store Connect/TestFlight build `54`.
...
```sh
npx eas-cli@latest build:version:set -p ios --build-number 55
```

Use a higher number if App Store Connect already has build `55` or later.
...
4. The submitted TestFlight/App Store build row shows marketing version `1.2.2` and a build number greater than `54`.
